codeunit 10871 "Create Currency Exc. Rate FR"
{
    SingleInstance = true;
    ...
    var
        ...
        CurrExchDate: Date;
    begin
        CurrExchDate := CalcDate('<CY - 3Y + 1D>', WorkDate());
        ContosoCurrency.InsertCurrencyExchangeRate(CreateCurrency.GBP(), CurrExchDate, 100, 100, 154.8801, 154.8801);

        CurrExchDate := DMY2Date(2, 1, 2013);
        ContosoCurrency.InsertCurrencyExchangeRate(CreateCurrency.GBP(), CurrExchDate, 100, 100, 165.793, 165.793);

        CurrExchDate := DMY2Date(2, 4, 2013);
        ContosoCurrency.InsertCurrencyExchangeRate(CreateCurrency.GBP(), CurrExchDate, 100, 100, 159.2372, 159.2372);
    end;
    ...
    begin
        ...
        if Rec."Currency Code" = Currency.AED() then
            if Rec."Starting Date" = CurrExchDate then
