...
codeunit 10692 "Generate SAF-T 1.3 File"
{
    ...
    begin
        ...
        repeat
            ...
            ExportGLAccount(
                SAFTGLAccountMapping."G/L Account No.",
    ...
    local procedure ExportGLAccount(GLAccNo: Code[20]; GroupingCategory: Code[20]; GroupingNo: Code[20]; StartingDate: Date; EndingDate: Date)
    var
    ...
    begin
        ...
        if SAFTMapping.Get(SAFTMapping."Mapping Type"::"Income Statement", GroupingCategory, GroupingNo) then
    ...
    begin
        ...
        ExportBalanceAccount(CustomerPostingGroup."Receivables Account", OpeningDebitBalance, OpeningCreditBalance, ClosingDebitBalance, ClosingCreditBalance);
    ...
    begin
        ...
        ExportBalanceAccount(VendorPostingGroup."Payables Account", OpeningDebitBalance, OpeningCreditBalance, ClosingDebitBalance, ClosingCreditBalance);
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            if GLEntryVATEntryLink.FindFirst() then begin
                ...
                IsDebitVATAmount := GLEntry."Debit Amount" <> 0;
            end;
            TaxAmountNode := IsDebitVATAmount ? 'DebitTaxAmount' : 'CreditTaxAmount';
            ExportAmountInfo(TaxAmountNode, abs(VATEntry.Amount));
        end;
        SAFTXMLHelper.FinalizeXMLNode();
    end;
    ...
    local procedure ExportBalanceAccount(AccountID: Text; OpeningDebitBalance: Decimal; OpeningCreditBalance: Decimal; ClosingDebitBalance: Decimal; ClosingCreditBalance: Decimal)
    begin
        SAFTXMLHelper.AddNewXMLNode('BalanceAccount', '');
        SAFTXMLHelper.AppendXMLNode('AccountID', AccountID);
        if OpeningDebitBalance = 0 then
            SAFTXMLHelper.AppendXMLNode('OpeningCreditBalance', FormatAmount(OpeningCreditBalance))
        else
            SAFTXMLHelper.AppendXMLNode('OpeningDebitBalance', FormatAmount(OpeningDebitBalance));
