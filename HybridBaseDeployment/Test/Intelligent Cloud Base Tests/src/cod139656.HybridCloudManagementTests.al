codeunit 139656 "Hybrid Cloud Management Tests"
{
    ...
    [Test]
    procedure SetDetailErrorMessageOnInstallApp()
    var
        ...
        i: Integer;
    begin
        // [SCENARIO] Installing the app moves the Errors [Blob] into the Error Message [Text] field

        // [GIVEN] Several detail records exist with the old blob field populated
        RunId := CreateGuid();
        for i := 1 to 10 do begin
            HybridReplicationDetail.Init();
            HybridReplicationDetail."Run ID" := RunId;
            HybridReplicationDetail."Table Name" := StrSubstNo('Dummy Table %1', i);
            HybridReplicationDetail."Company Name" := CopyStr(CompanyName(), 1, 30);
            HybridReplicationDetail.Errors.CreateOutStream(BlobStream, TextEncoding::UTF8);
            BlobStream.WriteText(StrSubstNo('Error %1', i));
            HybridReplicationDetail.Insert();
        end;

        // [WHEN] The extension app is installed
        HybridCloudInstall.UpdateHybridReplicationDetailRecords();

        // [THEN] The Error Message field is populated based on the Errors blob field
        for i := 1 to 10 do begin
            HybridReplicationDetail.Get(RunId, StrSubstNo('Dummy Table %1', i), CompanyName());
            Assert.AreEqual(StrSubstNo('Error %1', i), HybridReplicationDetail."Error Message", 'Error message not set');
        end;
    end;
