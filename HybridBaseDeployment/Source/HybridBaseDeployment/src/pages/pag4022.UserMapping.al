page 4022 "Migration User Mapping"
{
    ...
    begin
        GroupVisible := true;
        UserIds.SetFilter("Authentication Email", '<>%1', '');
        FillUserIDList();
    end;

    var
        UserIds: Record "User" temporary;
    ...
    begin
        ...
        UserMappingSource.FindFirst();
        repeat
            If not Rec.Get(UserMappingSource."User ID") then begin
                Rec.Init();
                Rec."Source User ID" := UserMappingSource."User ID";
                if FoundValidUser(UserMappingSource."Authentication Object ID", UserMappingSource."User Security ID", UserMappingSource."Name Identifier", UserMappingSource."Authentication Email", UserName) then
    ...
    local procedure FoundValidUser(AuthenticationObjectID: Text[80]; UserSecurityID: Guid; NameIdentifier: Text[250]; AuthenticationEmail: Text[50]; var UserID: Code[50]): Boolean;
    var
        User: Record User;
