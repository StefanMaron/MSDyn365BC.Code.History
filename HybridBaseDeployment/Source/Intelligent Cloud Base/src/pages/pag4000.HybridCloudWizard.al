page 4000 "Hybrid Cloud Setup Wizard"
{
    ...
    begin
        if not Backwards then
            HybridCloudManagement.HandleShowIRInstructionsStep(HybridProductType, RuntimeNameTxt, RuntimeKeyTxt);
    ...
    begin
        if not Backwards and IsChanged then begin
            HybridCloudManagement.HandleShowCompanySelectionStep(HybridProductType, SqlConnectionStringTxt, ConvertSqlServerTypeToText(), RuntimeNameTxt);
            IsChanged := false;
        end;
