codeunit 139733 "APIV1 - Attachments E2E"
{
    ...
    begin
        ...
        IncomingDocumentAttachment.SetRange(Id, AttachmentId);
        Assert.IsTrue(IncomingDocumentAttachment.IsEmpty(), 'The attachment should be deleted.');
    end;
    ...
    local procedure GetJournalBatchId(var DocumentRecordRef: RecordRef): Guid
    var
        GenJournalLine: Record "Gen. Journal Line";
        DataTypeManagement: Codeunit "Data Type Management";
        IdFieldRef: FieldRef;
        Id: Guid;
    begin
        IF DataTypeManagement.FindFieldByName(DocumentRecordRef, IdFieldRef, GenJournalLine.FIELDNAME("Journal Batch Id")) THEN
            EVALUATE(Id, FORMAT(IdFieldRef.VALUE()));
        EXIT(Id);
    end;
    ...
    begin
        ...
        IF IncludeID THEN
            AttachmentJSON := LibraryGraphMgt.AddPropertytoJSON(AttachmentJSON, 'id', FormatGuid(IncomingDocumentAttachment.Id));
