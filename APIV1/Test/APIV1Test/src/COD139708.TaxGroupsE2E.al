codeunit 139708 "Tax Groups E2E"
{
    ...
    var
        LibraryGraphMgt: Codeunit "Library - Graph Mgt";
    begin
        TaxGroupJSON := LibraryGraphMgt.AddPropertytoJSON('', 'id', FormatGuid(TaxGroupBuffer.Id));
    ...
    begin
        IF GeneralLedgerSetup.UseVat() THEN BEGIN
            VATProductPostingGroup.SetRange(Id, TaxGroupId);
            Assert.IsTrue(VATProductPostingGroup.IsEmpty(), 'VATProductPostingGroup should be deleted.');
        END ELSE BEGIN
            TaxGroup.SetRange(Id, TaxGroupId);
            Assert.IsTrue(TaxGroup.IsEmpty(), 'TaxGroup should be deleted.');
        END;
    end;
