...
codeunit 11781 "VAT Attribute Code Mgt. CZL"
{
    ...
    begin
        VATAttributeCodeMgt.SetOverwriteData(OverwriteData);
        BindSubscription(VATAttributeCodeMgt);
        ...
        UnbindSubscription(VATAttributeCodeMgt);
    end;
#if not CLEAN26
    ...
    begin
        InsertVATAttributeCode(
            VATStatementTemplateName, VATAttributeCodeCZL.BuildVATAttributeCode(XmlFormatCode, LineNo, Apendix), Description, XmlCode);
    end;

    internal procedure InsertVATAttributeCode(VATStatementTemplateName: Code[10]; AttributeCode: Code[20]; Description: Text[100]; XmlCode: Code[20])
    var
        VATAttributeCodeCZL: Record "VAT Attribute Code CZL";
        Exists: Boolean;
    begin
