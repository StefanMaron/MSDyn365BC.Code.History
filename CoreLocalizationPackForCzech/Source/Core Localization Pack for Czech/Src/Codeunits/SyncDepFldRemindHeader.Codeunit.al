...
codeunit 31181 "Sync.Dep.Fld-RemindHeader CZL"
{
    ...
    local procedure SyncOnAfterValidateBankNo(var Rec: Record "Reminder Header")
    var
        SyncLoopingHelper: Codeunit "Sync. Looping Helper";
    begin
        if SyncLoopingHelper.IsFieldSynchronizationSkipped(Database::"Reminder Header", Rec.FieldNo("Bank No.")) then
            exit;

        SyncLoopingHelper.SkipFieldSynchronization(SyncLoopingHelper, Database::"Reminder Header", Rec.FieldNo("Bank Account Code CZL"));
        Rec.Validate("Bank Account Code CZL", Rec."Bank No.");
        SyncLoopingHelper.RestoreFieldSynchronization(Database::"Reminder Header", Rec.FieldNo("Bank Account Code CZL"));
    end;

    [EventSubscriber(ObjectType::Table, Database::"Reminder Header", 'OnAfterValidateEvent', 'Bank Account Code CZL', false, false)]
    local procedure SyncOnAfterValidateBankAccountCodeCZL(var Rec: Record "Reminder Header")
    var
        SyncLoopingHelper: Codeunit "Sync. Looping Helper";
    begin
        if SyncLoopingHelper.IsFieldSynchronizationSkipped(Database::"Reminder Header", Rec.FieldNo("Bank Account Code CZL")) then
            exit;

        SyncLoopingHelper.SkipFieldSynchronization(SyncLoopingHelper, Database::"Reminder Header", Rec.FieldNo("Bank No."));
