...
codeunit 5262 "Import Audit Data Mgt."
{
    ...
    var
        ...
        AutoGeneratedCategoryNoFormatTxt: label 'CAT%1', Locked = true, Comment = '%1 = sequential number formatted as 000001';
    ...
    /// For category codes exceeding 20 characters, auto-generates "CAT000001" format numbers as primary key and stores original value in "Extended No." field.
    /// </remarks>
    ...
    var
        ...
        StandardAccount: Record "Standard Account";
        TempChildXMLBuffer: Record "XML Buffer" temporary;
        CategoryCodeValue: Text[500];
        CategoryCode: Code[20];
        LastExtendedCategoryNo: Text[500];
        AutoGeneratedCategoryNo: Integer;
    begin
        LastExtendedCategoryNo := '';
        AutoGeneratedCategoryNo := 0;

        repeat
            if not TempXMLBuffer.HasChildNodes() then
                Error(NotPossibleToParseMappingXMLFileErr, GroupingCodesTxt);
            TempXMLBuffer.FindChildElements(TempChildXMLBuffer);
            CategoryCodeValue := CopyStr(TempChildXMLBuffer.Value, 1, MaxStrLen(CategoryCodeValue));

            // Handle category codes that exceed 20 characters
            if StrLen(CategoryCodeValue) > MaxStrLen(CategoryCode) then begin
                if CategoryCodeValue <> LastExtendedCategoryNo then begin
                    StandardAccountCategory.Init();
                    StandardAccountCategory."Standard Account Type" := StandardAccountType;
                    AutoGeneratedCategoryNo += 1;
                    StandardAccountCategory."No." := CopyStr(StrSubstNo(AutoGeneratedCategoryNoFormatTxt, Format(AutoGeneratedCategoryNo, 6, '<Integer,6><Filler Character,0>')), 1, MaxStrLen(StandardAccountCategory."No."));
                    StandardAccountCategory."Extended No." := CategoryCodeValue;
                    TempChildXMLBuffer.Next();
                    ...
                    LastExtendedCategoryNo := CategoryCodeValue;
                end else
                    TempChildXMLBuffer.Next();
            end else begin
                CategoryCode := CopyStr(CategoryCodeValue, 1, MaxStrLen(CategoryCode));
