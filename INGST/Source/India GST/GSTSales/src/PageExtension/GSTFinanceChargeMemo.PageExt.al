    ...
    layout
    {
        addafter(Posting)
        {
            group("Tax Information")
            {
                ...
                field("Location Code"; Rec."Location Code")
                {
                    ...
                    trigger OnValidate()
                    var
                        GSTFinChargeMemoValidation: Codeunit "GST Fin Charge Memo Validation";
                    begin
                        GSTFinChargeMemoValidation.CallTaxEngineOnFinanceChargeMemoHeader(Rec);
                    end;
                }
        ...
        addfirst(factboxes)
        {
            part(TaxInformation; "Tax Information Factbox")
            {
                ...
                // Const Table -  Finance Charge Memo Line
                SubPageLink = "Table ID Filter" = const(303), "Document No. Filter" = field("Finance Charge Memo No."), "Line No. Filter" = field("Line No.");
                ApplicationArea = Basic, Suite;
            }
        }
    }
