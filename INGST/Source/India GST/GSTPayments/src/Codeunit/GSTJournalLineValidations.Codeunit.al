codeunit 18244 "GST Journal Line Validations"
{
    ...
    begin
        ...
        if GenJournalLine3."POS Out Of India" then begin
            ...
            if GenJournalLine3."Account Type" = GenJournalLine3."Account Type"::Customer then begin
                if not (GenJournalLine3."GST Place of Supply" in ["GST Place of Supply"::" ", "GST Place of Supply"::"Location Address"]) then
                    error(GSTPlaceOfSuppErr);

                if GenJournalLine3."GST Place of Supply" = "GST Place of Supply"::" " then
                ...
                if GenJournalLine3."GST Place of Supply" <> "GST Place of Supply"::"Location Address" then
                    VerifyPOSOutOfIndia(
                      ConfigType::Customer, GenJournalLine3."Location State Code", GetPlaceOfSupply(GenJournalLine3), GSTVendorType::" ", GenJournalLine3."GST Customer Type");
            end;
    ...
    begin
        ...
        case GenJournalLine."Account Type" OF
            GenJournalLine."Account Type"::Customer:
                begin
                    ...
                    if GenJournalLine."GST Place of Supply" = "GST Place of Supply"::" " then
            ...
            GenJournalLine."Account Type"::Vendor:
                begin
                    ...
                    if CheckRCMExemptDate(GenJournalLine) then begin
                        ...
                        error(
                            RCMExcemptErr, PurchasesPayablesSetup."RCM Exempt Start Date (Unreg)",
                            PurchasesPayablesSetup."RCM Exempt end Date (Unreg)");
                    end;
        ...
        if (GenJournalLine."GST Place of Supply" = "GST Place of Supply"::"Ship-to Address") and (GenJournalLine."Ship-to Code" = '') then
    ...
    begin
        ...
        if GenJournalLine."GST Place of Supply" <> "GST Place of Supply"::"Ship-to Address" then begin
            GenJournalLine."Ship-to Code" := '';
            GenJournalLine."Ship-to GST Reg. No." := '';
        end;
        if XGenJournalLine."GST Place of Supply" <> "GST Place of Supply"::" " then
    ...
    var
        ...
        GenJournalLine2: Record "Gen. Journal Line";
        PostingNoSeries: Record "Posting No. Series";
        TransactionType1: Enum "GST Buffer Transaction Type";
        Record: variant;
        GstPlaceOfSupply: Enum "GST Place Of Supply";
        GSTPOS: Text;
    begin
        GenJournalLine."GST TDS" := false;
        GenJournalLine."GST TCS" := false;
        ...
        then
            GenJournalLine."GST Reverse Charge" :=
              GenJournalLine."GST Vendor Type" in ["GST Vendor Type"::Import];
        ...
        if GenJournalLine."GST Group Code" <> '' then begin
            ...
            if GSTGroup.GET(GenJournalLine."GST Group Code") then begin
                ...
                if (GenJournalLine."GST Vendor Type" in ["GST Vendor Type"::Registered, "GST Vendor Type"::Unregistered, "GST Vendor Type"::SEZ]) and
                   (GenJournalLine."Document Type" in [GenJournalLine."Document Type"::Payment,
                                        GenJournalLine."Document Type"::Refund])
                then
                    GenJournalLine."GST Reverse Charge" := GSTGroup."Reverse Charge"
                else
                    if GenJournalLine."Document Type" in ["Document Type Enum"::Invoice,
            ...
            GetJournalHeader(GenJournalLine2, GenJournalLine, TransactionType1::Purchase);
            if (GenJournalLine2."GST Vendor Type" in [GenJournalLine2."GST Vendor Type"::Unregistered,
               ...
               GenJournalLine."GST Reverse Charge"
            then begin
                //GET PSOting No. Series 
                Record := GenJournalLine;
                GenJournalLine."Transaction Type" := GenJournalLine."Transaction Type"::purchase;
                PostingNoSeries.GetPostingNoSeriesCode(Record);
            end;
        end else begin
            GenJournalLine."GST Place of Supply" := "GST Place of Supply"::" ";
    ...
    procedure AfterInsert(var GenJournalLine: Record "Gen. Journal Line")
    var
        postingnoseries: Record "Posting No. Series";
        Record: Variant;
    begin
        //GET No. series from the Posting No. series
        Record := GenJournalLine;
        GenJournalLine."Transaction Type" := GenJournalLine."Transaction Type"::Sales;
        postingnoseries.GetPostingNoSeriesCode(Record);
    ...
    local procedure UpdateGSTInfoFromAcc(AccountNo: Code[20]; ClearVars: Boolean; var GenJnlLine: Record "Gen. Journal Line")
    var
    ...
    local procedure GetJournalHeader(VAR GenJournalLine: Record "Gen. Journal Line"; GenJournalLine1: Record "Gen. Journal Line"; TransactionType: Enum "GST Buffer Transaction Type")
    var
        DocNo: Code[20];
    begin
        ...
        if TransactionType = TransactionType::Sale then
    ...
    begin
        ...
        if GenJnlLine."GST on Advance Payment" and
           (GenJnlLine."GST Place of Supply" = "GST Place of Supply"::"Ship-to Address") and (GenJnlLine."Ship-to Code" = '')
        then
            error(BlankShiptoCodeErr);
    end;
    ...
    local procedure VerifyPOSOutOfIndia(ConfigType: Enum "Party Type"; LocationStateCode: Code[10];
    VendCustStateCode: Code[10]; GSTVendorType: Enum "GST Vendor Type";
