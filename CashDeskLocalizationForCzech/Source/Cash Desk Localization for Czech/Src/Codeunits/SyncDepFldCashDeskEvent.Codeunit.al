...
codeunit 31131 "Sync.Dep.Fld-CashDeskEvent CZP"
{
    ...
    begin
        if Rec.IsTemporary() then
            exit;
        if SyncLoopingHelper.IsFieldSynchronizationSkipped(Database::"Cash Desk Event", 0) then
            exit;
        SyncLoopingHelper.SkipFieldSynchronization(SyncLoopingHelper, Database::"Cash Desk Event CZP", 0);
        if CashDeskEventCZP.Get(xRec.Code) then
            CashDeskEventCZP.Rename(Rec.Code);
        SyncLoopingHelper.RestoreFieldSynchronization(Database::"Cash Desk Event CZP", 0);
    end;
    ...
    begin
        if Rec.IsTemporary() then
            exit;
        if SyncLoopingHelper.IsFieldSynchronizationSkipped(Database::"Cash Desk Event CZP", 0) then
            exit;
        SyncLoopingHelper.SkipFieldSynchronization(SyncLoopingHelper, Database::"Cash Desk Event", 0);
        if CashDeskEvent.Get(xRec.Code) then
            CashDeskEvent.Rename(Rec.Code);
        SyncLoopingHelper.RestoreFieldSynchronization(Database::"Cash Desk Event", 0);
    end;
