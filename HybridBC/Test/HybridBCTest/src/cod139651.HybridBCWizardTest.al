codeunit 139651 "HybridBC Wizard Tests"
{
    ...
    begin
        ...
        with wizard do begin
            ...
            VerifySaasSqlConfigurationWindow(wizard, 1);
            while optionCount <= "Sql Server Type".OptionCount() do begin
                currentOption := "Sql Server Type".GetOption(optionCount);

                case OptionCount of
                    1:
                        Assert.AreEqual(sqlServerTxt, currentOption, StrSubstNo('SqlType should be %1', sqlServerTxt));
                    2:
                        Assert.AreEqual(sqlAzureTxt, currentOption, StrSubstNo('SqlType should be %1', sqlAzureTxt));
                    else
                        Assert.Fail(StrSubstNo('SqlType ''%1'' was not expected', currentOption));
                end;
    ...
    begin
        ...
        with wizard do begin
            ...
            VerifyCompanySelectionWindow(wizard, 1);
            ...
            Assert.IsTrue(AssistedSetup.IsComplete('58623bfa-0559-4bc2-ae1c-0979c29fd9e0', Page::"Hybrid Cloud Setup Wizard"), 'Wizard status should be completed.');
        end;
    end;
    ...
    begin
        ...
        with wizard do begin
            ...
            // [THEN] ADF IR Runtime window is displayed.
            VerifySaasIRInstructionsWindow(wizard, 1);
    ...
    local procedure VerifySaasSqlConfigurationWindow(wizard: TestPage "Hybrid Cloud Setup Wizard"; executeNumber: Integer)
    begin
        with wizard do begin
            Assert.IsTrue(ActionBack.Enabled(), StrSubstNo('SQL Type window ActionBack should be enabled. Run %1', executeNumber));
            Assert.IsTrue(ActionNext.Enabled(), StrSubstNo('SQL Type window ActionNext should be enabled. Run %1', executeNumber));
            Assert.IsFalse(ActionFinish.Enabled(), StrSubstNo('SQL Type window ActionFinish should be disabled. Run %1', executeNumber));
        end;
    end;

    local procedure VerifySaasIRInstructionsWindow(wizard: TestPage "Hybrid Cloud Setup Wizard"; executeNumber: Integer)
    begin
        with wizard do begin
            Assert.IsTrue(ActionBack.Enabled(), StrSubstNo('ADF Info window ActionBack should be enabled. Run %1', executeNumber));
            Assert.IsTrue(ActionNext.Enabled(), StrSubstNo('ADF Info window ActionNext should be enabled. Run %1', executeNumber));
            Assert.IsFalse(ActionFinish.Enabled(), StrSubstNo('ADF Info window ActionFinish should be disabled. Run %1', executeNumber));
            Assert.IsTrue(DownLoadShir.Visible(), StrSubstNo('ADF Instructions window Download Self Hosted Integration Runtime should be visible. Run %1', executeNumber));
        end;
    end;

    local procedure VerifyCompanySelectionWindow(wizard: TestPage "Hybrid Cloud Setup Wizard"; executeNumber: Integer)
    begin
        with wizard do begin
            Assert.IsTrue(ActionBack.Enabled(), StrSubstNo('ADF Connection window ActionBack should be enabled. Run %1', executeNumber));
            Assert.IsTrue(ActionNext.Enabled(), StrSubstNo('ADF Connection window ActionNext should be enabled. Run %1', executeNumber));
            Assert.IsFalse(ActionFinish.Enabled(), StrSubstNo('ADF Connection window ActionFinish should be disabled. Run %1', executeNumber));
        end;
    end;
