...
report 4810 "Intrastat Report Get Lines"
{
    ...
    local procedure CalculateAverageCost(var AvgCost: Decimal; var AvgCostACY: Decimal): Boolean
    var
    ...
    begin
        ...
        AvgCost := ValueEntry2."Cost Amount (Actual)" + ValueEntry2."Cost Amount (Expected)";
        AvgCostACY := ValueEntry2."Cost Amount (Actual) (ACY)" + ValueEntry2."Cost Amount (Expected) (ACY)";

        if AverageQty <> 0 then begin
            AvgCost := AvgCost / AverageQty;
            AvgCostACY := AvgCostACY / AverageQty;
            if (AvgCost < 0) or (AvgCostACY < 0) then begin
                AvgCost := 0;
                AvgCostACY := 0;
            end;
        end else begin
    ...
    local procedure SetCountryRegionCode(var IntrastatReportLine3: Record "Intrastat Report Line"; ItemLedgerEntry: Record "Item Ledger Entry")
    var
        Location: Record Location;
    begin
        if (IntrastatReportLine3."Country/Region Code" = '') or
           (IntrastatReportLine3."Country/Region Code" = CompanyInfo."Country/Region Code")
        then
            if ItemLedgerEntry."Location Code" = '' then
                IntrastatReportLine3."Country/Region Code" := CompanyInfo."Ship-to Country/Region Code"
            else begin
                Location.Get(ItemLedgerEntry."Location Code");
                IntrastatReportLine3."Country/Region Code" := Location."Country/Region Code"
            end;
    end;

#if not CLEAN24
    local procedure SetJnlLineType(var IntrastatReportLine3: Record "Intrastat Report Line"; ValueEntryDocumentType: Enum "Item Ledger Document Type")
    begin
        if IntrastatReportLine3.Quantity < 0 then begin
            if ValueEntryDocumentType = "Value Entry"."Document Type"::"Sales Credit Memo" then
                IntrastatReportLine3.Type := IntrastatReportLine3.Type::Receipt
            else
