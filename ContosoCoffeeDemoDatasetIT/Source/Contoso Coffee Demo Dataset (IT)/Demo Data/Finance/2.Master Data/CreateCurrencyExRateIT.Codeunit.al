codeunit 12212 "Create Currency Ex. Rate IT"
{
    SingleInstance = true;
    ...
    begin
        ...
        ContosoCurrency.InsertCurrencyExchangeRate(CreateCurrency.GBP(), date, 100, 100, 165.793, 165.793);
        date := DMY2Date(2, 4, 2013);
        ContosoCurrency.InsertCurrencyExchangeRate(CreateCurrency.GBP(), date, 100, 100, 159.2372, 159.2372);
        date := CalcDate('<CY - 3Y + 1D>', WorkDate());
        ContosoCurrency.InsertCurrencyExchangeRate(CreateCurrency.GBP(), date, 100, 100, 154.8801, 154.8801);
    end;
    ...
    begin
        ...
        if Rec."Currency Code" = Currency.SEK() then
            if Rec."Starting Date" = CurrentDate then
                ValidateCurrencyExchRate(Rec, 100, 100, 15.7473, 15.7473);
        if Rec."Currency Code" = Currency.USD() then
            if Rec."Starting Date" = CurrentDate then
                ValidateCurrencyExchRate(Rec, 100, 100, 101.7599, 101.7599);

        CurrentDate := DMY2Date(2, 4, 2013);
        if Rec."Currency Code" = Currency.SEK() then
            if Rec."Starting Date" = CurrentDate then
                ValidateCurrencyExchRate(Rec, 100, 100, 16.2267, 16.2267);
        if Rec."Currency Code" = Currency.USD() then
            if Rec."Starting Date" = CurrentDate then
                ValidateCurrencyExchRate(Rec, 100, 100, 105.0287, 105.0287);
    end;
