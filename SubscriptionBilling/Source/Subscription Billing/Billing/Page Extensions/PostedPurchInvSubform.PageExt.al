    ...
    actions
    {
        addlast("&Line")
        {
            action(ShowArchivedBillingLines)
            {
                ...
                trigger OnAction()
                begin
                    ContractsGeneralMgt.ShowArchivedBillingLines(Rec."Subscription Contract No.", Rec."Subscription Contract Line No.", Enum::"Service Partner"::Vendor, Enum::"Rec. Billing Document Type"::Invoice, Rec."Document No.");
                end;
            }
            action("Usage Data")
            {
