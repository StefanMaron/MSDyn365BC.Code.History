...
codeunit 131300 "Library - ERM"
{
    ...
    begin
        ...
        Currency.Validate("Purch. PD Gains Acc. (TA)", CreateGLAccountNo());
        Currency.Validate("Purch. PD Losses Acc. (TA)", CreateGLAccountNo());
        Currency.Validate("Sales PD Gains Acc. (TA)", CreateGLAccountNo());
        Currency.Validate("Sales PD Losses Acc. (TA)", CreateGLAccountNo());
        Currency.Validate("PD Bal. Gain/Loss Acc. (TA)", CreateGLAccountNo());
        Currency.Modify(true);
        exit(Currency.Code);
    end;
    ...
    begin
        ...
        SetAppliestoIdCustomer(AppliesToCustLedgEntry);
        Commit();
        ApplyUnapplyParameters."Document No." := ApplyingCustLedgEntry."Document No.";
        ApplyUnapplyParameters."Posting Date" := 0D;
        CustEntryApplyPostedEntries.PreviewApply(ApplyingCustLedgEntry, ApplyUnapplyParameters);
    end;
    ...
    begin
        ...
        SetAppliestoIdVendor(AppliesToVendLedgEntry);
        Commit();
        ApplyUnapplyParameters."Document No." := ApplyingVendLedgEntry."Document No.";
        ApplyUnapplyParameters."Posting Date" := 0D;
        VendEntryApplyPostedEntries.PreviewApply(ApplyingVendLedgEntry, ApplyUnapplyParameters);
    end;
