...
xmlport 31222 "Import Transaction Types CZ"
{
    Caption = 'Import Transaction Types';
    Direction = Import;
    Encoding = UTF8;
    ...
    schema
    {
        textelement(ciselnik)
        {
            ...
            textelement(data)
            {
                ...
                MinOccurs = Zero;
                XmlName = 'data';
                tableelement(TransactionType; "Transaction Type")
                {
                    MaxOccurs = Unbounded;
                    MinOccurs = Once;
                    XmlName = 'radek';
                    ...
                    var
                        ...
                        ValidToDate: Date;
                    begin
                        TransactionType.Code := CopyStr(trans, 1, MaxStrlen(TransactionType.Code));
                        if TransactionType.Code = '' then
                            currXMLport.Skip();
                        if TransactionType.Get(TransactionType.Code) then
                            currXMLport.Skip();
                        ...
                        TransactionType.Description := CopyStr(popis, 1, MaxStrlen(TransactionType.Description));
                    end;
                }
            }
        }
