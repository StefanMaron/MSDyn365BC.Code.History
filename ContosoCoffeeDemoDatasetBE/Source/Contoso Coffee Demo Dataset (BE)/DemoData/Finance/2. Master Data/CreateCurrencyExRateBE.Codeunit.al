...
codeunit 11367 "Create Currency Ex. Rate BE"
{
    SingleInstance = true;
    ...
    begin
        ...
        if Rec."Currency Code" = Currency.SEK() then
            if Rec."Starting Date" = date then
                ValidateCurrencyExchRate(Rec, 15.7473, 15.7473);
        if Rec."Currency Code" = Currency.USD() then
            if Rec."Starting Date" = date then
                ValidateCurrencyExchRate(Rec, 101.7599, 101.7599);

        date := DMY2Date(2, 4, 2013);
        if Rec."Currency Code" = Currency.SEK() then
            if Rec."Starting Date" = date then
                ValidateCurrencyExchRate(Rec, 16.2267, 16.2267);
        if Rec."Currency Code" = Currency.USD() then
            if Rec."Starting Date" = date then
                ValidateCurrencyExchRate(Rec, 105.0287, 105.0287);
    end;
