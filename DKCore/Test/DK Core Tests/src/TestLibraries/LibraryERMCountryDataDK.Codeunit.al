codeunit 131307 "Library - ERM Country Data DK"
{
    ...
    begin
        if not UnitofMeasure.Get(PCSTxt) then
            CreateUnitOfMeasure(PCSTxt);
        if not UnitofMeasure.Get(BOXTxt) then
            CreateUnitOfMeasure(BOXTxt);
    end;
