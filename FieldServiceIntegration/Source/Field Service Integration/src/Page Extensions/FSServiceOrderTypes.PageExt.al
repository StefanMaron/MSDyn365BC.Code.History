            ...
            group(ActionFS)
            {
                ...
                action(SynchronizeNowFS)
                {
                    ...
                    begin
                        ...
                        ServiceOrderType.Next();

                        if ServiceOrderType.Count = 1 then
                            CRMIntegrationManagement.UpdateOneNow(ServiceOrderType.RecordId)
                        else begin
                            ServiceHeaderRecordRef.GetTable(ServiceOrderType);
                ...
                group(CouplingFS)
                {
                    ...
                    action(FSMatchBasedCoupling)
                    {
                        ...
                        Image = CoupledItem;
                        ToolTip = 'Couple service order types to work order types in Dynamics 365 Field Service record based on matching criteria.';
                        ...
                        begin
                            CurrPage.SetSelectionFilter(ServiceOrderType);
                            RecRef.GetTable(ServiceOrderType);
