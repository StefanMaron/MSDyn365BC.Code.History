codeunit 135060 "Document Mailing Tests"
{
    ...
    [Test]
    [HandlerFunctions('SelectSendingOptionsOKModalPageHandler,SelectSendingOptionsStrMenuHandler')]
    procedure SendPostedServiceInvoiceInBackgroundConfirmDefaultProfileUsingDefaultSettingsAndPdfAndNotCombineEmails()
    var
        DocumentSendingProfile: Record "Document Sending Profile";
    begin
        // [SCENARIO] Send Posted Service Invoice when Select sending options = "ConfirmDefault" Document Sending Profile using default settings, E-Mail Attachment = "PDF" and Combine Email Documents = false.
        Initialize();
        LibraryWorkflow.SetUpEmailAccount();
        ...
        SendPostedServiceInvoice(true, 2, 4, 2, 2);
    ...
    [Test]
    [HandlerFunctions('SelectSendingOptionsOKModalPageHandler,SelectSendingOptionsStrMenuHandler,EmailEditorSendModalPageHandler')]
    procedure SendPostedServiceInvoiceInForegroundConfirmDefaultProfileUsingPromptForSettingsAndPdfAndNotCombineEmails()
    var
        DocumentSendingProfile: Record "Document Sending Profile";
    begin
        // [SCENARIO] Send Posted Service Invoice when Select sending options = "ConfirmDefault" Document Sending Profile using prompt for settings, E-Mail Attachment = "PDF" and Combine Email Documents = false.
        Initialize();
        LibraryWorkflow.SetUpEmailAccount();
        ...
        SendPostedServiceInvoice(false, 2, 4, 2, 2);
        Assert.AreEqual(1, ModalCount, 'SelectSendingOptions modal did not open the correct number of times');
    end;

    [Test]
    [HandlerFunctions('SelectSendingOptionsOKModalPageHandler,SelectSendingOptionsStrMenuHandler')]
    procedure SendPostedServiceInvoiceInBackgroundConfirmDefaultProfileUsingDefaultSettingsAndPdfAndCombineEmails()
    var
        DocumentSendingProfile: Record "Document Sending Profile";
    begin
        // [SCENARIO] Send Posted Service Invoice when Select sending options = "ConfirmDefault" Document Sending Profile using default settings, E-Mail Attachment = "PDF" and Combine Email Documents = true.
        Initialize();
        LibraryWorkflow.SetUpEmailAccount();
        ...
        SendPostedServiceInvoice(true, 3, 2, 2, 1);
        Assert.AreEqual(1, ModalCount, 'SelectSendingOptions modal did not open the correct number of times');
    end;

    [Test]
    [HandlerFunctions('SelectSendingOptionsOKModalPageHandler,SelectSendingOptionsStrMenuHandler,EmailEditorSendModalPageHandler')]
    procedure SendPostedServiceInvoiceInForegroundConfirmDefaultProfileUsingPromptForSettingsAndPdfAndCombineEmails()
    var
        DocumentSendingProfile: Record "Document Sending Profile";
    begin
        // [SCENARIO] Send Posted Service Invoice when Select sending options = "ConfirmDefault" Document Sending Profile using prompt for settings, E-Mail Attachment = "PDF" and Combine Email Documents = true.
        Initialize();
        LibraryWorkflow.SetUpEmailAccount();
        ...
        SendPostedServiceInvoice(false, 3, 2, 2, 1);
        Assert.AreEqual(1, ModalCount, 'SelectSendingOptions modal did not open the correct number of times');
    end;

    [Test]
    [HandlerFunctions('SelectSendingOptionsStrMenuHandler')]
    procedure SendPostedServiceInvoiceInBackgroundUseDefaultProfileUsingDefaultSettingsAndPdfAndNotCombineEmails()
    var
        DocumentSendingProfile: Record "Document Sending Profile";
    begin
        // [SCENARIO] Send Posted Service Invoice when Select sending options = "UseDefaultProfile" Document Sending Profile using default settings, E-Mail Attachment = "PDF" and Combine Email Documents = false.
        Initialize();
        LibraryWorkflow.SetUpEmailAccount();
    ...
    [Test]
    [HandlerFunctions('SelectSendingOptionsStrMenuHandler,EmailEditorSendModalPageHandler')]
    procedure SendPostedServiceInvoiceInForegroundUseDefaultProfileUsingPromptForSettingsAndPdfAndNotCombineEmails()
    var
        DocumentSendingProfile: Record "Document Sending Profile";
    begin
        // [SCENARIO] Send Posted Service Invoice when Select sending options = "UseDefaultProfile" Document Sending Profile using prompt for settings, E-Mail Attachment = "PDF" and Combine Email Documents = false.
        Initialize();
        LibraryWorkflow.SetUpEmailAccount();
    ...
    [Test]
    [HandlerFunctions('SelectSendingOptionsStrMenuHandler')]
    procedure SendPostedServiceInvoiceInBackgroundUseDefaultProfileUsingDefaultSettingsAndPdfAndCombineEmails()
    var
        DocumentSendingProfile: Record "Document Sending Profile";
    begin
        // [SCENARIO] Send Posted Service Invoice when Select sending options = "UseDefaultProfile" Document Sending Profile using default settings, E-Mail Attachment = "PDF" and Combine Email Documents = true.
        Initialize();
        LibraryWorkflow.SetUpEmailAccount();
    ...
    [Test]
    [HandlerFunctions('SelectSendingOptionsStrMenuHandler,EmailEditorSendModalPageHandler')]
    procedure SendPostedServiceInvoiceInForegroundUseDefaultProfileUsingPromptForSettingsAndPdfAndCombineEmails()
    var
        DocumentSendingProfile: Record "Document Sending Profile";
    begin
        // [SCENARIO] Send Posted Service Invoice when Select sending options = "UseDefaultProfile" Document Sending Profile using prompt for settings, E-Mail Attachment = "PDF" and Combine Email Documents = true.
        Initialize();
        LibraryWorkflow.SetUpEmailAccount();
    ...
    local procedure SendPostedServiceInvoice(RunJQ: Boolean; InvoicesCount: Integer; JQCount: Integer; CustomersCount: Integer; EmailCount: Integer)
    var
        Customer: Record Customer;
        JobQueueEntry: Record "Job Queue Entry";
    ...
    begin
        ...
        // [GIVEN] Posted Service Invoices for above customers
        for Count := 1 to CustomersCount do begin
            CustomerNos.Get(Count, CustomerNo);
            Customer.Get(CustomerNo);
            CreateMultipleServiceDocumentForCustomer(Customer, Enum::"Service Document Type"::Invoice, InvoicesCount);
            StrBuilder.Append(CustomerNo + '|');
        end;
        ServiceInvoiceHeader.SetFilter("Bill-to Customer No.", StrBuilder.ToText().TrimEnd('|'));
        ServiceInvoiceHeader.FindSet();
        Assert.RecordCount(ServiceInvoiceHeader, InvoicesCount * CustomersCount);

        // [WHEN] Send Posted Service Invoices
