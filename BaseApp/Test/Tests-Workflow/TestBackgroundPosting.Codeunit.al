codeunit 139027 "Test Background Posting"
{
    ...
    var
        ...
        FailPostErr: Label 'Document %1 expected to fail posting.';
        DocRemainsInTableErr: Label 'Document %1 expected to remain in the table when failing to post.';
        JobQueueLogEntriesCntErr: Label 'The count of job queue log entries is not the expected one.';
        UnexpectedStatusErr: Label 'Header %1 expected to change Job Queue Status to %2 but was set to %3.';
    ...
    local procedure WaitForPurchaseHeaderJobQueueStatus(var PurchaseHeader: Record "Purchase Header"; ExpectedStatus: Option): Boolean
    var
        i: Integer;
    begin
        while (i < 1000) and PurchaseHeader.Get(PurchaseHeader."Document Type", PurchaseHeader."No.") do begin
            if PurchaseHeader."Job Queue Status" = ExpectedStatus then
                exit(true);
    ...
    local procedure WaitForSalesHeaderJobQueueStatus(var SalesHeader: Record "Sales Header"; ExpectedStatus: Option): Boolean
    var
        i: Integer;
    begin
        while (i < 1000) and SalesHeader.Get(SalesHeader."Document Type", SalesHeader."No.") do begin
            if SalesHeader."Job Queue Status" = ExpectedStatus then
                exit(true);
    ...
    local procedure GetSessionIdForHeader(HeaderRecId: RecordID): Integer
    var
        JobQueueEntry: Record "Job Queue Entry";
    begin
        // Returns the Session ID for the user session created when a Sales or Purchase document has been Scheduled for Posting.
        with JobQueueEntry do begin
            LockTable;
            SetFilter("Record ID to Process", Format(HeaderRecId));
            if not FindFirst then
                exit(0);
            exit("User Session ID");
        end;
    end;
    ...
    [Scope('OnPrem')]
    procedure PostPurchOrderTest()
    var
        PurchHeader: Record "Purchase Header";
    begin
        ValidatePostPurch(PurchHeader."Document Type"::Order);
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure PostPurchInvoiceTest()
    var
        PurchHeader: Record "Purchase Header";
    begin
        ValidatePostPurch(PurchHeader."Document Type"::Invoice);
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure PostSalesOrderTest()
    var
        SalesHeader: Record "Sales Header";
    begin
        ValidatePostSales(SalesHeader."Document Type"::Order);
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure Post2StepSalesOrderShipAndInvoiceTest()
    var
        SalesHeader: Record "Sales Header";
    begin
        ValidatePost2StepSalesOrderShipAndInvoiceTest(SalesHeader."Document Type"::Order, false);
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure Post2StepSalesReturnOrderShipAndInvoiceTest()
    var
        SalesHeader: Record "Sales Header";
    begin
        ValidatePost2StepSalesOrderShipAndInvoiceTest(SalesHeader."Document Type"::"Return Order", true);
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure Post2StepPurchOrderShipAndInvoiceTest()
    var
        PurchaseHeader: Record "Purchase Header";
    begin
        ValidatePost2StepPurchOrderAndInvoice(PurchaseHeader."Document Type"::Order, false);
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure Post2StepPurchReturnOrderShipAndInvoiceTest()
    var
        PurchaseHeader: Record "Purchase Header";
    begin
        ValidatePost2StepPurchOrderAndInvoice(PurchaseHeader."Document Type"::"Return Order", true);
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure PostSalesInvoiceTest()
    var
        SalesHeader: Record "Sales Header";
    begin
        ValidatePostSales(SalesHeader."Document Type"::Invoice);
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure PostSalesCreditMemoTest()
    var
        SalesHeader: Record "Sales Header";
    begin
        ValidatePostSales(SalesHeader."Document Type"::"Credit Memo");
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure PostPurchOrderErrorTest()
    var
        PurchHeader: Record "Purchase Header";
        JobQueueLogEntry: Record "Job Queue Log Entry";
    ...
    begin
        ...
        WaitForPurchaseHeaderJobQueueStatus(PurchHeader, PurchHeader."Job Queue Status"::Error);

        BackgroundSessionsTestLib.CleanupAll;

        // Does PurchHeader."Document Type" and PurchHeader."No." work if the record was removed?
        Assert.IsTrue(PurchHeader.Get(PurchHeader."Document Type", PurchHeader."No."), StrSubstNo(DocRemainsInTableErr, PurchHeader."No."));
        Assert.AreEqual(PurchHeader."Job Queue Status"::Error, PurchHeader."Job Queue Status", StrSubstNo(FailPostErr, PurchHeader."No."));

        Assert.AreEqual(
          1,
          WaitForNumberOfJobQueueLogEntries(1, JobQueueLogEntry.Status::Error),
    ...
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure PostSalesOrderErrorTest()
    var
        SalesHeader: Record "Sales Header";
        JobQueueLogEntry: Record "Job Queue Log Entry";
    ...
    begin
        ...
        WaitForSalesHeaderJobQueueStatus(SalesHeader, SalesHeader."Job Queue Status"::Error);

        BackgroundSessionsTestLib.CleanupAll;

        // Does SalesHeader."Document Type" and SalesHeader."No." work if the record was removed?
        Assert.IsTrue(SalesHeader.Get(SalesHeader."Document Type", SalesHeader."No."), StrSubstNo(DocRemainsInTableErr, SalesHeader."No."));
        Assert.AreEqual(SalesHeader."Job Queue Status"::Error, SalesHeader."Job Queue Status", StrSubstNo(FailPostErr, SalesHeader."No."));
    ...
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure PostSalesReturnOrderTest()
    var
        SalesHeader: Record "Sales Header";
        JobQueueLogEntry: Record "Job Queue Log Entry";
        BackgroundSessionsTestLib: Codeunit "Background Sessions Test Lib";
    ...
    begin
        ...
        Assert.AreEqual(
          1,
          WaitForNumberOfJobQueueLogEntries(1, JobQueueLogEntry.Status::Success),
          JobQueueLogEntriesCntErr);
    end;
    ...
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure PostPurchReturnOrderTest()
    var
        PurchHeader: Record "Purchase Header";
    begin
        ValidatePostPurch(PurchHeader."Document Type"::"Return Order");
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure PostPurchCreditMemoTest()
    var
        PurchHeader: Record "Purchase Header";
    begin
        ValidatePostPurch(PurchHeader."Document Type"::"Credit Memo");
    end;
    ...
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure CancelPurchOrderTest()
    var
        PurchHeader: Record "Purchase Header";
        PurchPostViaJobQueue: Codeunit "Purchase Post via Job Queue";
        BackgroundSessionsTestLib: Codeunit "Background Sessions Test Lib";
    ...
    begin
        ...
        CreatePurchDocument(PurchHeader, PurchHeader."Document Type"::Order);
        PurchPostViaJobQueue.EnqueuePurchDoc(PurchHeader); // 1 Background Session created
        Commit;

        WaitForPurchaseHeaderJobQueueStatus(PurchHeader, PurchHeader."Job Queue Status"::"Scheduled for Posting");
        RecRef.GetTable(PurchHeader);
        SessionId := GetSessionIdForHeader(RecRef.RecordId);
        if PurchHeader."Job Queue Status" <> PurchHeader."Job Queue Status"::"Scheduled for Posting" then begin
            BackgroundSessionsTestLib.Abort(SessionId);
            Assert.Fail(
              StrSubstNo(
                UnexpectedStatusErr,
                PurchHeader."No.",
                PurchHeader."Job Queue Status"::"Scheduled for Posting",
                PurchHeader."Job Queue Status"));
        end;

        PurchPostViaJobQueue.CancelQueueEntry(PurchHeader);
        BackgroundSessionsTestLib.WaitEnd(SessionId, true);
        WaitForPurchaseHeaderJobQueueStatus(PurchHeader, PurchHeader."Job Queue Status"::" ");
        Assert.AreEqual(
          PurchHeader."Job Queue Status"::" ",
          PurchHeader."Job Queue Status",
          StrSubstNo(UnexpectedStatusErr, PurchHeader."No.", 'empty (no status)', PurchHeader."Job Queue Status"));
    end;

    [Test]
    [HandlerFunctions('MessageHandler')]
    [Scope('OnPrem')]
    procedure CancelSalesOrderTest()
    var
        SalesHeader: Record "Sales Header";
        SalesPostViaJobQueue: Codeunit "Sales Post via Job Queue";
        BackgroundSessionsTestLib: Codeunit "Background Sessions Test Lib";
    ...
    begin
        ...
        CreateSalesDocument(SalesHeader, SalesHeader."Document Type"::Order);
        SalesPostViaJobQueue.EnqueueSalesDoc(SalesHeader); // 1 Background Session created
        Commit;

        WaitForSalesHeaderJobQueueStatus(SalesHeader, SalesHeader."Job Queue Status"::"Scheduled for Posting");
        RecRef.GetTable(SalesHeader);
        SessionId := GetSessionIdForHeader(RecRef.RecordId);
        if SalesHeader."Job Queue Status" <> SalesHeader."Job Queue Status"::"Scheduled for Posting" then begin
            BackgroundSessionsTestLib.Abort(SessionId);
            Assert.Fail(
              StrSubstNo(
                UnexpectedStatusErr,
                SalesHeader."No.",
                SalesHeader."Job Queue Status"::"Scheduled for Posting",
                SalesHeader."Job Queue Status"));
        end;

        SalesPostViaJobQueue.CancelQueueEntry(SalesHeader);
        BackgroundSessionsTestLib.WaitEnd(SessionId, true);
        WaitForSalesHeaderJobQueueStatus(SalesHeader, SalesHeader."Job Queue Status"::" ");
        Assert.AreEqual(
          SalesHeader."Job Queue Status"::" ",
          SalesHeader."Job Queue Status",
          StrSubstNo(UnexpectedStatusErr, SalesHeader."No.", 'empty (no status)', SalesHeader."Job Queue Status"));
    end;
    ...
    [Normal]
    local procedure ValidatePostPurch(DocumentType: Integer)
    var
        PurchHeader: Record "Purchase Header";
        JobQueueLogEntry: Record "Job Queue Log Entry";
    ...
    begin
        ...
        case DocumentType of
            PurchHeader."Document Type"::"Credit Memo":
                begin
                    // Find yet unused credit memo number
                    CrMemoNo := 122;
                    repeat
                        CrMemoNo := CrMemoNo + 1;
                        PurchCrMemoHdr.SetRange("Vendor Cr. Memo No.", Format(CrMemoNo));
                    until PurchCrMemoHdr.Count = 0;
                    PurchHeader."Vendor Cr. Memo No." := Format(CrMemoNo);
                    PurchHeader.Modify;
                end;
    ...
    [Normal]
    local procedure ValidatePost2StepPurchOrderAndInvoice(DocumentType: Integer; ShipPurchaseHeader: Boolean)
    var
        PurchaseHeader: Record "Purchase Header";
        JobQueueLogEntry: Record "Job Queue Log Entry";
        BackgroundSessionsTestLib: Codeunit "Background Sessions Test Lib";
    ...
    begin
        ...
        if ShipPurchaseHeader = true then
            PurchaseHeader.Ship := true;

        PurchaseHeader.Modify(true);

        PurchPostViaJobQueue.EnqueuePurchDoc(PurchaseHeader); // background session created
        Commit;

        WaitForPurchaseHeaderJobQueueStatus(PurchaseHeader, PurchaseHeader."Job Queue Status"::" ");
        ...
        WaitForPurchaseHeaderRemoved(PurchaseHeader);
        NoOfJQLogEntries := WaitForNumberOfJobQueueLogEntries(2, JobQueueLogEntry.Status::Success);

        BackgroundSessionsTestLib.CleanupAll;

        Assert.AreEqual(2, NoOfJQLogEntries, JobQueueLogEntriesCntErr);

        Assert.IsFalse(
          PurchaseHeader.Get(PurchaseHeader."Document Type", PurchaseHeader."No."),
          StrSubstNo(PostTimeoutErr, PurchaseHeader."No.", PurchaseHeader."Job Queue Status"));
    end;

    [Normal]
    local procedure ValidatePost2StepSalesOrderShipAndInvoiceTest(DocumentType: Integer; ReceiveSalesHeader: Boolean)
    var
        SalesHeader: Record "Sales Header";
        JobQueueLogEntry: Record "Job Queue Log Entry";
        BackgroundSessionsTestLib: Codeunit "Background Sessions Test Lib";
