codeunit 134993 "Reminder - Line Fee on Reports"
{
    ...
    [Scope('OnPrem')]
    procedure PrintSalesInvoiceWithAddFeePerLine()
    var
        CustomerNo: Code[20];
    ...
    begin
        // [SCENARIO 107048] A sales invoice report contains Add. Fee per Line note with the amount picked up from the Reminder Terms
        // as the selected reminder terms has a Add. Fee per Line > 0
        Initialize;

        // [GIVEN] A Reminder Term X, with level 1 with Add. Fee per Line = A, where A > 0
        ...
        InvoiceNo := PostSalesInvoice(CustomerNo, WorkDate);

        // [WHEN] The invoice is printed
        ExportSalesInvoice(CustomerNo);

        // [THEN] The Add. Fee per Line note on report with amount A is printed on the invoice
    ...
    [HandlerFunctions('RHSalesInvoice')]
    [Scope('OnPrem')]
    procedure PrintSalesInvoiceWithMultipleAddFeePerLine()
    var
        CustomerNo: Code[20];
        InvoiceNo: Code[20];
        ReminderTermsCode: Code[10];
    ...
    begin
        // [SCENARIO 107048] A sales invoice report contains multiple Add. Fee per line notes as the selected reminder terms
        // has two reminder levels with Add. Fee per Line defined
        Initialize;

        // [GIVEN] A Reminder Term X with level 1 having Add. Fee per Line = A, where A > 0
        AddFeePerLine1 := LibraryRandom.RandDec(100, 2);
        CreateCustomerWithReminderTermsAddFeePerLine(CustomerNo, ReminderTermsCode, true, '', AddFeePerLine1);
        ...
        // [THEN] The two Add. Fee per Line Notes are printed
    ...
    [HandlerFunctions('RHSalesInvoice')]
    [Scope('OnPrem')]
    procedure PrintSalesInvoiceWithoutAddFeePerLine()
    var
        CustomerNo: Code[20];
        InvoiceNo: Code[20];
        ReminderTermsCode: Code[10];
    begin
        // [SCENARIO 107048] A sales invoice does not contain Add. Fee per Line note as the selected reminder terms has a Add. Fee per Line = 0
        Initialize;

        // [GIVEN] A Reminder Term X with level 1 having Add. Fee per Line = A, where A = 0
        CreateCustomerWithReminderTermsAddFeePerLine(CustomerNo, ReminderTermsCode, true, '', 0); // WithLump = TRUE
    ...
    [HandlerFunctions('RHSalesInvoice')]
    [Scope('OnPrem')]
    procedure PrintSalesInvoiceWithMarginalPerc()
    var
        CustomerNo: Code[20];
        ReminderTermsCode: Code[10];
        InvoiceNo: Code[20];
        AddFeePerLine: Decimal;
    begin
        // [SCENARIO 107048] A sales invoice report contains Add. Fee per Line note with Marginal Percentage shown
    ...
    [HandlerFunctions('RHSalesInvoice')]
    [Scope('OnPrem')]
    procedure PrintSalesInvoiceTextonInvoiceTranslated()
    var
        ...
        ReminderTermsCode: Code[10];
    begin
        // [SCENARIO 107048] A sales invoice report contains translated Add. Fee Note
        // as selected Reminder Terms contain Transalted text on Report and Customer Country is set to use specific lang.
        Initialize;
    ...
    [RequestPageHandler]
    [Scope('OnPrem')]
    procedure RHSalesInvoice(var SalesInvoice: TestRequestPage "Sales - Invoice")
    begin
        SalesInvoice.DisplayAdditionalFeeNote.SetValue(true);
