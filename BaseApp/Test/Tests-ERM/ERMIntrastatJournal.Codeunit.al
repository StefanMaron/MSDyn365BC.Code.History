...
codeunit 134150 "ERM Intrastat Journal"
{
    ...
    [Test]
    procedure IntrastatExport2022_Receipt()
    var
        IntrastatJnlLine: Record "Intrastat Jnl. Line";
        FileTempBlob: Codeunit "Temp Blob";
    begin
        // [FEATURE] [Intrastat] [Export]
        // [SCENARIO 402338] Intrastat journal basic file export in format of 2022 (receipt)
        Initialize();
        IntrastatJnlLine.DeleteAll();
        ...
        // [THEN] Total Weight value is rounded up to integer
        // [THEN] Tariff No value is exported w\o spaces (TFS 423720)
        // [THEN] Country Of Origin is exported (TFS 424312)
        VerifyIntrastatExportedFile2022(FileTempBlob, IntrastatJnlLine);
    end;

    [Test]
    procedure IntrastatExport2022_Shipment()
    var
        IntrastatJnlLine: Record "Intrastat Jnl. Line";
        FileTempBlob: Codeunit "Temp Blob";
    begin
        // [FEATURE] [Intrastat] [Export]
        // [SCENARIO 402338] Intrastat journal basic file export in format of 2022 (shipment)
        Initialize();
        IntrastatJnlLine.DeleteAll();
    ...
    begin
        ...
        FileInStream.ReadText(Line);
        FileInStream.ReadText(Line);
        Assert.AreEqual(Format(IntrastatJnlLine."Country/Region of Origin Code", 2), CopyStr(Line, 19, 2), '');
    end;
    ...
    begin
        ...
        Assert.AreEqual(7, LineCount, ''); // 4 data lines + 3 header/footer lines
    end;
    ...
    begin
        ...
        IntrastatJnlLine."Quantity 2" := LibraryRandom.RandDecInRange(1000, 2000, 2);
        IntrastatJnlLine."Statistical Value" := LibraryRandom.RandDecInRange(1000, 2000, 2);
        IntrastatJnlLine."Unit of Measure" := CopyStr(LibraryUtility.GenerateRandomAlphabeticText(3, 0), 1, 3);
        IntrastatJnlLine.Modify();
    end;
    ...
    begin
        ...
        IntrastatJnlLine.SetRange("Journal Template Name", IntrastatJnlLine."Journal Template Name");
        IntrastatJnlLine.SetRange("Journal Batch Name", IntrastatJnlLine."Journal Batch Name");
        IntrastatMakeDiskTaxAuth.InitializeRequest(FileOutStream, ExportFormat, IntrastatJnlLine.Type);
    ...
    begin
        ...
        FileInStream.ReadText(Line);
        Assert.AreEqual('KON0037', CopyStr(Line, 1, 7), '');

        FileInStream.ReadText(Line);
        Assert.AreEqual('OTS', CopyStr(Line, 1, 3), '');

        FileInStream.ReadText(Line);
        Assert.AreEqual('NIM00001', CopyStr(Line, 1, 8), '');
        Assert.AreEqual(Format(DelChr(IntrastatJnlLine."Tariff No."), 8), CopyStr(Line, 9, 8), '');
        Assert.AreEqual(Format(IntrastatJnlLine."Transaction Type", 2), CopyStr(Line, 17, 2), '');
        Assert.AreEqual(Format(IntrastatJnlLine."Country/Region of Origin Code", 2), CopyStr(Line, 19, 2), '');
        Assert.AreEqual(Format(IntrastatJnlLine."Transport Method", 1), CopyStr(Line, 25, 1), '');
        Assert.AreEqual(
          Format(Round(IntrastatJnlLine."Statistical Value", 1, '>'), 0, '<Integer,10><Filler Character,0>'), CopyStr(Line, 26, 10), 'Statistical Value');
        Assert.AreEqual(
          Format(Round(IntrastatJnlLine."Total Weight", 1, '>'), 0, '<Integer,10><Filler Character,0>'), CopyStr(Line, 57, 10), 'Total Weight');
        Assert.AreEqual(
          Format(Round(IntrastatJnlLine."Quantity 2", 1, '>'), 0, '<Integer,10><Filler Character,0>'), CopyStr(Line, 73, 10), 'Quantity 2');
        Assert.AreEqual(
          Format(Round(IntrastatJnlLine.Amount, 1, '>'), 0, '<Integer,10><Filler Character,0>'), CopyStr(Line, 83, 10), 'Amount');

        FileInStream.ReadText(Line);
        Assert.AreEqual('SUM', CopyStr(Line, 1, 3), '');
    end;
    ...
    begin
        ...
        Assert.AreEqual(Format(IntrastatJnlLine."Transport Method", 1), CopyStr(Line, 25, 1), '');
        Assert.AreEqual(
          Format(Round(IntrastatJnlLine."Statistical Value", 1, '>'), 0, '<Integer,10><Filler Character,0>'), CopyStr(Line, 26, 10), '');
        Assert.AreEqual(
          Format(Round(IntrastatJnlLine."Total Weight", 1, '>'), 0, '<Integer,10><Filler Character,0>'), CopyStr(Line, 57, 10), '');
        Assert.AreEqual(
          Format(Round(IntrastatJnlLine."Quantity 2", 1, '>'), 0, '<Integer,10><Filler Character,0>'), CopyStr(Line, 73, 10), '');
        Assert.AreEqual(
          Format(Round(IntrastatJnlLine.Amount, 1, '>'), 0, '<Integer,10><Filler Character,0>'), CopyStr(Line, 83, 10), '');
        Assert.AreEqual(Format(IntrastatJnlLine."Partner VAT ID", 14), CopyStr(Line, 93, 14), '');
