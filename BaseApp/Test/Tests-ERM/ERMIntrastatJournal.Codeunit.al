codeunit 134150 "ERM Intrastat Journal"
{
    ...
    begin
        ...
        // [GIVEN] Intrastat journal line
        PrepareIntrastatJnlLine(IntrastatJnlLine, IntrastatJnlLine.Type::Shipment);
    ...
    [Test]
    procedure IntrastatExport2022_SingleShipment()
    var
        IntrastatJnlLine: Record "Intrastat Jnl. Line";
        FileTempBlob: Codeunit "Temp Blob";
    begin
        // [FEATURE] [Intrastat] [Export] [Shipment]
        // [SCENARIO 390312] Single shipment Intrastat journal line basic file export in format of 2022
        Initialize();
        IntrastatJnlLine.DeleteAll();

        // [GIVEN] Shipment Intrastat journal line
        PrepareIntrastatJnlLine(IntrastatJnlLine, IntrastatJnlLine.Type::Shipment);
        ...
        // [THEN] Tariff No value is exported w\o spaces (TFS 423720)
        VerifyIntrastatExportedFile2022_Shipment(FileTempBlob, IntrastatJnlLine);
    end;

    [Test]
    procedure IntrastatExport2022_SingleReceipt()
    var
        IntrastatJnlLine: Record "Intrastat Jnl. Line";
        FileTempBlob: Codeunit "Temp Blob";
    begin
        // [FEATURE] [Intrastat] [Export] [Receipt]
        // [SCENARIO 420025] Single receipt Intrastat journal line basic file export in format of 2022
        Initialize();
        IntrastatJnlLine.DeleteAll();

        // [GIVEN] Receipt Intrastat journal line
        PrepareIntrastatJnlLine(IntrastatJnlLine, IntrastatJnlLine.Type::Receipt);
        ...
        // [THEN] Basic fields are exported in format of 2022
        // [THEN] Tariff No value is exported w\o spaces (TFS 423720)
        VerifyIntrastatExportedFile2022_Receipt(FileTempBlob, IntrastatJnlLine);
    end;
    ...
    begin
        ...
        Assert.ExpectedErrorCode('Dialog');
        Assert.ExpectedError(StatPeriodFormatErr);

        asserterror IntrastatJnlBatch.Validate("Statistics Period", '2201');
        Assert.ExpectedErrorCode('Dialog');
        Assert.ExpectedError(StatPeriodMonthErr);

        IntrastatJnlBatch.Validate("Statistics Period", '0122'); // MMYY
    end;
    ...
    begin
        ...
        Values := Line.Split(',');
        Values.Get(9, Line);
        Assert.AreEqual(CountryRegion.Code, Line, '');
    end;
    ...
    local procedure PrepareIntrastatJnlLine(var IntrastatJnlLine: Record "Intrastat Jnl. Line"; Type: Option)
    begin
        CreateIntrastatJnlLine(IntrastatJnlLine);
    ...
    begin
        ...
        header :=
            ...
            ',,' + CopyStr(CompanyInformation.Name, 1, 30) + ',X,D';
        ...
        Values.Get(1, Line);
        Assert.AreEqual(DelChr(IntrastatJnlLine."Tariff No."), Line, '');
        Values.Get(2, Line);
    ...
    local procedure VerifyIntrastatExportedFile2022_Shipment(var FileTempBlob: Codeunit "Temp Blob"; IntrastatJnlLine: Record "Intrastat Jnl. Line")
    var
        CompanyInformation: Record "Company Information";
        CountryRegion: Record "Country/Region";
        Values: List of [Text];
    ...
    local procedure VerifyIntrastatExportedFile2022_Receipt(var FileTempBlob: Codeunit "Temp Blob"; IntrastatJnlLine: Record "Intrastat Jnl. Line")
    var
        CompanyInformation: Record "Company Information";
        CountryRegion: Record "Country/Region";
        Values: List of [Text];
