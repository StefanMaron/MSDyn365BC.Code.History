codeunit 143052 "Library - Certificate CZ"
{
    ...
    [Scope('OnPrem')]
    procedure CreateCertificateCode(var CertificateCZCode: Record "Certificate CZ Code")
    begin
    ...
    [Scope('OnPrem')]
    procedure CreateCertificate(var CertificateCZ: Record "Certificate CZ"; CertificateCode: Code[10]; UserCode: Code[50]; StoreType: Option)
    var
        CertificateCZCode: Record "Certificate CZ Code";
    begin
        if not CertificateCZCode.Get(CertificateCode) then
            CreateCertificateCode(CertificateCZCode);

        CertificateCode := CertificateCZCode.Code;
        with CertificateCZ do begin
            Init;
            Validate("Certificate Code", CertificateCode);
            Validate("User ID", UserCode);
            Validate("Store Type", StoreType);
            Insert(true);
        end;
    end;

    [Scope('OnPrem')]
    procedure CreateCertificateWithImportedCertificate(var CertificateCZ: Record "Certificate CZ"; CertificateCode: Code[10]; UserCode: Code[50]; WithPrivateKey: Boolean)
    begin
        CreateCertificate(CertificateCZ, CertificateCode, UserCode, CertificateCZ."Store Type"::Database);
        SaveCertificateObjectToDatabase(CertificateCZ, WithPrivateKey);
    end;

    [Scope('OnPrem')]
    procedure FormatStoreLocation(StoreLocation: Option): Text
    var
    ...
    [Scope('OnPrem')]
    procedure FormatStoreName(StoreName: Option): Text
    var
        DummyCertificateCZ: Record "Certificate CZ";
    begin
    ...
    [Scope('OnPrem')]
    procedure GetCertificateBlob(WithPrivateKey: Boolean; var TempBlob: Codeunit "Temp Blob")
    var
    ...
    begin
        ...
        CertificateCZMgt.SaveCertificateToBlob(X509Certificate2, WithPrivateKey, TempBlob);
    end;
    ...
    [Scope('OnPrem')]
    procedure GetCertificatePrivateKey(var "Key": DotNet AsymmetricAlgorithm)
    var
        X509Certificate2: DotNet X509Certificate2;
    begin
        GetCertificateObject(X509Certificate2);
        Key := X509Certificate2.PrivateKey;
    end;

    [Scope('OnPrem')]
    procedure GetCertificatePublicKey(var "Key": DotNet AsymmetricAlgorithm)
    var
        X509Certificate2: DotNet X509Certificate2;
    begin
        GetCertificateObject(X509Certificate2);
        Key := X509Certificate2.PublicKey.Key;
    end;

    [Scope('OnPrem')]
    procedure GetCertificateThumbprint(): Text
    var
        X509Certificate2: DotNet X509Certificate2;
    begin
        GetCertificateObject(X509Certificate2);
    ...
    [Scope('OnPrem')]
    procedure GetFakeThumbprint(): Text
    begin
        exit('ffffffffffffffffffffffffffffffffffffffff');
    end;

    [Scope('OnPrem')]
    procedure GetFakePassword(): Text
    begin
        exit('fakepassword');
    end;

    [Scope('OnPrem')]
    procedure SaveCertificateObjectToDatabase(var CertificateCZ: Record "Certificate CZ"; WithPrivateKey: Boolean)
    var
    ...
    begin
        if (CertificateCZ."Certificate Code" = '') and
           (CertificateCZ."User ID" = '') and
           (CertificateCZ."Valid From" = 0DT)
        then
            CreateCertificate(CertificateCZ, '', UserId, CertificateCZ."Store Type"::Database);

        GetCertificateBlob(true, TempBlob);
        TempBlob.CreateInStream(InputStream);

        TempCertificateCZ := CertificateCZ;
        if CertificateCZ.SaveCertificateFromStream(InputStream, '', WithPrivateKey) then begin
    ...
    local procedure CertificateBase64(): Text
    begin
        exit(
