...
codeunit 5601 "FA Insert G/L Account"
{
    ...
    begin
        ...
        FAGLPostBuf."Disposal Entry No." := Rec."Disposal Entry No.";
        ...
        Flag := false;
        if Rec."Disposal Entry No." in [-1, 1] then
            case Rec."FA Posting Category" of
...
#pragma warning restore AA0074
                ...
                if NewAmount <> 0 then begin
                    ...
                    if Flag then begin
                        ...
                        if FAPostingGr."Sales Bal. Acc." <> '' then begin
                            ...
                            FAGLPostBuf.Amount := -NewAmount;
                            FAGLPostBuf.Correction := Correction;
                            FAGLPostBuf."Global Dimension 1 Code" := FAAlloc."Global Dimension 1 Code";
                            FAGLPostBuf."Global Dimension 2 Code" := FAAlloc."Global Dimension 2 Code";
                            FAGLPostBuf."Automatic Entry" := AutomaticEntry;
                            FAGLPostBuf."FA Posting Group" := FAAlloc.Code;
                            FAGLPostBuf."FA Allocation Type" := FAAlloc."Allocation Type";
                            FAGLPostBuf."FA Allocation Line No." := FAAlloc."Line No.";
    ...
    begin
        ...
        FAGLPostBuf."Account No." := FAPostingGr."Sales Bal. Acc.";
        FAGLPostBuf.Amount := FALedgEntry.Amount;
        if Convert then
            FAGLPostBuf.Amount := -FAGLPostBuf.Amount;
        FAGLPostBuf.Correction := FALedgEntry.Correction;
        FAGLPostBuf."Global Dimension 1 Code" := FALedgEntry."Global Dimension 1 Code";
        FAGLPostBuf."Global Dimension 2 Code" := FALedgEntry."Global Dimension 2 Code";
        FAGLPostBuf."Dimension Set ID" := FALedgEntry."Dimension Set ID";
        FAGLPostBuf."FA Entry No." := FALedgEntry."Entry No.";
        if not Convert then
            FAGLPostBuf."Disposal Entry No." := -1
        //for sorting
        else
            FAGLPostBuf."Disposal Entry No." := FALedgEntry."Disposal Entry No.";
        if FALedgEntry."Entry No." > 0 then
            FAGLPostBuf."FA Entry Type" := FAGLPostBuf."FA Entry Type"::"Fixed Asset";
        FAGLPostBuf."Automatic Entry" := FALedgEntry."Automatic Entry";
        GLEntryNo := FALedgEntry."G/L Entry No.";
        InsertBufferEntry();
        FALedgEntry."G/L Entry No." := TempFAGLPostBuf."Entry No.";
    end;
