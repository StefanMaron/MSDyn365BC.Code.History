codeunit 5606 "FA Check Consistency"
{
    ...
    begin
        if (not FixedAsset."Undepreciable FA") and (FADeprBook."Disposal Date" > 0D) then
    ...
    [Scope('OnPrem')]
    procedure IsFAMovement(): Boolean
    var
        FALedgEntry3: Record "FA Ledger Entry";
        PstdFADocHeader: Record "Posted FA Doc. Header";
        FADocAmount: Decimal;
    begin
        if FALedgEntry."Entry No." = FALedgEntry2."Entry No." then
            exit(false);
