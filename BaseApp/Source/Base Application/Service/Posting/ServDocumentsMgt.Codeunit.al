...
#pragma warning restore AA0074
    ...
    begin
        if not ServHeader.FindVATExemption(VATExemption, Check, true) then
            if Check then
                Error(Text12100);
        if Invoice then begin
            ...
            if PaymentTermsLine.Find('-') then
                ...
                if ServHeader."Payment Terms Code" <> '' then
                    Error(Text1130003,
                      ServHeader.FieldCaption(ServHeader."Payment Terms Code"),
                      ServHeader."Payment Terms Code");
        end;
    ...
    begin
        ...
        then begin
            ...
            ServShptHeader."Shipment Method Code" := ServHeader."Shipment Method Code";
            ServShptHeader."Shipping Agent Code" := ServHeader."Shipping Agent Code";
            ...
            repeat
                ...
                if (ServLine.Type <> ServLine.Type::" ") and not ServLine."System-Created Entry" and not SkipCheckContractNoAndShipmentNo then begin
                    if ServDocType = Database::"Service Contract Header" then
                        if not ServLine."Automatically Generated" then
                            ServLine.TestField(ServLine."Contract No.");
                ...
                if ServLine."Document Type" in [ServLine."Document Type"::Order, ServLine."Document Type"::Invoice, ServLine."Document Type"::"Credit Memo"] then
                    if VATPostingSetup.IsEUService(ServLine."VAT Bus. Posting Group", ServLine."VAT Prod. Posting Group") then begin
                        if VATPostingSetup."Service Tariff No. Mandatory" then
                            ServLine.TestField(ServLine."Service Tariff No.");
    ...
    begin
        ...
        if Ship and (ServHeader."Shipping No." = '') then
            ...
            then begin
                ...
                ServHeader."Shipping No." := NoSeriesCodeunit.GetNextNo(ServHeader."Shipping No. Series", ServHeader."Posting Date");
                ModifyHeader := true;
            end;
        ...
        if Invoice and (ServHeader."Posting No." = '') then begin
            if (ServHeader."No. Series" <> '') or (ServHeader."Document Type" = ServHeader."Document Type"::Order) then begin
                ServHeader.TestField(ServHeader."Posting No. Series");
                NoSeries.Get(ServHeader."Posting No. Series");
            ...
            if (ServHeader."No. Series" <> ServHeader."Posting No. Series") or (ServHeader."Document Type" = ServHeader."Document Type"::Order) then begin
                ServHeader."Posting No." := NoSeriesCodeunit.GetNextNo(ServHeader."Posting No. Series", ServHeader."Posting Date");
                ModifyHeader := true;
            end;
        end;

        GLSetup.Get();
        if Invoice and (ServHeader."Operation Occurred Date" < GLSetup."Last Gen. Jour. Printing Date") then
            ServHeader.FieldError(ServHeader."Operation Occurred Date", StrSubstNo(Text1130000, GLSetup."Last Gen. Jour. Printing Date"));
