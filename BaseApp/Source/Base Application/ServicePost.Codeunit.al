codeunit 5980 "Service-Post"
{
    ...
    begin
        with PassedServiceHeader do begin
            ...
            if PassedServiceLine.IsEmpty then
                TestServLinePostingDate("Document Type", "No.", "Journal Template Name")
            else
                if "Posting Date" <> PassedServiceLine."Posting Date" then begin
                    if PassedServiceLine.Type <> PassedServiceLine.Type::" " then
                        if GenJnlCheckLine.DateNotAllowed(PassedServiceLine."Posting Date", "Journal Template Name") then
    ...
    local procedure TestServLinePostingDate(ServHeaderDocType: Integer; ServHeaderNo: Code[20]; JnlTemplateName: Code[10])
    var
