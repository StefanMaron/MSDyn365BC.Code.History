page 9303 "Blanket Sales Orders"
{
        ...
        area(processing)
        {
            ...
            action("Make &Order")
            {
                ...
                var
                    ApprovalsMgmt: Codeunit "Approvals Mgmt.";
                    SalesHeader: Record "Sales Header";
                    CreateRepeatingOrder: Report "Create Recurring Orders";
                begin
                    if "Recurring Group Code" = '' then begin
                        if ApprovalsMgmt.PrePostApprovalCheckSales(Rec) then
                            CODEUNIT.Run(CODEUNIT::"Blnkt Sales Ord. to Ord. (Y/N)", Rec)
                    end else begin
                        // Create Repeating Order
                        if not Confirm(CreateRecurringOrderQst, false) then
                            exit;
                        SalesHeader := Rec;
    ...
    var
        DocPrint: Codeunit "Document-Print";
        CreateRecurringOrderQst: Label 'This is a Recurring Order.\Do you whish to create a Sales Order?';
