table 112 "Sales Invoice Header"
{
    ...
    fields
    {
        ...
        field(12190; "Document Remaining Amount"; Decimal)
        {
            AutoFormatExpression = "Currency Code";
            AutoFormatType = 1;
            CalcFormula = Sum ("Detailed Cust. Ledg. Entry".Amount WHERE("Original Document Type" = CONST(Invoice), "Original Document No." = FIELD("No.")));
    ...
    begin
        ...
        if SalesHeader.FindVATExemption(VATExemption, VATExemptionCheck, true) then
            if VATExemptionCheck then
                exit(
                  StrSubstNo(
                    CustVATExemLbl,
