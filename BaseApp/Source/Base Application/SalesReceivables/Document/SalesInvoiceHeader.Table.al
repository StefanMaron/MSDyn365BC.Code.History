table 112 "Sales Invoice Header"
{
    ...
    begin
        ...
        DocSignMgt.DeletePostedDocSign(DATABASE::"Sales Invoice Header", "No.");
    ...
    begin
        DocumentPrint.PrintSalesInvoiceHeader(Rec, SendAsEmail, ShowRequestPage);
    end;
    ...
    [Scope('OnPrem')]
    procedure FindShipments(SalesInvHeader: Record "Sales Invoice Header")
    var
        ValueEntry: Record "Value Entry";
    ...
    begin
        ...
        if SalesInvLine.Find('-') then
            repeat
                ...
                ValueEntry.SetRange("Item No.", SalesInvLine."No.");
                if ValueEntry.Find('-') then
                    repeat
                        ...
                        I := I + 1;
                        if I = 1 then
                            DocNoFilter := SalesShptHeader."No."
                        else begin
                            if StrPos('|' + DocNoFilter + '|', '|' + SalesShptHeader."No." + '|') = 0 then
                                if (StrLen(DocNoFilter) + StrLen(SalesShptHeader."No.")) < MaxStrLen(DocNoFilter) then
                                    DocNoFilter := DocNoFilter + '|' + SalesShptHeader."No."
                                else
                                    Error(Text12400);
                        end;
