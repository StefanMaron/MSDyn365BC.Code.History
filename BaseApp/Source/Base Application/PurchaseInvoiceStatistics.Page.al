page 400 "Purchase Invoice Statistics"
{
    ...
    layout
    {
        area(content)
        {
            ...
            {
                Caption = 'General';
                field("VendAmount + InvDiscAmount + PmtDiscAmount"; VendAmount + InvDiscAmount + PmtDiscAmount)
                {
                ...
                field(PmtDiscAmount; PmtDiscAmount)
                {
                    ...
                    Editable = true;
                    ToolTip = 'Specifies the payment discount amount that you have been granted by vendors. ';
                }
        ...
        if PurchInvLine.Find('-') then
            repeat
                ...
                if "Prices Including VAT" then begin
                    InvDiscAmount +=
                        PurchInvLine."Inv. Discount Amount" / (1 + (PurchInvLine."VAT %" + PurchInvLine."EC %") / 100);
                    PmtDiscAmount +=
                        PurchInvLine."Pmt. Discount Amount" / (1 + (PurchInvLine."VAT %" + PurchInvLine."EC %") / 100)
                end else begin
                    InvDiscAmount += PurchInvLine."Inv. Discount Amount";
                    PmtDiscAmount += PurchInvLine."Pmt. Discount Amount";
                end;
