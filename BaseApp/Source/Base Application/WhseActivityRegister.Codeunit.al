codeunit 7307 "Whse.-Activity-Register"
{
    ...
    begin
        with WhseActivLine do begin
            ...
            WhseJnlLine.SetTracking("Serial No.", "Lot No.", "CD No.", "Warranty Date", "Expiration Date");
    ...
    begin
        with WhseActivLine do begin
            ...
            then begin
                ...
                TempBinContentBuffer."CD No." := "CD No.";
                ...
                repeat
                    if "Qty. to Handle (Base)" < 0 then begin
                        ...
                        ItemTrackingMgt.CheckWhseItemTrkgSetup(
                          BinContent."Item No.", BinContent."Location Code", WhseSNRequired, WhseLNRequired, WhseCDRequired, false);
    ...
    begin
        with TempBinContentBuffer do begin
            ...
            BreakBulkWhseActivLine.SetTrackingFilter("Serial No.", "Lot No.", "CD No.");
    ...
    begin
        ...
        if TempWhseActivLine.Find('-') then
            repeat
                ...
                ItemTrackingMgt.CheckWhseItemTrkgSetup(
                  TempWhseActivLine."Item No.", TempWhseActivLine."Location Code", WhseSNRequired, WhseLNRequired, WhseCDRequired, false);
                if WhseSNRequired or WhseLNRequired or WhseCDRequired then
                    repeat
                        ...
                        if WhseCDRequired then
                            TempWhseActivLine.TestField("CD No.");
            ...
            repeat
                ...
                repeat
                    ...
                    TempWhseActivLine.SetTrackingFilter(
                      TempWhseActivLine."Serial No.", TempWhseActivLine."Lot No.", TempWhseActivLine."CD No.");
                    QtyToRegisterBase := 0;
                    repeat
                    ...
                    if (TempWhseActivLine."Serial No." <> '') or (TempWhseActivLine."Lot No." <> '') or (TempWhseActivLine."CD No." <> '') then
                        if not IsQtyAvailToPickNonSpecificReservation(
                           TempWhseActivLine, WhseSNRequired, WhseLNRequired, WhseCDRequired, QtyToRegisterBase)
                        then
    ...
    begin
        ...
        ItemTrackingMgt.CheckWhseItemTrkgSetup(
          WhseActivLine2."Item No.", WhseActivLine2."Location Code", WhseSNRequired, WhseLNRequired, WhseCDRequired, false);
        ...
        if QtyToRegisterBase > 0 then begin
            ...
            then begin
                ...
                TempTrackingSpecification.SetTracking(
                  WhseActivLine2."Serial No.", WhseActivLine2."Lot No.", WhseActivLine2."CD No.",
    ...
    begin
        ...
        with WhseItemTrkgLine do begin
            ...
            SetRange("CD No.", WhseActivLine2."CD No.");
            if FindSet then
                repeat
    ...
    begin
        ...
        WhseItemTrkgLine.SetTrackingFilter(WhseActivLine."Serial No.", WhseActivLine."Lot No.", WhseActivLine."CD No.");
    ...
    local procedure CalcQtyReservedOnInventory(WhseActivLine: Record "Warehouse Activity Line"; SNRequired: Boolean; LNRequired: Boolean; CDRequired: Boolean)
    begin
        with WhseActivLine do begin
            GetItem("Item No.");
    ...
    begin
        with WhseItemTrkgLine2 do begin
            ...
            SetTracking(
              WhseActivLine."Serial No.", WhseActivLine."Lot No.", WhseActivLine."CD No.",
              WhseActivLine."Warranty Date", WhseActivLine."Expiration Date");
    ...
    local procedure CalcTotalAvailQtyToPick(WhseActivLine: Record "Warehouse Activity Line"; SNRequired: Boolean; LNRequired: Boolean; CDRequired: Boolean): Decimal
    var
    ...
    begin
        with WhseActivLine do begin
            CalcQtyReservedOnInventory(WhseActivLine, SNRequired, LNRequired, CDRequired);
            ...
            then begin
                ...
                QtyOnOutboundBinsBase :=
                  CreatePick.CalcQtyOnOutboundBins(
                    "Location Code", "Item No.", "Variant Code", "Lot No.", "Serial No.", "CD No.", true);
                ...
                if SubTotalBase < 0 then begin
                    CreatePick.FilterWhsePickLinesWithUndefinedBin(
                      ...
                      LNRequired, "Lot No.", SNRequired, "Serial No.", CDRequired, "CD No.");
                ...
                QtyPickedNotShipped := CalcQtyPickedNotShipped(WhseActivLine, SNRequired, LNRequired, CDRequired);
    ...
    local procedure IsQtyAvailToPickNonSpecificReservation(WhseActivLine: Record "Warehouse Activity Line"; SNRequired: Boolean; LNRequired: Boolean; CDRequired: Boolean; QtyToRegister: Decimal): Boolean
    var
        QtyAvailToPick: Decimal;
    begin
        QtyAvailToPick := CalcTotalAvailQtyToPick(WhseActivLine, SNRequired, LNRequired, CDRequired);
        if QtyAvailToPick < QtyToRegister then
            if ReleaseNonSpecificReservations(WhseActivLine, SNRequired, LNRequired, CDRequired, QtyToRegister - QtyAvailToPick) then
                QtyAvailToPick := CalcTotalAvailQtyToPick(WhseActivLine, SNRequired, LNRequired, CDRequired);

        exit(QtyAvailToPick >= QtyToRegister);
    end;

    local procedure CalcQtyPickedNotShipped(WhseActivLine: Record "Warehouse Activity Line"; SNRequired: Boolean; LNRequired: Boolean; CDRequired: Boolean) QtyBasePicked: Decimal
    var
    ...
    begin
        with WhseActivLine do begin
            ...
            if CDRequired then
                ReservEntry.SetRange("CD No.", "CD No.")
            else
                ReservEntry.SetFilter("CD No.", '%1|%2', "CD No.", '');
            ...
            if SNRequired or LNRequired then begin
                ...
                RegWhseActivLine.SetRange("CD No.", "CD No.");
    ...
    begin
        with WhseActivLine2 do begin
            ...
            TempTrackingSpecification.SetTrackingFilter("Serial No.", "Lot No.", "CD No.");
            if TempTrackingSpecification.IsEmpty then begin
                ...
                TempTrackingSpecification.SetTracking("Serial No.", "Lot No.", "CD No.", "Warranty Date", "Expiration Date");
        ...
        if TempWhseActivLineToReserve.FindSet then
            repeat
                ...
                if not IsSalesLineCompletelyReserved(SalesLine) then begin
                    ...
                    ReservMgt.SetSerialLotNo(
                      TempWhseActivLineToReserve."Serial No.", TempWhseActivLineToReserve."Lot No.", TempWhseActivLineToReserve."CD No.");
    ...
    local procedure ReleaseNonSpecificReservations(WhseActivLine: Record "Warehouse Activity Line"; SNRequired: Boolean; LNRequired: Boolean; CDRequired: Boolean; QtyToRelease: Decimal): Boolean
    var
    ...
    begin
        exit(
          ...
          ((WhseActivityLine."Serial No." <> '') or (WhseActivityLine."Lot No." <> '') or (WhseActivityLine."CD No." <> '')));
    end;
