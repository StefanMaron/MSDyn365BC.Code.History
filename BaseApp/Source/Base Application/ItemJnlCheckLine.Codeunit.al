    ...
    begin
        ...
        with ItemJournalLine do begin
            ...
            if (InvtSetup."Unit of Measure Mandatory") and
              ("Value Entry Type" = "Value Entry Type"::"Direct Cost") and
              ("Item Charge No." = '') and (Quantity <> 0)
            then
                TestField("Unit of Measure Code", ErrorInfo.Create());
            ...
            if "Red Storno" and InvtSetup."Enable Red Storno" and not ItemJnlLineReserve.ReservEntryExist(ItemJournalLine) then
    ...
    begin
        with ItemJnlLine do begin
            ...
            if "Document Type" = "Document Type"::"Sales Return Receipt" then
                TestField("Applies-from Entry", ErrorInfo.Create());
            ...
            if ("Value Entry Type" = "Value Entry Type"::Revaluation) and
              ("Unit Cost" > 0) then
                TestField("Red Storno", false, ErrorInfo.Create());
                ...
                if ("FA No." <> '') or (LocItemLedgerEntry."FA No." <> '') then begin
                    if "FA No." <> LocItemLedgerEntry."FA No." then
                        Error(
                            ErrorInfo.Create(
                                StrSubstNo(
                                    Text12401, FieldCaption("FA No."), Format(LocItemLedgerEntry."FA No.")),
                                true));
                    if "Depreciation Book Code" <> LocItemLedgerEntry."Depreciation Book Code" then
                        Error(
                            ErrorInfo.Create(
                                StrSubstNo(
                                    Text12401, FieldCaption("Depreciation Book Code"),
                    ...
                    if LocLatestItemLedgerEntry.FindFirst() then
                        Error(
                            ErrorInfo.Create(
                                StrSubstNo(
                                    Text12402, FieldCaption("FA No."), Format("FA No."),
