table 4 Currency
{
    ...
    fields
    {
        ...
        {
            Caption = 'Unit Kind';
            OptionCaption = 'Female,Male,Neuter';
            OptionMembers = Female,Male,Neuter;
        }
        field(12401; "Unit Name 1"; Text[30])
        {
            Caption = 'Unit Name 1';
        }
        field(12402; "Unit Name 2"; Text[30])
        {
            Caption = 'Unit Name 2';
        }
        field(12403; "Unit Name 5"; Text[30])
        {
            Caption = 'Unit Name 5';
        }
        field(12404; "Hundred Kind"; Option)
        {
            Caption = 'Hundred Kind';
            OptionCaption = 'Female,Male,Neuter';
            OptionMembers = Female,Male,Neuter;
        }
        field(12405; "Hundred Name 1"; Text[30])
        {
            Caption = 'Hundred Name 1';
        }
        field(12406; "Hundred Name 2"; Text[30])
        {
            Caption = 'Hundred Name 2';
        }
        field(12407; "Hundred Name 5"; Text[30])
        {
            Caption = 'Hundred Name 5';
        }
        field(12409; "Search Method"; Option)
        {
            Caption = 'Search Method';
            OptionCaption = 'To Date,As of Date';
            OptionMembers = "To Date","As of Date";
        }
        field(12410; Conventional; Boolean)
        {
            Caption = 'Conventional';
        }
        field(12411; "Invoice Comment"; Text[250])
        {
            Caption = 'Invoice Comment';
        }
        field(12416; "Sales PD Gains Acc. (TA)"; Code[20])
        {
            Caption = 'Sales PD Gains Acc. (TA)';
            TableRelation = "G/L Account";
        }
        field(12417; "Sales PD Losses Acc. (TA)"; Code[20])
        {
            Caption = 'Sales PD Losses Acc. (TA)';
            TableRelation = "G/L Account";
        }
        field(12418; "Purch. PD Gains Acc. (TA)"; Code[20])
        {
            Caption = 'Purch. PD Gains Acc. (TA)';
            TableRelation = "G/L Account";
        }
        field(12419; "Purch. PD Losses Acc. (TA)"; Code[20])
        {
            Caption = 'Purch. PD Losses Acc. (TA)';
            TableRelation = "G/L Account";
        }
        field(12420; Import; Boolean)
        {
            Caption = 'Import';

            trigger OnValidate()
            begin
                if Import and ("RU Bank Code" = '') then
                    "RU Bank Code" := Code;
            end;
        }
        field(12421; "RU Bank Code"; Code[10])
        {
            Caption = 'RU Bank Code';
        }
        field(12422; "PD Bal. Gain/Loss Acc. (TA)"; Code[20])
        {
            Caption = 'PD Bal. Gain/Loss Acc. (TA)';
            TableRelation = "G/L Account";
        }
        field(12423; "Description 2"; Text[30])
        {
            Caption = 'Description 2';
        }
        field(12424; "RU Bank Digital Code"; Code[3])
        {
            Caption = 'RU Bank Digital Code';
            Numeric = true;
        }
        field(12491; "Agreement Filter"; Code[20])
        {
    ...
    begin
        ...
        if GLSetup."Inv. Rounding Precision (LCY)" = 0 then begin
            if "Amount Rounding Precision" > 0.01 then
                "Invoice Rounding Precision" := "Amount Rounding Precision"
            else
    ...
    [Scope('OnPrem')]
    procedure GetSalesPrepmtDiffAccount(DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer"): Code[20]
    begin
        case DtldCVLedgEntryBuf."Entry Type" of
            DtldCVLedgEntryBuf."Entry Type"::"Realized Loss":
                begin
                    TestField("Sales PD Losses Acc. (TA)");
                    exit("Sales PD Losses Acc. (TA)");
                end;
            DtldCVLedgEntryBuf."Entry Type"::"Realized Gain":
                begin
                    TestField("Sales PD Gains Acc. (TA)");
                    exit("Sales PD Gains Acc. (TA)");
                end;
            else
                Error(IncorrectEntryTypeErr, DtldCVLedgEntryBuf."Entry Type");
        end;
    ...
    [Scope('OnPrem')]
    procedure GetPurchPrepmtDiffAccount(DtldCVLedgEntryBuf: Record "Detailed CV Ledg. Entry Buffer"): Code[20]
    begin
        case DtldCVLedgEntryBuf."Entry Type" of
            DtldCVLedgEntryBuf."Entry Type"::"Realized Loss":
                begin
                    TestField("Purch. PD Losses Acc. (TA)");
                    exit("Purch. PD Losses Acc. (TA)");
                end;
            DtldCVLedgEntryBuf."Entry Type"::"Realized Gain":
                begin
                    TestField("Purch. PD Gains Acc. (TA)");
                    exit("Purch. PD Gains Acc. (TA)");
                end;
            else
                Error(IncorrectEntryTypeErr, DtldCVLedgEntryBuf."Entry Type");
        end;
    ...
    procedure GetSalesPDGainsAccTA(): Code[20]
    begin
        TestField("Sales PD Gains Acc. (TA)");
        exit("Sales PD Gains Acc. (TA)");
    end;

    procedure GetSalesPDLossesAccTA(): Code[20]
    begin
        TestField("Sales PD Losses Acc. (TA)");
        exit("Sales PD Losses Acc. (TA)");
    end;

    procedure GetPurchPDGainsAccTA(): Code[20]
    begin
        TestField("Purch. PD Gains Acc. (TA)");
        exit("Purch. PD Gains Acc. (TA)");
    end;

    procedure GetPurchPDLossesAccTA(): Code[20]
    begin
        TestField("Purch. PD Losses Acc. (TA)");
        exit("Purch. PD Losses Acc. (TA)");
    end;

    procedure GetPDBalGainLossAccTA(): Code[20]
    begin
        TestField("PD Bal. Gain/Loss Acc. (TA)");
        exit("PD Bal. Gain/Loss Acc. (TA)");
    end;
