...
page 610 "IC General Journal"
{
    ...
    begin
        ...
        SetControlAppearance();
        SetJobQueueVisibility();
        Clear(GeneralJournalBatchApprovalStatus);
        if EnabledGeneralJournalBatchWorkflowsExist and GenJournalBatch.Get(Rec."Journal Template Name", CurrentJnlBatchName) then
            if ApprovalsMgmt.FindApprovalEntryByRecordId(ApprovalEntry, GenJournalBatch.RecordId) then
                GeneralJournalBatchApprovalStatus := GetApprovalStatusFromApprovalEntry(ApprovalEntry);
    end;
    ...
    begin
        ...
        if EnabledGeneralJournalLineWorkflowsExist then
            if ApprovalsMgmt.FindApprovalEntryByRecordId(ApprovalEntry, Rec.RecordId) then
                GeneralJournalLineApprovalStatus := GetApprovalStatusFromApprovalEntry(ApprovalEntry);
    ...
    begin
        ...
        if FieldRef.GetEnumValueName(ApprovalEntry.Status.AsInteger() + 1) = 'Open' then
            ApprovalStatus := PendingApprovalLbl
        else
            ApprovalStatus := FieldRef.GetEnumValueCaption(ApprovalEntry.Status.AsInteger() + 1);
