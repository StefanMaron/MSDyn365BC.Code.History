    ...
    begin
        if not TempICPartnerICInboxSalesHeader.IsEmpty() then begin
            ...
            repeat
                ...
                BufferICInboxPurchaseLine.TransferFields(TempICPartnerICInboxSalesHeader);
                BufferICInboxPurchaseLine.Insert();
...
#pragma warning disable AA0139 // Ignore warning about possible overflow from JSON Text
    ...
    begin
        ...
        foreach SelectedToken in AttributeToken.AsArray() do
            PopulateICInboxTransactionFromJson(SelectedToken, ICInboxTransaction);
    ...
    begin
        ...
        ICInboxPurchaseHeader."IC Partner Code" := GetValueFromJsonTokenOrEmptyText(IndividualToken, 'icPartnerCode');
        ICInboxPurchaseHeader."IC Transaction No." := GetValueFromJsonTokenOrIntegerZero(IndividualToken, 'icTransactionNumber');
    ...
    begin
        ...
        ICInboxSalesHeader."IC Partner Code" := GetValueFromJsonTokenOrEmptyText(IndividualToken, 'icPartnerCode');
        ICInboxSalesHeader."IC Transaction No." := GetValueFromJsonTokenOrIntegerZero(IndividualToken, 'icTransactionNumber');
    ...
    local procedure GetValueFromJsonTokenOrDecimalZero(IndividualToken: JsonToken; AttributeName: Text): Integer
    begin
        if IndividualToken.AsObject().Get(AttributeName, AttributeToken) then
