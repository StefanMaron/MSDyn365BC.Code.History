report 2 "General Journal - Test"
{
    ...
    dataset
    {
        dataitem("Gen. Journal Batch"; "Gen. Journal Batch")
        {
            ...
            dataitem("Integer"; "Integer")
            {
                ...
                column(TransactionNoCaption_GenJnlLine; "Gen. Journal Line".FieldCaption("Transaction No."))
                {
                }
                column(CurrencyCodeCaption_GenJnlLine; "Gen. Journal Line".FieldCaption("Currency Code"))
                {
                }
                column(DebitAmtLCYCaption; DebitAmtLCYCap)
                {
                }
                column(Credit_AmountCaption; CreditAmountCap)
                {
                }
                dataitem("Gen. Journal Line"; "Gen. Journal Line")
                {
                    ...
                    column(TransactionNo_GenJnlLine; "Transaction No.")
                    {
                    }
                    ...
                    begin
                        ...
                        if "Amount (LCY)" > 0 then
                            DebitAmtLCY := "Amount (LCY)"
                        else
                            CreditAmtLCY := "Amount (LCY)";
                        ...
                        if CarteraSetup.ReadPermission then
                            DocPost.CheckDocInfo("Gen. Journal Line", ErrorCounter, ErrorText);
                    end;
    ...
    var
        ...
        Text10700: Label 'Transaction %1 is out of balance by %2.';
        Text10701: Label 'The reversing entries for transaction %1 are out of balance by %2.';
        LastTransNo: Integer;
        DebitAmtLCY: Decimal;
        ...
        DebitAmtLCYCap: Label 'Debit Amount';
    ...
    begin
        ...
        with NextGenJnlLine do begin
            ...
            if not GenJnlTemplate."Force Doc. Balance" and
               (LastDate <> 0D) and
               (("Posting Date" <> LastDate) or
                ("Transaction No." <> LastTransNo) or
                ("Line No." = LastLineNo))
            then begin
                case true of
                    DocBalance <> 0:
                        AddError(
                          StrSubstNo(
                            Text10700,
                            LastTransNo, DocBalance));
                    DocBalanceReverse <> 0:
                        AddError(
                          StrSubstNo(
                            Text10701,
                            LastTransNo, DocBalanceReverse));
                end;
                DocBalance := 0;
                DocBalanceReverse := 0;
            end;
