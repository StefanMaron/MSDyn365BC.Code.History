table 5767 "Warehouse Activity Line"
{
    ...
    fields
    {
        ...
        field(26; "Qty. to Handle"; Decimal)
        {
            ...
            begin
                ...
                if not UseBaseQty then begin
                    "Qty. to Handle (Base)" := UOMMgt.CalcBaseQty("Qty. to Handle", "Qty. per Unit of Measure");
        ...
        field(28; "Qty. Handled"; Decimal)
        {
            ...
            trigger OnValidate()
            begin
                "Qty. Handled (Base)" := UOMMgt.CalcBaseQty("Qty. Handled", "Qty. per Unit of Measure");
            end;
        }
        ...
        field(6500; "Serial No."; Code[50])
        {
            ...
            begin
                if "Serial No." <> '' then begin
                    ItemTrackingMgt.CheckWhseItemTrkgSetup("Item No.", "Location Code", SNRequired, LNRequired, CDRequired, true);
                    TestField("Qty. per Unit of Measure", 1);
        ...
        field(7300; "Bin Code"; Code[20])
        {
            ...
            begin
                if "Action Type" = "Action Type"::Take then
                    BinCode :=
                      WMSMgt.BinContentLookUp("Location Code", "Item No.", "Variant Code", "Zone Code", "Lot No.", "Serial No.", "CD No.", "Bin Code")
                else
                                    ...
                                    if Location."Directed Put-away and Pick" then begin
                                        ...
                                        AvailableQtyBase :=
                                          CreatePick.CalcTotalAvailQtyToPick(
                                            "Location Code", "Item No.", "Variant Code", "Lot No.", "Serial No.", "CD No.",
        ...
        field(14900; "CD No."; Code[30])
        {
            ...
            begin
                LookUpBinContent := ("Activity Type" <= "Activity Type"::Movement) or ("Action Type" <> "Action Type"::Place);
                LookUpTrackingSummary(Rec, LookUpBinContent, -1, 2);
            end;
    ...
    keys
    {
        ...
        key(Key14; "Whse. Document No.", "Whse. Document Type", "Activity Type", "Whse. Document Line No.", "Action Type", "Unit of Measure Code", "Original Breakbulk", "Breakbulk No.", "Lot No.", "Serial No.", "CD No.", "Assemble to Order")
        {
            MaintainSIFTIndex = false;
        ...
        key(Key16; "Item No.", "Location Code", "Activity Type", "Bin Type Code", "Unit of Measure Code", "Variant Code", "Breakbulk No.", "Action Type", "Lot No.", "Serial No.", "CD No.", "Assemble to Order")
        {
            MaintainSIFTIndex = false;
        ...
        key(Key19; "Source No.", "Source Line No.", "Source Subline No.", "Serial No.", "Lot No.", "CD No.")
        {
            MaintainSQLIndex = false;
        }
    }
    ...
    begin
        ...
        with WhseActivLine do begin
            ...
            then begin
                ...
                WhseActivLine2.SetRange("CD No.", "CD No.");
    ...
    begin
        ...
        WhseActivLine.SetRange("CD No.", "CD No.");
    ...
    procedure LookUpTrackingSummary(var WhseActivLine: Record "Warehouse Activity Line"; SearchForSupply: Boolean; SignFactor: Integer; TrackingType: Option SerialNo,LotNo,CDNo)
    var
            ...
            case TrackingType of
                ...
                TrackingType::CDNo:
                    if TempTrackingSpecification."CD No." <> '' then begin
                        Validate("CD No.", TempTrackingSpecification."CD No.");
                        Modify;
                    end;
            end;
        end;
    ...
    procedure CheckReservedItemTrkg(CheckType: Option "Serial No.","Lot No.","CD No."; ItemTrkgCode: Code[50])
    var
        Item: Record Item;
            ...
            CheckType::"CD No.":
                begin
                    ...
                    LineReservedQty :=
                      WhseAvailMgt.CalcLineReservedQtyOnInvt(
                        "Source Type", "Source Subtype", "Source No.", "Source Line No.", "Source Subline No.", true, '', '',
                        CopyStr(ItemTrkgCode, 1, 30), TempWhseActivLine);
                    ReservEntry.SetCurrentKey("Item No.", "Variant Code", "Location Code", "Reservation Status");
                    ReservEntry.SetRange("Item No.", "Item No.");
                    ReservEntry.SetRange("Variant Code", "Variant Code");
                    ReservEntry.SetRange("Location Code", "Location Code");
                    ...
                    if (Item.Inventory - Abs(Item."Reserved Qty. on Inventory") +
                       ...
                       "Qty. to Handle (Base)"
                    then
                        Error(Text017, FieldCaption("CD No."), ItemTrkgCode);
                end;
        end;
    end;
    ...
    begin
        with WhseActivityLine do begin
            ...
            TrackingSpecification."CD No." := "CD No.";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterLookupTrackingSummary(var WarehouseActivityLine: Record "Warehouse Activity Line"; var TempTrackingSpecification: Record "Tracking Specification" temporary; TrackingType: Option SerialNo,LotNo)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckReservedItemTrkg(var WarehouseActivityLine: Record "Warehouse Activity Line"; CheckType: Option "Serial No.","Lot No."; ItemTrkgCode: Code[50]; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnLookUpTrackingSummaryOnAfterAssistEditTrackingNo(var WarehouseActivityLine: Record "Warehouse Activity Line"; var TrackingSpecification: Record "Tracking Specification"; TrackingType: Option SerialNo,LotNo)
    begin
    end;
