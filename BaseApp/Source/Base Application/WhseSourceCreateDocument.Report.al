report 7305 "Whse.-Source - Create Document"
{
            ...
            begin
                ...
                if not WhseWkshLine.FindFirst then begin
                    ...
                    if PostedWhseReceiptLine2."Qty. (Base)" > 0 then begin
                        ...
                        ItemTrackingManagement.CheckWhseItemTrkgSetup("Item No.", "Location Code", WhseSNRequired, WhseLNRequired, WhseCDRequired, false);
    ...
    begin
        ...
        with WhseItemTrackingLine do begin
            ...
            SetRange("CD No.", PostedWhseRcptLine."CD No.");
        ...
        with WhseItemTrackingLine do begin
            ...
            repeat
                TrackedQtyInBin := WarehouseAvailabilityMgt.CalcQtyOnBin(
                    ...
                    WhseWorksheetLine."Variant Code", "Lot No.", "Serial No.","CD No.");
                if TrackedQtyInBin < "Quantity (Base)" + WarehouseAvailabilityMgt.CalcQtyAssignedToMove(
                     WhseWorksheetLine, WhseItemTrackingLine)
                then
                    Error(TotalPendingMovQtyExceedsBinAvailErr, WhseWorksheetLine."Line No.", "Lot No.", "Serial No.","CD No.");
            until Next = 0;
        end;
    end;
        ...
        with WhseItemTrackingLine do begin
            ...
            if Find('-') then
                repeat
                    ...
                    TempWhseItemTrkgLine.SetRange("CD No.", "CD No.");
    ...
    begin
        if ItemNo <> '' then begin
            ...
            if Item."Item Tracking Code" <> '' then begin
                ...
                exit(((ItemTrackingCode."SN Specific Tracking") or (ItemTrackingCode."Lot Specific Tracking") or (ItemTrackingCode."CD Specific Tracking")));
            end;
        end;
        exit(false);
    end;
