codeunit 444 "Purchase-Post Prepayments"
{
    ...
    var
        ...
        PreviewMode: Boolean;
        PrepaymentType: Option " ",Prepayment,Advance;
        SaveLineRelations: Boolean;
            ...
            if PurchLine.Find('-') then
                repeat
                    if PrepmtAmount(PurchLine, DocumentType) <> 0 then begin
                        ...
                        // NAVCZ
                        if SaveLineRelations then begin
                            BufferEntryNo := BufferEntryNo + 1;
                            PrepmtInvLineBuf."Entry No." := BufferEntryNo;
                            PrepmtInvLineBuf.Modify;
                        end;
                        InsertLineRelation(PrepmtInvLineBuf, PurchLine, PurchHeader);
                        // NAVCZ
                    end;
    ...
    begin
        with PrepmtInvLineBuf do begin
            ...
            if PurchHeader."Prepayment Type" = PurchHeader."Prepayment Type"::Advance then begin
                ...
                "G/L Account No." := VATPostingSetup."Purch. Advance VAT Account";
            end else
                // NAVCZ
    ...
    begin
        with PurchLine do begin
            ...
            // NAVCZ
            if DocumentType in [DocumentType::Invoice, DocumentType::Statistic, DocumentType::Advance] then
                // NAVCZ
        ...
        with PurchLine do
            case DocumentType of
                ...
                DocumentType::Advance:
                    begin
                        CalcFields("Prepmt. Amt. Requested");
                        exit("Prepmt. Line Amount" - "Prepmt. Amt. Requested");
                    end;
                // NAVCZ
    ...
    local procedure UpdatePurchaseDocument(var PurchaseHeader: Record "Purchase Header"; var PurchLine: Record "Purchase Line"; DocumentType: Option Invoice,"Credit Memo"; GenJnlLineDocType: Option; GenJnlLineDocNo: Code[20]; GenJnlLineExtDocNo: Code[35]; SourceCode: Code[10])
    var
        GenJnlLine: Record "Gen. Journal Line";
    begin
    ...
    begin
        with PurchHeader do begin
            ...
            // NAVCZ
            PurchInvHeader."Prepayment Type" := PurchInvHeader."Prepayment Type"::Prepayment;
            // NAVCZ
    ...
    begin
        with PurchHeader do begin
            ...
            // NAVCZ
            PurchCrMemoHdr."Prepayment Type" := PurchCrMemoHdr."Prepayment Type"::Prepayment;
            // NAVCZ
    ...
    var
        LineRelation: Record "Advance Letter Line Relation";
        Currency: Record Currency;
    begin
        // NAVCZ
        if not SaveLineRelations then
            exit;
    ...
    local procedure xIncrAmounts(PrepmtInvLineBuf: Record "Prepayment Inv. Line Buffer"; var TotalPrepmtInvLineBuf: Record "Prepayment Inv. Line Buffer"; PurchHeader: Record "Purchase Header"; PurchLine: Record "Purchase Line"; UpdatePurchLine: Boolean; DocumentType: Option Invoice,"Credit Memo")
    var
        PrePmtAmount: Decimal;
    begin
