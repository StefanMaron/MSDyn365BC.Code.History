        ...
        area(content)
        {
            repeater(Control1)
            {
                ...
                field(ColumnValue; ColumnValue)
                {
                    ...
                    begin
                        if Rec.Type in [Rec.Type::"VAT Entry Totaling", Rec.Type::"EC Entry Totaling"] then
                            if VATPostingSetup.Get(Rec."VAT Bus. Posting Group", Rec."VAT Prod. Posting Group") and
                               (VATPostingSetup."VAT Calculation Type" = VATPostingSetup."VAT Calculation Type"::"No Taxable VAT") and
                               (Rec.Type = Rec.Type::"VAT Entry Totaling")
                            then begin
                                NoTaxableEntry.SetRange(Type, Rec."Gen. Posting Type");
                                NoTaxableEntry.SetRange("VAT Bus. Posting Group", Rec."VAT Bus. Posting Group");
                                NoTaxableEntry.SetRange("VAT Prod. Posting Group", Rec."VAT Prod. Posting Group");
                                if Rec.GetFilter("Date Filter") <> '' then
                                    if PeriodSelection = PeriodSelection::"Before and Within Period" then
                                        NoTaxableEntry.SetRange("Posting Date", 0D, Rec.GetRangeMax("Date Filter"))
                                    else
                                        Rec.CopyFilter("Date Filter", NoTaxableEntry."Posting Date");
                                case Selection of
                                    Selection::Open:
