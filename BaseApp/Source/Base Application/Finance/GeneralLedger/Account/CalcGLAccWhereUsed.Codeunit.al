        ...
        with GLAccWhereUsed do
            case "Table ID" of
                ...
                Database::"Withhold Code":
                    begin
                        WithholdCode.Code := CopyStr("Key 1", 1, MaxStrLen(WithholdCode.Code));
                        PAGE.Run(0, WithholdCode);
                    end;
                Database::"Contribution Code":
                    begin
                        ContributionCode.Code := CopyStr("Key 1", 1, MaxStrLen(ContributionCode.Code));
                        Evaluate(ContributionCode."Contribution Type", "Key 2");
                ...
                Database::"Bill Posting Group":
                    begin
                        BillPostingGroup."No." := CopyStr("Key 1", 1, MaxStrLen(BillPostingGroup."No."));
                        BillPostingGroup."Payment Method" := CopyStr("Key 2", 1, MaxStrLen(BillPostingGroup."Payment Method"));
                        PAGE.Run(PAGE::"Bill Posting Group", BillPostingGroup);
                    end;
                Database::Bill:
                    begin
                        Bill.Code := CopyStr("Key 1", 1, MaxStrLen(Bill.Code));
                        PAGE.Run(0, Bill);
                    end;
                else
