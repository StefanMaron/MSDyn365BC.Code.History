...
codeunit 100 "Calc. G/L Acc. Where-Used"
{
            ...
            DATABASE::"WHT Posting Setup":
                begin
                    WHTPostingSetup."WHT Business Posting Group" :=
                      CopyStr(GLAccWhereUsed."Key 1", 1, MaxStrLen(WHTPostingSetup."WHT Business Posting Group"));
                    WHTPostingSetup."WHT Product Posting Group" :=
                      CopyStr(GLAccWhereUsed."Key 2", 1, MaxStrLen(WHTPostingSetup."WHT Product Posting Group"));
                    PAGE.Run(PAGE::"WHT Posting Setup", WHTPostingSetup);
                end;
            else
    ...
    begin
        TableBuffer.Reset();
        AddTable(TableBuffer, DATABASE::"WHT Posting Setup");
    end;
