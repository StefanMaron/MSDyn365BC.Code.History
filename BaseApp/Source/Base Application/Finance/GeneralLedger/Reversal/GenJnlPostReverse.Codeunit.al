...
codeunit 17 "Gen. Jnl.-Post Reverse"
{
    ...
    begin
        ...
        ReversalEntry.CopyReverseFilters2(ValueEntry, TaxDiffLedgEntry);
    ...
    begin
        if GLEntry2.Find('+') then
            repeat
                ...
                GLEntry := GLEntry2;
                if (GLEntry2."FA Entry No." <> 0) and GLSetup."Enable Russian Tax Accounting" then
                    InsertReverseTaxEntry(FAInsertLedgerEntry, GLEntry2."Entry No.", GenJnlPostLine.GetNextEntryNo(), GLEntry2."FA Entry Type",
                      GLEntry2."FA Entry No.", GLEntry."FA Entry No.", GenJnlPostLine.GetNextTransactionNo());
                ...
                if GLSetup."Enable Russian Tax Accounting" then begin
                    DetailedCustLedgEntry.SetRange("Tax Diff. Transaction No.", GLEntry2."Transaction No.");
                    if DetailedCustLedgEntry.FindFirst() then begin
                        DetailedCustLedgEntry."Tax Diff. Transaction No." := 0;
                        DetailedCustLedgEntry.Modify();
                    end;
                    DetailedVendorLedgEntry.SetRange("Tax Diff. Transaction No.", GLEntry2."Transaction No.");
                    ...
                    TempTaxDiffLedgerEntry.SetRange("Transaction No.", GLEntry2."Transaction No.");
                end;
                ...
                if GLSetup."Enable Russian Accounting" then
                    ReverseVATRU(ReversalEntry, GLEntry, GenJournalLine."Source Code")
                else
    ...
    begin
        ...
        if GLEntryVATEntryLink.FindSet() then
            repeat
                ...
                NewVATEntry.Positive := NewVATEntry.Amount > 0;
    ...
    local procedure CopyTaxDiffLedgEntry(var TaxDiffLedgerEntry: Record "Tax Diff. Ledger Entry"; var TempTaxDiffLedgerEntry: Record "Tax Diff. Ledger Entry" temporary)
    begin
        if TaxDiffLedgerEntry.FindSet() then
            repeat
                if TaxDiffLedgerEntry.Reversed then
                    Error(CannotReverseErr);
                TempTaxDiffLedgerEntry := TaxDiffLedgerEntry;
                TempTaxDiffLedgerEntry.Insert();
            until TaxDiffLedgerEntry.Next(1) = 0;
    end;
        ...
        if GLEntryVATEntryLink.FindSet() then
            repeat
                ...
                if ReversalEntry."Corrected Period Date" <> 0D then begin
                    NewVATEntry."Corrected Document Date" := ReversalEntry."Corrected Period Date";
                    NewVATEntry."Additional VAT Ledger Sheet" := true;
                    NewVATEntry."Posting Date" := ReversalEntry."Posting Date";
                end;
                NewVATEntry.Insert();
                GLEntryVATEntryLink.InsertLink(GLEntry."Entry No.", NewVATEntry."Entry No.");
                GenJnlPostLine.IncrNextVATEntryNo();
                if VATEntry."Unrealized VAT Entry No." <> 0 then begin
                    NewVATEntry.Get(VATEntry."Unrealized VAT Entry No.");
                    NewVATEntry."Remaining Unrealized Amount" := NewVATEntry."Remaining Unrealized Amount" + VATEntry.Amount;
                    NewVATEntry."Remaining Unrealized Base" := NewVATEntry."Remaining Unrealized Base" + VATEntry.Base;
                    NewVATEntry."Add.-Curr. Rem. Unreal. Amount" := NewVATEntry."Add.-Curr. Rem. Unreal. Amount" + VATEntry."Additional-Currency Amount";
                    NewVATEntry."Add.-Curr. Rem. Unreal. Base" := NewVATEntry."Add.-Curr. Rem. Unreal. Base" + VATEntry."Additional-Currency Base";
                    NewVATEntry."VAT Settlement Part" := VATSettlementMgt.GetPart(VATEntry."Unrealized VAT Entry No.");
                    NewVATEntry.Modify();
                end;
            until GLEntryVATEntryLink.Next() = 0;
    end;
    ...
    begin
        repeat
            TaxDiffLedgEntry.Get(TmpTaxDiffLedgEntry."Entry No.");
            ...
            TaxDiffLedgEntry.ReverseDeprBonusRecover();

            TaxDiffLedgEntry.Reversed := true;
            TaxDiffLedgEntry."Reversed by Entry No." := NewTaxDiffLedgEntry."Entry No.";
            TaxDiffLedgEntry.Modify();

            if TaxDiffRegister."No." = 0 then begin
                TaxDiffRegister.LockTable();
                if TaxDiffRegister.Find('+') then
                    TaxDiffRegister."No." := TaxDiffRegister."No." + 1;
                TaxDiffRegister.Init();
                TaxDiffRegister."From Entry No." := NewTaxDiffLedgEntry."Entry No.";
                TaxDiffRegister."Creation Date" := Today;
                TaxDiffRegister."Journal Batch Name" := NewTaxDiffLedgEntry."Journal Batch Name";
                TaxDiffRegister."User ID" := UserId;
                TaxDiffRegister.Insert();
            end;
            TaxDiffRegister."To Entry No." := NewTaxDiffLedgEntry."Entry No.";
            TaxDiffRegister.Modify();

            NewTaxDiffLedgEntry."Transaction No." := GenJnlPostLine.GetNextTransactionNo();
            NewTaxDiffLedgEntry."Amount (Base)" := -TaxDiffLedgEntry."Amount (Base)";
            NewTaxDiffLedgEntry."Amount (Tax)" := -TaxDiffLedgEntry."Amount (Tax)";
            NewTaxDiffLedgEntry.Difference := -TaxDiffLedgEntry.Difference;
            NewTaxDiffLedgEntry."Tax Amount" := -TaxDiffLedgEntry."Tax Amount";
            NewTaxDiffLedgEntry."Asset Tax Amount" := -TaxDiffLedgEntry."Asset Tax Amount";
            NewTaxDiffLedgEntry."Liability Tax Amount" := -TaxDiffLedgEntry."Liability Tax Amount";
            NewTaxDiffLedgEntry."Disposal Tax Amount" := -TaxDiffLedgEntry."Disposal Tax Amount";
            if NewTaxDiffLedgEntry."Partial Disposal" then begin
                NewTaxDiffLedgEntry."DTA Ending Balance" := -TaxDiffLedgEntry."DTA Ending Balance";
                NewTaxDiffLedgEntry."DTL Ending Balance" := -TaxDiffLedgEntry."DTL Ending Balance";
            end else begin
                NewTaxDiffLedgEntry."DTA Starting Balance" := TaxDiffLedgEntry."DTA Ending Balance";
                NewTaxDiffLedgEntry."DTL Starting Balance" := TaxDiffLedgEntry."DTL Ending Balance";
                NewTaxDiffLedgEntry."DTA Ending Balance" := TaxDiffLedgEntry."DTA Starting Balance";
                NewTaxDiffLedgEntry."DTL Ending Balance" := TaxDiffLedgEntry."DTL Starting Balance";
            end;
            NewTaxDiffLedgEntry."YTD Amount (Base)" := -TaxDiffLedgEntry."YTD Amount (Base)";
            NewTaxDiffLedgEntry."YTD Amount (Tax)" := -TaxDiffLedgEntry."YTD Amount (Tax)";
            NewTaxDiffLedgEntry."YTD Difference" := -TaxDiffLedgEntry."YTD Difference";

            NewTaxDiffLedgEntry.Reversed := true;
            NewTaxDiffLedgEntry."Reversed Entry No." := TaxDiffLedgEntry."Entry No.";
    ...
    begin
        TaxRegSetup.Get();
        FALedgEntryLoc.SetCurrentKey("G/L Entry No.");
        FALedgEntryLoc.SetRange("G/L Entry No.", GLEntryNo);
        if FALedgEntryLoc.Find('-') then
            repeat
                if (FALedgEntryLoc."Entry No." <> FAEntryNo) and (FALedgEntryLoc."Depreciation Book Code" = TaxRegSetup."Tax Depreciation Book") then
                    FAInsertLedgEntry.InsertReverseEntry(NewGLEntryNo, FAEntryType, FALedgEntryLoc."Entry No.", NewFAEntryNo, TransactionNo);
            until FALedgEntryLoc.Next() = 0;
    end;
    ...
    begin
        ...
        TempReversalEntry."Corrected Period Date" := GenJnlLine."Corrected Document Date";
        TempReversalEntry."Posting Date" := GenJnlLine."Posting Date";
        TempReversalEntry.SetVATSettlReverseFilters(UnrealizedVATEntry."Entry No.", FirstEntryNo, LastEntryNo);
        Reverse(TempReversalEntry, TempReversalEntry);
    end;
