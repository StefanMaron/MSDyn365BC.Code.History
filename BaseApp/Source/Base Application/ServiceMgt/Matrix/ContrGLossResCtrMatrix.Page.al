page 9265 "Contr. G/Loss (Res.Ctr) Matrix"
{
    ...
    Caption = 'Contract Gain/Loss (Resp.Ctr) Matrix';
    Editable = false;
    LinksAllowed = false;
    PageType = List;
    SourceTable = Date;
                ...
                field(TotalGainLoss; TotalGainLoss)
                {
                    ...
                    begin
                        ...
                        if RespCRFilter <> '' then
                            ContractGainLossEntry.SetRange("Reason Code", RespCRFilter);
                        if AmountType = AmountType::"Net Change" then
                            ContractGainLossEntry.SetRange("Change Date", "Period Start", "Period End")
                        else
                            ContractGainLossEntry.SetRange("Change Date", 0D, "Period End");
    ...
    var
        ...
        RespCr: Record "Responsibility Center";
    ...
    begin
        if AmountType = AmountType::"Net Change" then
            RespCr.SetRange("Date Filter", "Period Start", "Period End")
        else
            RespCr.SetRange("Date Filter", PeriodStart, "Period End");
    end;

    local procedure CalculateMovement("Code": Code[10]): Decimal
    begin
        if RespCr.Get(Code) then begin
            SetDateFilter();
            RespCr.CalcFields("Contract Gain/Loss Amount");
            exit(RespCr."Contract Gain/Loss Amount");
        end;
        exit(0);
    end;
    ...
    procedure LoadMatrix(NewMatrixColumns: array[32] of Text[1024]; var NewMatrixRecords: array[32] of Record "Responsibility Center"; NewCurrentNoOfMatrixColumns: Integer; NewAmountType: Enum "Analysis Amount Type"; NewPeriodType: Enum "Analysis Period Type"; NewRespCrFilter: Text[250]; NewPeriodStart: Date)
    begin
