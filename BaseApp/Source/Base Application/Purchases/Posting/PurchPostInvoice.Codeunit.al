...
codeunit 816 "Purch. Post Invoice" implements "Invoice Posting"
{
    ...
    begin
        ...
        if InvoicePostingBuffer.Type = InvoicePostingBuffer.Type::"Fixed Asset" then begin
            ...
            InvoicePostingBuffer."FA Location Code" := PurchLine."FA Location Code";
            InvoicePostingBuffer."Employee No." := PurchLine."Employee No.";
            if PurchLine.IsCreditDocType() then
                InvoicePostingBuffer.Quantity := -PurchLine.Quantity
            else
                InvoicePostingBuffer.Quantity := PurchLine.Quantity;
            InvoicePostingBuffer."FA Charge No." := PurchLine."FA Charge No.";
            InvoicePostingBuffer."Tax Difference Code" := PurchLine."Tax Difference Code";
        end;
