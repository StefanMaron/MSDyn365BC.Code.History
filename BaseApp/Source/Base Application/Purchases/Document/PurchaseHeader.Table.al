...
table 38 "Purchase Header"
{
    ...
    fields
    {
        ...
        field(2; "Buy-from Vendor No."; Code[20])
        {
            ...
            begin
                ...
                if Vend."Preferred Bank Account Code" <> '' then
                    VendBankAccount.SetRange(Code, Vend."Preferred Bank Account Code");
                if VendBankAccount.FindFirst() then
                    "Bank Account" := VendBankAccount.Code
                else
                    "Bank Account" := '';
                ...
                if "Document Type" <> "Document Type"::Quote then begin
                    if ("Document Date" > "Posting Date") then begin
                        if HideValidationDialog or not GuiAllowed then
                            ...
                            Confirmed := Confirm(Text1130015, false, FieldCaption("Document Date"), FieldCaption("Posting Date"));
                        if Confirmed and ("Incoming Document Entry No." = 0) then
                            ValidateDocumentDateWithPostingDate()
                        else
        ...
        field(21; "Expected Receipt Date"; Date)
        {
            ...
            begin
                ...
                if "Expected Receipt Date" < "Posting Date" then
                    Validate("Operation Occurred Date", "Expected Receipt Date");
        ...
        field(23; "Payment Terms Code"; Code[10])
        {
            ...
            begin
                PaymentLines.CreatePaymentLiensPurchases(Rec);
        ...
        field(27; "Shipment Method Code"; Code[10])
        {
            ...
            begin
                ...
                if "Shipment Method Code" <> '' then begin
                    ...
                    Validate("Entry Point", ShipmentMethod."Intra Shipping Code");
                end;
            end;
        }
        ...
        field(53; "Applies-to Doc. No."; Code[20])
        {
            ...
            begin
                ...
                if ApplyVendEntries.RunModal() = ACTION::LookupOK then begin
                    ...
                    "Applies-to Occurrence No." := VendLedgEntry."Document Occurrence";
            ...
            begin
                if ("Prepmt. Payment Terms Code" <> '') and ("Document Date" <> 0D) then begin
                    ...
                    if IsCreditDocType() and not PaymentTerms."Calc. Pmt. Disc. on Cr. Memos" then begin
                        ...
                        PaymentTermsLine.SetRange("Sales/Purchase", PaymentPurchase."Sales/Purchase"::" ");
                        PaymentTermsLine.SetRange(Type, PaymentTermsLine.Type::"Payment Terms");
                        PaymentTermsLine.SetRange(Code, "Prepmt. Payment Terms Code");
                        PaymentTermsLine.FindFirst();
                        IsHandled := false;
                        OnValidatePaymentTermsCodeOnBeforeCalcDueDate(Rec, xRec, FieldNo("Prepmt. Payment Terms Code"), CurrFieldNo, IsHandled);
                        if not IsHandled then
                            "Prepayment Due Date" := CalcDate(PaymentTermsLine."Due Date Calculation", "Document Date");
                        IsHandled := false;
                        ...
                        if not IsHandled then
                            "Prepmt. Pmt. Discount Date" := CalcDate(PaymentTermsLine."Discount Date Calculation", "Document Date");
                        if not UpdateDocumentDate then
                            Validate("Prepmt. Payment Discount %", PaymentTermsLine."Discount %")
                    end;
                end else begin
                    IsHandled := false;
        ...
        field(12100; "Operation Type"; Code[20])
        {
            ...
            begin
                if "Posting No." <> '' then
                    Error(Text1130012, FieldCaption("Operation Type"), FieldCaption("Posting No."));

                if "Operation Type" <> '' then begin
                    GlobalNoSeries.Get("Operation Type");

                    if GlobalNoSeries."No. Series Type" <> GlobalNoSeries."No. Series Type"::Purchase then
                        if not Confirm(Text1130011, false, FieldCaption("Operation Type")) then
                            FieldError("Operation Type");

                    "Posting No. Series" := "Operation Type";
                    "Prepayment No. Series" := "Operation Type";
                    "Prepmt. Cr. Memo No. Series" := "Operation Type";
                    "Reverse Sales VAT No. Series" := GlobalNoSeries."Reverse Sales VAT No. Series";
                end;
            end;
        }
            ...
            begin
                if ("Posting No." <> '') and ("Posting No. Series" <> '') then begin
                    ...
                    if GlobalNoSeries."Date Order" then
                        Error(
                          Text040,
                          FieldCaption("Operation Occurred Date"), FieldCaption("Posting No. Series"), "Posting No. Series",
                          GlobalNoSeries.FieldCaption("Date Order"), GlobalNoSeries."Date Order", "Document Type",
                          FieldCaption("Posting No."), "Posting No.");
                end;
                if not ("Document Type" in ["Document Type"::"Blanket Order", "Document Type"::Quote]) then begin
                    if "Operation Occurred Date" > "Posting Date" then
                        Error(Text1130017, FieldCaption("Operation Occurred Date"), FieldCaption("Posting Date"));
                end;
        ...
        field(12125; "Service Tariff No."; Code[10])
        {
            ...
            begin
                ...
                if ("Service Tariff No." <> xRec."Service Tariff No.") and
                   (xRec."Sell-to Customer No." = "Sell-to Customer No.")
                then
                    MessageIfPurchLinesExist(FieldCaption("Service Tariff No."));
            end;
        }
        ...
        field(12132; "Related Entry No."; Integer)
        {
            ...
            trigger OnValidate()
            begin
                if "Related Entry No." <> xRec."Related Entry No." then
                    UpdatePurchLinesByFieldNo(FieldNo("Related Entry No."), CurrFieldNo <> 0);
            end;
        }
        ...
        field(12138; "Refers to Period"; Option)
        {
            ...
            begin
                if xRec."Refers to Period" <> "Refers to Period" then
                    MessageIfPurchLinesExist(FieldCaption("Refers to Period"));
            end;
        }
        field(12139; "Prepmt. CM Refers to Period"; Option)
        {
            ...
            trigger OnValidate()
            begin
                if xRec."Prepmt. CM Refers to Period" <> "Prepmt. CM Refers to Period" then
                    MessageIfPurchLinesExist(FieldCaption("Prepmt. CM Refers to Period"));
            end;
        }
    ...
    keys
    {
        ...
        key(Key6; "Document Type", "Posting Date")
        {
        }
        key(Key7; "Incoming Document Entry No.")
        {
        }
        key(Key8; "Document Date")
        {
        }
        key(Key9; Status, "Expected Receipt Date", "Location Code", "Responsibility Center")
        {
        }
    ...
    var
        ...
        Text1130011: Label 'This %1 has a sales type VAT register. Continue anyway?';
        Text1130012: Label 'You cannot change %1 because %2 is not blank';
        Text1130015: Label '%1 will be modified according to %2';
        Text1130016: Label '%1 cannot be less than %2';
        Text1130017: Label '%1 cannot be greater than %2';
        Text1130018: Label 'A Posting No. has been assigned to this record. You cannot delete this document.';
        Text1130019: Label 'You cannot update the order line because the order line is associated with production order %1.';
        Text12100: Label ' %1 %2 must be Vendor/Contact for %3 %4 3rd-Party Loader.';
        Text12101: Label 'The vendor has an active VAT exemption and VAT Bus. Posting Group hasn''t "Check VAT Exemption". Do you want to continue?';
        Text12102: Label 'It is not possible to insert a vendor with VAT exemption if an active VAT exemption doesn''t exist.';
    ...
    begin
        ...
        if "Currency Code" <> '' then begin
            if "Posting Date" = 0D then
                ...
                GLSetup.Get();
                if GLSetup."Use Document Date in Currency" then
            ...
            repeat
                ...
                if not IsHandled then
                    case ChangedFieldNo of
                        ...
                        FieldNo("Related Entry No."):
                            if PurchLine."VAT Prod. Posting Group" <> '' then
                                PurchLine."Related Entry No." := "Related Entry No.";
    ...
    begin
        IsHandled := false;
        OnBeforeSetOperationType(IsHandled);
        if IsHandled then
            exit;
    ...
    begin
        ...
        VATExemption.SetRange("No.", "Pay-to Vendor No.");
    ...
    [Scope('OnPrem')]
    procedure AssignVATRegistrationNo(VendorNo: Code[20])
    begin
    ...
    begin
        ...
        if Vendor."VAT Registration No." <> '' then
            ...
            if Vendor."Tax Representative No." <> '' then
                case Vendor."Tax Representative Type" of
                    Vendor."Tax Representative Type"::Contact:
                        begin
                            TaxRepContact.Get(Vendor."Tax Representative No.");
                    ...
                    Vendor."Tax Representative Type"::Vendor:
                        begin
                            TaxRepVendor.Get(Vendor."Tax Representative No.");
                            TaxRepVendor.TestField("VAT Registration No.");
                            VATRegNo := TaxRepVendor."VAT Registration No.";
                        end;
                end;
        exit(VATRegNo);
    end;
...
#pragma warning restore AL0432
    ...
    begin
        ...
        if PurchLine."Prod. Order No." <> '' then
            Error(Text1130019, PurchLine."Prod. Order No.");
        PurchLine.TestField("Quantity Received", 0);
        PurchLine.TestField("Quantity Invoiced", 0);
        PurchLine.TestField("Return Qty. Shipped", 0);
    ...
    [IntegrationEvent(false, false)]
    procedure OnBeforeFindVATExemption(var PurchaseHeader: Record "Purchase Header"; var VATExemption: Record "VAT Exemption"; var Check: Boolean; CheckFirst: Boolean; var Result: Boolean; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterSetOperationType(var PurchaseHeader: Record "Purchase Header");
    begin
    end;
    ...
    [IntegrationEvent(true, false)]
    local procedure OnBeforeSetOperationType(var IsHandled: Boolean)
    begin
    end;
