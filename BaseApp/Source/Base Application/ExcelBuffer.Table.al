table 370 "Excel Buffer"
{
    ...
    var
        ...
        EnumValue: DotNet Enumvalue1;
        StringValue: DotNet StringValue;
        CustomFont: DotNet Font;
        CustomFontName: DotNet FontName;
        CustomPatternFill: DotNet PatternFill;
        CustomFill: DotNet Fill;
        CustomColor: DotNet "Spreadsheet.Color";
        CustomForegroundColor: DotNet "Spreadsheet.ForegroundColor";
    ...
    begin
        ...
        CustomFont := BaseDecorator.Font.Clone();
        CustomFill := BaseDecorator.Fill.Clone();

        // Font Name
        if FontName <> '' then begin
            GetFontScheme(EnumValue);
            CustomFont.FontScheme.Val := EnumValue;

            CustomFontName := CustomFontName.FontName();
            CustomFontName.Val := StringValue.StringValue(FontName);
        ...
        if FontColor <> -1 then begin
            ...
            CustomColor.Rgb(HexColor.HexBinaryValue(IntegerToHexARGB(FontColor)));

            CustomFont.Color := CustomColor;
        end;

        // Foreground Color
        if ForegroundColor <> -1 then begin
            CustomForegroundColor := CustomForegroundColor.ForegroundColor();
            CustomForegroundColor.Rgb(HexColor.HexBinaryValue(IntegerToHexARGB(ForegroundColor)));

            CustomPatternFill := CustomPatternFill.PatternFill();
            CustomPatternFill.ForegroundColor := CustomForegroundColor;
            GetPatternType(EnumValue);
            CustomPatternFill.PatternType := EnumValue;

            CustomFill.PatternFill := CustomPatternFill;
        end;

        Decorator.Font := CustomFont;
        Decorator.Fill := CustomFill;
    end;

    [Scope('OnPrem')]
    procedure GetColorNumber(Color: Option Red,Green,Blue,Yellow,Magenta,Cyan,White,Black): Integer
    begin
        // NAVCZ
        case Color of
            Color::Red:
                exit(HexARGBToInteger('00FF0000'));
            Color::Green:
                exit(HexARGBToInteger('00008000'));
            Color::Blue:
                exit(HexARGBToInteger('000000FF'));
            Color::Yellow:
                exit(HexARGBToInteger('00FFFF00'));
            Color::Magenta:
                exit(HexARGBToInteger('00FF00FF'));
            Color::Cyan:
                exit(HexARGBToInteger('0000FFFF'));
            Color::White:
                exit(HexARGBToInteger('00FFFFFF'));
            Color::Black:
                exit(HexARGBToInteger('00000000'));
        end;
    end;

    [Obsolete('The function is not needed anymore.', '21.0')]
    [Scope('OnPrem')]
    procedure IntegerToHexARGB(Value: Integer): Text
    var
    ...
    begin
        // NAVCZ
        PaddingChar := '0';
        String := String.Format('{0:X}', Value);
        exit(String.PadLeft(8, PaddingChar)); // Alpha Red Green Blue (AARRGGBB)
    end;

    [Obsolete('The function is not needed anymore.', '21.0')]
    [Scope('OnPrem')]
    procedure HexARGBToInteger(Value: Text): Integer
    var
        Int32: DotNet Int32;
        NumberStyles: DotNet NumberStyles;
    begin
        // NAVCZ
        exit(Int32.Parse(Value, NumberStyles.HexNumber));
    end;

    local procedure GetPatternType(var EnumValue: DotNet EnumValue1)
    var
        Type: DotNet Type;
        Enum: DotNet Enum;
        Activator: DotNet Activator;
        ArrTypes: DotNet Array;
        PatternFill: DotNet PatternFill;
        PatternValues: DotNet PatternValues;
    begin
        // NAVCZ
        PatternFill := PatternFill.PatternFill();

        Enum := PatternValues.Solid;

        ArrTypes := ArrTypes.CreateInstance(GetDotNetType(Type), 1);
        ArrTypes.SetValue(Enum.GetType(), 0);

        Type := PatternFill.GetType();
        Type := Type.GetProperty('PatternType').PropertyType();

        EnumValue := Activator.CreateInstance(Type);
        EnumValue.Value := PatternValues.Solid;
    end;

    local procedure GetFontScheme(var EnumValue: DotNet EnumValue1)
    var
        Type: DotNet Type;
        Enum: DotNet Enum;
        Activator: DotNet Activator;
        ArrTypes: DotNet Array;
        FontScheme: DotNet FontScheme;
        FontSchemeValues: DotNet FontSchemeValues;
    begin
        // NAVCZ
        FontScheme := FontScheme.FontScheme();

        Enum := FontSchemeValues.None;

        ArrTypes := ArrTypes.CreateInstance(GetDotNetType(Type), 1);
        ArrTypes.SetValue(Enum.GetType(), 0);

        Type := FontScheme.GetType();
        Type := Type.GetProperty('Val').PropertyType;
