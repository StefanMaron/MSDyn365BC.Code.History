            ...
            begin
                ...
                case Type of
                    ...
                    Type::"Fixed Asset":
                        CopyFromFixedAsset();
                    Type::"Charge (Item)":
                        CopyFromItemCharge(TempSalesLine);
                end;
...
#pragma warning restore
        ...
        field(12441; "Corrected Doc. Line No."; Integer)
        {
            Caption = 'Corrected Doc. Line No.';
        }
        field(12442; "Original Type"; Option)
        {
        ...
        field(12443; "Original No."; Code[20])
        {
            Caption = 'Original No.';
            Editable = false;
            TableRelation = if ("Original Type" = const(" ")) "Standard Text"
            else
            if ("Original Type" = const("G/L Account")) "G/L Account"
            else
            if ("Original Type" = const(Item)) Item
            else
            if ("Original Type" = const(Resource)) Resource
            else
            if ("Original Type" = const("Fixed Asset")) "Fixed Asset"
            else
            if ("Original Type" = const("Charge (Item)")) "Item Charge";
        }
        field(12445; "Quantity (Before)"; Decimal)
        {
            Caption = 'Quantity (Before)';
            Editable = false;
        }
        field(12446; "Quantity (After)"; Decimal)
        {
            Caption = 'Quantity (After)';
        ...
        {
            Caption = 'Unit Price (Before)';
            Editable = false;
        }
        field(12448; "Unit Price (After)"; Decimal)
        {
            Caption = 'Unit Price (After)';
        ...
        {
            Caption = 'Amount (Before)';
            Editable = false;
        }
        field(12450; "Amount (After)"; Decimal)
        {
            Caption = 'Amount (After)';
            Editable = false;
        }
        field(12451; "Original Doc. Type"; Option)
        {
            Caption = 'Original Doc. Type';
            OptionCaption = ' ,Invoice,Credit Memo';
            OptionMembers = " ",Invoice,"Credit Memo";
        }
        field(12452; "Original Doc. No."; Code[20])
        {
            Caption = 'Original Doc. No.';
        }
        field(12453; "Original Doc. Line No."; Integer)
        {
            Caption = 'Original Doc. Line No.';
        }
        field(12454; "Amount Including VAT (Before)"; Decimal)
        {
            Caption = 'Amount Including VAT (Before)';
            Editable = false;
        }
        field(12455; "Amount Including VAT (After)"; Decimal)
        {
            Caption = 'Amount Including VAT (After)';
            Editable = false;
        }
        field(12456; "Amount (LCY) (Before)"; Decimal)
        {
            Caption = 'Amount (LCY) (Before)';
        }
        field(12457; "Amt. Incl. VAT (LCY) (Before)"; Decimal)
        {
            Caption = 'Amt. Incl. VAT (LCY) (Before)';
        }
        field(12458; "Amount (LCY) (After)"; Decimal)
        {
            Caption = 'Amount (LCY) (After)';
        }
        field(12459; "Amt. Incl. VAT (LCY) (After)"; Decimal)
        {
            Caption = 'Amt. Incl. VAT (LCY) (After)';
        }
        field(12485; "No Ledger Entry"; Boolean)
        {
            Caption = 'No Ledger Entry';
            Editable = false;
        }
...
#pragma warning restore AS0038
        ...
        {
            IncludedFields = Amount, "Amount Including VAT", "Outstanding Amount", "Shipped Not Invoiced", "Outstanding Amount (LCY)", "Shipped Not Invoiced (LCY)", "Amount (LCY)", "Amount Including VAT (LCY)";
        }
    ...
    begin
        ...
        if (Rec.Quantity <> 0) and Rec.ItemExists("No.") then begin
    ...
    begin
        ...
        if not IsHandled then begin
            ...
            if SalesHeader."Prepmt. Diff. Appln. Entry No." <> 0 then begin
                "Gen. Prod. Posting Group" := TempSalesLine."Gen. Prod. Posting Group";
                "VAT Prod. Posting Group" := TempSalesLine."VAT Prod. Posting Group";
            end;
                    ...
                    "VAT Calculation Type"::"Reverse Charge VAT":
                        begin
                            if not SalesSetup."Calc. VAT per Line" then
                                ...
                                Amount :=
                                  Round(
                                    CalcLineAmount() / (1 + "VAT %" / 100), Currency."Amount Rounding Precision");
                            "VAT Base Amount" :=
                              Round(
    ...
    begin
        ...
        if SalesHeader."Prepmt. Diff." then
            exit;
    ...
    begin
        ...
        if "Depreciation Book Code" = '' then begin
            FASetup.Get();
            GLSetup.Get();
            if (FASetup."Disposal Depr. Book" <> '') and GLSetup."Enable Russian Accounting" then
                "Depreciation Book Code" := FASetup."Disposal Depr. Book"
            else
    ...
    begin
        ...
        if not IsHandled then
            VATAmountLine.UpdateLines(
                TotalVATAmount, Currency, SalesHeader."Currency Factor", SalesHeader."Prices Including VAT", SalesHeader."VAT Base Discount %",
                SalesHeader."Tax Area Code", SalesHeader."Tax Liable", SalesHeader."Posting Date", SalesSetup."Calc. VAT per Line");
    ...
    var
        AdjustmentFactorForInvoiceDiscount: Decimal;
    begin
...
"Document Type", Type, "No.", "Variant Code", "Drop Shipment", "Location Code", "Shipment Date");
    ...
    begin
        ...
        FAComment.SetRange("Document Type", FAComment."Document Type"::"Sales Invoice");
        ...
        if SalesLine.FindSet() then
            repeat
                ...
                if not AutoReserv then
                    Error(Text12402, SalesLine."Qty. to Ship (Base)", SalesLine."Line No.");
    ...
    begin
        ...
        if Type in [Type::" ", Type::Item] then
            FieldError(Type);
        if "Unit Price (After)" <> xRec."Unit Price (After)" then begin
            if SalesHeader."Original Doc. Type" = SalesHeader."Original Doc. Type"::"Credit Memo" then begin
                if ("Unit Price (After)" > "Unit Price (Before)") and
            ...
            if "Unit Price (After)" = "Unit Price (Before)" then
                ...
                Validate("Unit Price", Abs("Unit Price (After)" - "Unit Price (Before)"));
        end;
    end;
    ...
    begin
        ...
        if "Quantity (After)" <> xRec."Quantity (After)" then begin
            if SalesHeader."Original Doc. Type" = SalesHeader."Original Doc. Type"::"Credit Memo" then begin
                if ("Quantity (After)" > "Quantity (Before)") and
                   ("Unit Price (After)" < "Unit Price (Before)")
                then
                    Error(Text12404, FieldCaption("Quantity (After)"), FieldCaption("Quantity (Before)"));
                if ("Quantity (After)" < "Quantity (Before)") and
                   ("Unit Price (After)" > "Unit Price (Before)")
                then
                    Error(Text12402, FieldCaption("Quantity (After)"), FieldCaption("Quantity (Before)"));
            end else begin
                case SalesHeader."Document Type" of
                    SalesHeader."Document Type"::Invoice:
                        if "Quantity (After)" < "Quantity (Before)" then
                            Error(Text12402, FieldCaption("Quantity (After)"), FieldCaption("Quantity (Before)"));
                    SalesHeader."Document Type"::"Credit Memo":
                        if "Quantity (After)" > "Quantity (Before)" then
                            Error(Text12404, FieldCaption("Quantity (After)"), FieldCaption("Quantity (Before)"));
            ...
            if "Quantity (After)" = "Quantity (Before)" then
                ...
                Validate(Quantity, Abs("Quantity (After)" - "Quantity (Before)"));
            Validate("Unit Price");
        end;
    end;

    [Scope('OnPrem')]
    procedure UpdateCorrLine(var TempSalesLine: Record "Sales Line")
    begin
        if (not SalesHeader."Corrective Document") or (Type = Type::" ") then
            exit;

        Description := TempSalesLine.Description;
        "Corrected Doc. Line No." := TempSalesLine."Corrected Doc. Line No.";
        ...
        "Quantity (Before)" := TempSalesLine."Quantity (Before)";
        "Quantity (After)" := TempSalesLine."Quantity (After)";
        Quantity := TempSalesLine.Quantity;
        "Unit Price (Before)" := TempSalesLine."Unit Price (Before)";
        "Unit Price (After)" := TempSalesLine."Unit Price (After)";
        "Amount (Before)" := TempSalesLine."Amount (Before)";
        "Amount Including VAT (Before)" := TempSalesLine."Amount Including VAT (Before)";
        "Amount (LCY) (Before)" := TempSalesLine."Amount (LCY) (Before)";
        "Amt. Incl. VAT (LCY) (Before)" := TempSalesLine."Amt. Incl. VAT (LCY) (Before)";
    end;

    [Scope('OnPrem')]
    procedure UpdateCorrLineAmounts(var SalesLine: Record "Sales Line")
    var
        CorrDocSign: Integer;
    begin
        if (not SalesHeader."Corrective Document") or (Type = Type::" ") then
    ...
    [Scope('OnPrem')]
    procedure GetCorrDocSign(SalesLine: Record "Sales Line"): Integer
    begin
    ...
    local procedure ValidateVATProdPostingGroupRU()
    begin
    ...
    begin
        ...
        "Agreement No." := SalesHeader."Agreement No.";
