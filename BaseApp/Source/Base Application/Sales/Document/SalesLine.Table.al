...
#pragma warning restore AA0470
    ...
    begin
        ...
        if SalesHeader."Document Type" <> SalesHeader."Document Type"::Invoice then begin
            ...
            ShouldCalcPrepmtLineAmount := (not PrePaymentLineAmountEntered) and (not CalculateFullGST("Prepmt. Line Amount"));
                  ...
                  Round(
                    CurrExchRate.ExchangeAmtLCYToFCY(
                      ...
                      Round(CurrExchRate.ExchangeAmtFCYToLCY(
                          SalesHeader."Posting Date", SalesHeader."Currency Code", "Line Amount",
        ...
        if LineIsInvDiscountAmount then begin
            ...
            then begin
                ...
                if not SalesLine2.IsEmpty() then begin
                    ...
                    TotalLineAmountACY := SalesLine2."Amount (ACY)";
                    TotalAmountInclVATACY := SalesLine2."Amount Including VAT (ACY)";
                end;
            end;
                        ...
                        begin
                            ...
                            Round(
                              CurrExchRate.ExchangeAmtLCYToFCY(
                                SalesHeader."Posting Date", GLSetup."Additional Reporting Currency",
                                Round(CurrExchRate.ExchangeAmtFCYToLCY(
                                    SalesHeader."Posting Date", SalesHeader."Currency Code", Amount,
                                    SalesHeader."Currency Factor"), Currency."Amount Rounding Precision"), CurrencyFactor),
                              AddCurrency."Amount Rounding Precision");
                            "VAT Base (ACY)" :=
                              Round("Amount (ACY)" * (1 - SalesHeader."VAT Base Discount %" / 100), Currency."Amount Rounding Precision");
                            "Amount Including VAT (ACY)" :=
                              TotalLineAmountACY + "Amount (ACY)" +
                              Round(
                                (TotalLineAmountACY + "Amount (ACY)") * (1 - SalesHeader."VAT Base Discount %" / 100) * "VAT %" / 100,
                                Currency."Amount Rounding Precision", Currency.VATRoundingDirection()) -
        ...
        if SalesLine.FindSet() then
            repeat
                if not SalesLine.ZeroAmountLine(QtyType) then begin
                    ...
                    VATAmountLine.Get(SalesLine."VAT Identifier", SalesLine."VAT Calculation Type", SalesLine."Tax Group Code", false, SalesLine."Line Amount" >= 0, FullGST);
                    if VATAmountLine.Modified then begin
                        ...
                        if QtyType = QtyType::General then begin
                                        ...
                                        VATAmountACY :=
                                          ...
                                          Round(
                                            CurrExchRate.ExchangeAmtLCYToFCY(
                                              ...
                                              Round(
                                                CurrExchRate.ExchangeAmtFCYToLCY(
                                                  GetDate(), SalesHeader."Currency Code", VATAmount,
                                                  SalesHeader."Currency Factor"), Currency."Amount Rounding Precision"), CurrencyFactor),
                                            AddCurrency."Amount Rounding Precision");
                                    NewAmount := 0;
                                    NewAmountACY := 0;
                            ...
                            if LineAmountToInvoice = 0 then begin
                                ...
                                SalesLine."VAT Difference (ACY)" := 0;
                            end else begin
                    ...
                    case QtyType of
                        QtyType::General:
                            begin
                                ...
                                VATAmountLine."VAT Difference (ACY)" += SalesLine."VAT Difference (ACY)";
                    ...
                    case VATAmountLine."VAT Calculation Type" of
                        ...
                        VATAmountLine."VAT Calculation Type"::"Reverse Charge VAT":
                            begin
                                ...
                                VATAmountLine."VAT Base" :=
                                  Round(
                                    VATAmountLine."VAT Base" * (1 - SalesHeader."VAT Base Discount %" / 100),
                                    Currency."Amount Rounding Precision", Currency.VATRoundingDirection());
                                ...
                                VATAmountLine."VAT Base (ACY)" :=
                                  Round(
                                    VATAmountLine."Amount (ACY)" * (1 - SalesHeader."VAT Base Discount %" / 100),
                                    Currency."Amount Rounding Precision", Currency.VATRoundingDirection());
                                VATAmountLine."VAT Amount (ACY)" :=
                                  VATAmountLine."VAT Difference (ACY)" +
                                  Round(
                                    PrevVatAmountLine."VAT Amount (ACY)" +
                                    VATAmountLine."VAT Base (ACY)" * VATAmountLine."VAT %" / 100 * (1 - SalesHeader."VAT Base Discount %" / 100),
                                    Currency."Amount Rounding Precision", Currency.VATRoundingDirection());
                                VATAmountLine."Amount Including VAT" := VATAmountLine."Line Amount" - VATAmountLine."Invoice Discount Amount" + VATAmountLine."VAT Amount";
                                        ...
                                        PrevVatAmountLine."VAT Amount (ACY)" :=
                                              Round(
                                                CurrExchRate.ExchangeAmtLCYToFCY(
                                                  ...
                                                  Round(CurrExchRate.ExchangeAmtFCYToLCY(
                                                      UseDate, SalesHeader."Currency Code", PrevVatAmountLine."VAT Amount",
                                                      SalesHeader."Currency Factor"), Currency."Amount Rounding Precision"), CurrencyFactor),
                                                AddCurrency."Amount Rounding Precision");
                                    end;
                                    if VATAmountLine."Full GST on Prepayment" then
...
"Document Type", Type, "No.", "Variant Code", "Drop Shipment", "Location Code", "Shipment Date");
    ...
    begin
        ...
        RecSalesLine.SetFilter("VAT Identifier", '%1', VATAmountLine."VAT Identifier");
        if RecSalesLine.FindSet() then begin
            repeat
                if PriceIncludingVAT then begin
                    ...
                    VATAmt :=
                      VATAmountLine."VAT Difference" +
                      (RecSalesLine."Line Amount" - RecSalesLine."Inv. Discount Amount" - VATBase - VATAmountLine."VAT Difference") *
                      (1 - SalesHeader."VAT Base Discount %" / 100);
                    VATAmountLine."VAT Base" := VATAmountLine."VAT Base" + VATBase * (RecSalesLine."Qty. to Invoice" / RecSalesLine.Quantity);
                    VATAmountLine."VAT Amount" := VATAmountLine."VAT Amount" + VATAmt * (RecSalesLine."Qty. to Invoice" / RecSalesLine.Quantity);
                    VATAmountLine."Invoice Discount Amount" := VATAmountLine."Invoice Discount Amount" + ("Inv. Discount Amount" * RecSalesLine."Prepayment %" / 100);
                end else begin
                    VATBase :=
                      Round(
                        (RecSalesLine."Line Amount" - RecSalesLine."Inv. Discount Amount") *
                    ...
                    VATAmountLine."VAT Amount" := VATAmountLine."VAT Amount" + (VATBase * RecSalesLine."Prepayment VAT %" / 100);
                    VATAmountLine."Invoice Discount Amount" :=
                      VATAmountLine."Invoice Discount Amount" + RecSalesLine."Inv. Discount Amount" * RecSalesLine."Prepayment %" / 100;
                end;
            ...
            if PriceIncludingVAT then
                ...
                if not VATAmountLine.Positive then
                    VATAmountLine."Amount Including VAT" := VATAmountLine."Line Amount" - SalesLine."Inv. Discount Amount" + VATAmountLine."VAT Amount"
                else
                    VATAmountLine."Amount Including VAT" := VATAmountLine."VAT Base" - SalesLine."Inv. Discount Amount" + VATAmountLine."VAT Amount";
        end;
    end;
    ...
    procedure UpdateACYAmounts(SalesHeaderToCalc: Record "Sales Header")
    var
        AddCurrency: Record Currency;
        CurrencyFactor: Decimal;
    begin
        GetGLSetup();
        SalesHeader := SalesHeaderToCalc;
        CurrencyFactor := GetCurrencyFactorACY(AddCurrency);
        "VAT Base (ACY)" :=
          Round(
            CurrExchRate.ExchangeAmtLCYToFCY(
              SalesHeader."Posting Date", GLSetup."Additional Reporting Currency",
              Round(CurrExchRate.ExchangeAmtFCYToLCY(
                  SalesHeader."Posting Date", "Currency Code", "VAT Base Amount",
                  SalesHeader."Currency Factor"), Currency."Amount Rounding Precision"), CurrencyFactor),
            AddCurrency."Amount Rounding Precision");
        "Amount Including VAT (ACY)" :=
    ...
    local procedure GetCurrencyFactorACY(var AddCurrency: Record Currency): Decimal
    begin
        GetGLSetup();
