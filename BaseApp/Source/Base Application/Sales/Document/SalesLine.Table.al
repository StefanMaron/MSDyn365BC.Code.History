                    ...
                    "VAT Calculation Type"::"No Taxable VAT":
                        begin
                            Amount :=
                              Round(
                                "Amount Including VAT" /
                                (1 + (1 - GetVatBaseDiscountPct(SalesHeader) / 100) * ("VAT %" + "EC %") / 100),
                    ...
                    "VAT Calculation Type"::"No Taxable VAT":
                        begin
                            ...
                            "Amount Including VAT" :=
                              ...
                              Round(
                                (TotalAmount + Amount) * (GetVatBaseDiscountPct(SalesHeader) / 100) * ("VAT %" + "EC %") / 100,
                    ...
                    "VAT Calculation Type"::"No Taxable VAT":
                        begin
                            ...
                            "Amount Including VAT" :=
                              TotalAmount + Amount +
                              Round(
                                (TotalAmount + Amount) * (1 - GetVatBaseDiscountPct(SalesHeader) / 100) * ("VAT %" + "EC %") / 100,
                                Currency."Amount Rounding Precision", Currency.VATRoundingDirection()) -
                    ...
                    if VATAmountLine.Modified then begin
                        ...
                        if QtyType = QtyType::General then begin
                            if SalesHeader."Prices Including VAT" then begin
                                if (VATAmountLine.CalcLineAmount() = 0) or (SalesLine."Line Amount" = 0) then begin
                                    ...
                                    VATAmount :=
                                      ...
                                      VATAmountLine."EC Amount" * SalesLine.CalcLineAmount() / VATAmountLine.CalcLineAmount();
                                ...
                                if SalesLine."VAT Calculation Type" = SalesLine."VAT Calculation Type"::"Full VAT" then begin
                                    VATAmount := SalesLine.CalcLineAmount() - SalesLine."Pmt. Discount Amount";
                                    NewAmount := 0;
                                    NewVATBaseAmount := 0;
                                end else begin
                                    NewAmount := SalesLine.CalcLineAmount() - SalesLine."Pmt. Discount Amount";
                                    NewVATBaseAmount :=
                                      Round(
                                        NewAmount * (1 - SalesLine.GetVatBaseDiscountPct(SalesHeader) / 100), Currency."Amount Rounding Precision");
                            ...
                            if LineAmountToInvoice = 0 then begin
                                ...
                                SalesLine."EC Difference" := 0;
                            end else begin
                                SalesLine."VAT Difference" := Round(VATDifference, Currency."Amount Rounding Precision");
                                SalesLine."EC Difference" := Round(ECDifference, Currency."Amount Rounding Precision");
                            end;
                        end;
                        ...
                        TempVATAmountLineRemainder."EC Difference" := ECDifference - SalesLine."EC Difference";
                ...
                if not SalesLine.ZeroAmountLine(QtyType) then begin
                    ...
                    then
                        VATAmountLine.InsertNewLine(
                          SalesLine."VAT Identifier", SalesLine."VAT Calculation Type", SalesLine."Tax Group Code", false, SalesLine."VAT %", SalesLine."Line Amount" >= 0, false, SalesLine."EC %", 0);
                    ...
                    case QtyType of
                        QtyType::General:
                            begin
                                ...
                                VATAmountLine.SumLine(SalesLine."Line Amount", SalesLine."Inv. Discount Amount", SalesLine."Pmt. Discount Amount", SalesLine."VAT Difference", SalesLine."EC Difference", SalesLine."Allow Invoice Disc.", SalesLine."Prepayment Line");
                            end;
                        QtyType::Invoicing:
                            begin
                                IsHandled := false;
                                ...
                                if SalesHeader."Invoice Discount Calculation" <> SalesHeader."Invoice Discount Calculation"::Amount then
                                    VATAmountLine.SumLine(
                                      ...
                                      Round(SalesLine."Pmt. Discount Amount" * QtyToHandle / SalesLine.Quantity, Currency."Amount Rounding Precision"),
                                      SalesLine."VAT Difference", SalesLine."EC Difference", SalesLine."Allow Invoice Disc.", SalesLine."Prepayment Line")
                                else
                                    VATAmountLine.SumLine(
                                      AmtToHandle, SalesLine."Inv. Disc. Amount to Invoice",
                                      Round(SalesLine."Pmt. Discount Amount" * QtyToHandle / SalesLine.Quantity, Currency."Amount Rounding Precision"),
                                      SalesLine."VAT Difference", SalesLine."EC Difference", SalesLine."Allow Invoice Disc.", SalesLine."Prepayment Line");
                            end;
                        QtyType::Shipping:
...
"Document Type", Type, "No.", "Variant Code", "Drop Shipment", "Location Code", "Shipment Date");
    ...
    begin
        SalesLine.SetLoadFields(
            ...
            "Unit Price", "Amount Including VAT", Amount, "Line Amount", "Inv. Discount Amount", "Inv. Disc. Amount to Invoice", "VAT Difference", "VAT Base Amount", "EC Difference", "Pmt. Discount Amount",
            ...
            "System-Created Entry", "VAT Identifier", "VAT Calculation Type", "Tax Group Code", "VAT %", "Allow Invoice Disc.", "Prepayment Line", "Completely Shipped", Planned, "EC %");
    end;
