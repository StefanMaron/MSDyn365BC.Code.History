    ...
    fields
    {
        ...
        field(10; "Shipment Date"; Date)
        {
            ...
            begin
                ...
                if CurrFieldNo in [FieldNo("Shipment Date"), FieldNo("Planned Shipment Date")] then
        ...
        field(15; Quantity; Decimal)
        {
            ...
            begin
                ...
                if not HasTypeToFillMandatoryFields() and (Quantity <> 0) then
                    Error(Text90804, FieldCaption(Type), Type);
...
#pragma warning restore
        ...
        field(3010803; "Quote Variant"; Option)
        {
            ...
            begin
                ...
                if Type in [Type::Item, Type::Resource, Type::"Fixed Asset", Type::"Charge (Item)"] then begin
                    if Item.Get("No.") then;
    ...
    begin
        ...
        if CheckQuoteMgtPermission() or (CopyStr(SerialNumber, 7, 3) = '000') then begin
            if xRec.Classification <> '' then begin
                if xRec.Type = xRec.Type::"End-Total" then
                    Level := xRec.Level + 1
                else
                    Level := xRec.Level;
                Classification := xRec.Classification;
                "Title No." := xRec."Title No.";
            end else begin
                SalesLine2.Copy(xRec);
                if SalesLine2.Next(-1) <> 0 then begin
                    Level := SalesLine2.Level;
                    Classification := SalesLine2.Classification;
                    "Title No." := SalesLine2."Title No.";
                end;
            end;
            CalcPosition(SalesLineBuffer.Position);
        end;
    ...
    begin
        ...
        if CheckQuoteMgtPermission() or (CopyStr(SerialNumber, 7, 3) = '000') then
            CalcPosition(SalesLineBuffer.Position);
    end;
    ...
    var
        ...
        SalesLineBuffer: Record "Sales Line";
        ...
        Text90801: Label 'You cannot insert further lines on this position. If the %1 has no entries yet, you can reorganize the lines with Function, Re-Calculate.';
        Text90802: Label 'Otherwise, you can insert after the last line.';
        Text90803: Label 'Line type %1 can only be used with the module quote management.';
        Text90804: Label '%1 must not be %2.';
        TempVATDiff: Decimal;
    ...
    begin
                ...
                if not SalesLine.ZeroAmountLine(QtyType) then begin
                    ...
                    if VATAmountLine.Modified then begin
                        ...
                        if SalesSetup."Apply Inv. Round. Amt. To VAT" then begin
                            if (SalesLine."Amount Including VAT" - SalesLine.Amount) <> VATAmount then
                                SalesLine.Amount := SalesLine.Amount + TempVATDiff;
                            TempVATDiff := 0;
                        end;
                        OnUpdateVATOnLinesOnBeforeModifySalesLine(SalesLine, VATAmount, TempVATDiff);
                        SalesLine.Modify();
        ...
        SalesLine.SetRange("Quote Variant");
        ...
        if SalesSetup."Apply Inv. Round. Amt. To VAT" then begin
            if VATAmountLine.FindSet() then begin
                ...
                if VATAmountLine.Find('-') then begin
                    TempVATDiff :=
                      Round(
                        TmpTotalInclVAT, Currency."Invoice Rounding Precision", Currency.InvoiceRoundingDirection()) - TmpTotalInclVAT;
                    VATAmountLine."VAT Amount" += TempVATDiff;
                    VATAmountLine."Amount Including VAT" += TempVATDiff;
                    VATAmountLine.Modify();
                end;
                VATAmountLine.SetRange("VAT %");
                VATAmountLine.SetRange("VAT Amount");
            end;
        end else
            ShouldProcessRounding := RoundingLineInserted and (TotalVATAmount <> 0);
        OnCalcVATAmountLinesOnAfterCalcShouldProcessRounding(VATAmountLine, Currency, TempVATDiff, ShouldProcessRounding, SalesLine, TotalVATAmount);
...
"Document Type", Type, "No.", "Variant Code", "Drop Shipment", "Location Code", "Shipment Date");
    ...
    begin
        SalesLine.SetLoadFields(
            "Document Type", "Document No.", Type, "No.", "Shipment No.", "Return Receipt No.", "Deferral Code", "Quote Variant",
    ...
    [IntegrationEvent(false, false)]
    local procedure OnInsertOnAfterCheckInventoryConflict(var SalesLine: Record "Sales Line"; xSalesLine: Record "Sales Line"; var SalesLine2: Record "Sales Line"; SalesLineBuffer: Record "Sales Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnUpdateVATOnLinesOnBeforeModifySalesLine(var SalesLine: Record "Sales Line"; VATAmount: Decimal; var TempVATDiff: Decimal)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCalcVATAmountLinesOnAfterCalcShouldProcessRounding(var VATAmountLine: Record "VAT Amount Line"; Currency: Record Currency; var TempVATDiff: Decimal; var IsHandled: Boolean; var SalesLine: Record "Sales Line"; var TotalVATAmount: Decimal)
    begin
    end;
