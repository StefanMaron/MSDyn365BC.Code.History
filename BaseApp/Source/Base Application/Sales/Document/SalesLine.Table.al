...
#pragma warning restore
    ...
    begin
        ...
        LineIsInvDiscountAmount :=
           ("Line Amount" = "Inv. Discount Amount") and
            not SalesTaxCalculate.HasExciseTax("Tax Area Code", "Tax Group Code", "Tax Liable", Quantity, SalesHeader."Document Date");
        ...
        if SalesLine.FindSet() then
            repeat
                if not SalesLine.ZeroAmountLine(QtyType) then begin
                    ...
                    VATAmountLine.Get(SalesLine."VAT Identifier", SalesLine."VAT Calculation Type", SalesLine."Tax Group Code", SalesLine."Tax Area Code", false, SalesLine."Line Amount" >= 0);
                    if VATAmountLine.Modified then begin
                        if not TempVATAmountLineRemainder.Get(
                             SalesLine."VAT Identifier", SalesLine."VAT Calculation Type", SalesLine."Tax Group Code", SalesLine."Tax Area Code", false, SalesLine."Line Amount" >= 0)
                        then begin
                ...
                if not SalesLine.ZeroAmountLine(QtyType) then begin
                    ...
                    then
                        VATAmountLine.InsertNewLine(
                          SalesLine."VAT Identifier", SalesLine."VAT Calculation Type", SalesLine."Tax Group Code", SalesLine."Tax Area Code", false, SalesLine."VAT %", SalesLine."Line Amount" >= 0, false, 0);
