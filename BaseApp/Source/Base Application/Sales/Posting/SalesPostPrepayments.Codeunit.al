    ...
    begin
        ...
        if not IsHandled then begin
            ...
            if SalesSetup."Apply Inv. Round. Amt. To VAT" then begin
                TempVATAmountLine.SetFilter("VAT %", '>0');
                TempVATAmountLine.SetFilter("VAT Amount", '>0');
                TempVATAmountLine.SetFilter("VAT Difference", '<>0');
                if TempVATAmountLine.FindFirst() then begin
                    TempPrepmtInvLineBuffer.SetRange("VAT Identifier", TempVATAmountLine."VAT Identifier");
                    TempPrepmtInvLineBuffer.SetRange("VAT Calculation Type", TempVATAmountLine."VAT Calculation Type");
                    TempPrepmtInvLineBuffer.SetRange("Tax Group Code", TempVATAmountLine."Tax Group Code");
                    if TempPrepmtInvLineBuffer.FindFirst() then begin
                        TempPrepmtInvLineBuffer."VAT Difference" := TempVATAmountLine."VAT Difference";
                        TempPrepmtInvLineBuffer.Modify();
                    end;
                    TempPrepmtInvLineBuffer.SetRange("VAT Identifier");
                    TempPrepmtInvLineBuffer.SetRange("VAT Calculation Type");
                    TempPrepmtInvLineBuffer.SetRange("Tax Group Code");
                end;
                TempVATAmountLine.SetRange("VAT %");
                TempVATAmountLine.SetRange("VAT Amount");
                TempVATAmountLine.SetRange("VAT Difference");
            end;
        end;
    ...
    begin
        ...
        if SalesSetup."Apply Inv. Round. Amt. To VAT" then
            if VATAmountLine.FindFirst() then begin
                ...
                if VATAmountLine.FindFirst() then begin
                    VATAmountLine."VAT Difference" := VATAmountLine."VAT Difference" +
                      Round(
                        TmpTotalInclVAT, Currency."Invoice Rounding Precision", Currency.InvoiceRoundingDirection()) - TmpTotalInclVAT;
                    VATAmountLine."VAT Amount" := VATAmountLine."Calculated VAT Amount" + VATAmountLine."VAT Difference";
                    VATAmountLine."Amount Including VAT" :=
                      VATAmountLine."Amount Including VAT" + VATAmountLine."VAT Difference";
                    VATAmountLine.Modify();
                end;
