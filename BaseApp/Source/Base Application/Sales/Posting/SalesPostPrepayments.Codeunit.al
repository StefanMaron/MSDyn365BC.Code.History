...
codeunit 442 "Sales-Post Prepayments"
{
    ...
    var
        ...
        Text1130004: Label 'To specify the installment to apply to please select the %1 or use the function Apply Entries.', Comment = '%1 = Applies-to Occurrence No.';
        Text1130007: Label 'must not be prior to %1', Comment = '%1 = Date';
    ...
    begin
        ...
        if DocumentType <> DocumentType::"Credit Memo" then
            InsertPostedPaymentLines(SalesHeader, DocumentType, SalesInvHeader."No.")
        else
            InsertPostedPaymentLines(SalesHeader, DocumentType, SalesCrMemoHeader."No.");

        InsertPaymentJnlLine(SalesHeader);
    ...
    begin
        ...
        SalesHeader.TestField("Operation Occurred Date");
        SalesHeader.TestField("Operation Type");
        GLSetup.GetRecordOnce();
        ...
        if SalesHeader."Applies-to Doc. No." <> '' then
            if SalesHeader."Applies-to Occurrence No." = 0 then
                Error(Text1130004, SalesHeader.FieldCaption("Applies-to Occurrence No."));
        ...
        case DocumentType of
            ...
            DocumentType::"Credit Memo":
                begin
                    ...
                    CheckPrepmtReferstoPeriod(SalesHeader);
    ...
    begin
        ...
        PrepmtInvLineBuf."G/L Account No." := GetPrepmtAccNo(SalesLine."VAT Bus. Posting Group", SalesLine."VAT Prod. Posting Group");
        ...
        PrepmtInvLineBuf."Gen. Bus. Posting Group" := SalesLine."Gen. Bus. Posting Group";
        PrepmtInvLineBuf."VAT Bus. Posting Group" := SalesLine."VAT Bus. Posting Group";
        ...
        if VATPostingSetup."EU Service" then begin
            PrepmtInvLineBuf."Service Tariff No." := SalesLine."Service Tariff No.";
            PrepmtInvLineBuf."Transport Method" := SalesLine."Transport Method";
        end;
        PrepmtInvLineBuf."Prepmt. CM Refers to Period" := SalesLine."Prepmt. CM Refers to Period";
        ...
        if FromSalesLine.Find('-') then begin
            ...
            if SalesSetup."Invoice Rounding" then begin
                ...
                if InitInvoiceRoundingLine(SalesHeader, TotalAmt, InvRoundingSalesLine) then begin
                    ...
                    ToSalesLine."Prepayment Deductible %" := ToSalesLine."Deductible %";
    ...
    begin
        ...
        GenJnlLine.Validate(GenJnlLine."Document Date", GenJnlLine."Document Date");
    ...
    [Scope('OnPrem')]
    procedure CheckPrepmtReferstoPeriod(var SalesHeader: Record "Sales Header")
    var
        SalesLine: Record "Sales Line";
    begin
        SalesLine.Reset();
        ...
        if SalesLine.FindSet() then
            repeat
                SalesLine.TestField("Prepmt. CM Refers to Period");
            until SalesLine.Next() = 0;
    end;
    ...
    begin
        ...
        SalesInvLine."Deductible %" := PrepmtInvLineBuffer."Deductible %";
    ...
    begin
        ...
        SalesCrMemoLine."Deductible %" := PrepmtInvLineBuffer."Deductible %";
        ...
        SalesCrMemoLine."Prepmt. CM Refers to Period" := PrepmtInvLineBuffer."Prepmt. CM Refers to Period";
    ...
    local procedure InsertPostedPaymentLines(SalesHeader: Record "Sales Header"; DocumentType: Option Invoice,"Credit Memo"; DocumentNo: Code[20])
    var
    ...
    begin
        ...
        PostedPaymentLines."Sales/Purchase" := PostedPaymentLines."Sales/Purchase"::Sales;
        PostedPaymentLines."Line No." := 10000;
        PostedPaymentLines."Payment %" := 100;
        PostedPaymentLines."Due Date" := SalesHeader."Prepayment Due Date";
        PostedPaymentLines."Discount %" := SalesHeader."Prepmt. Payment Discount %";
        PostedPaymentLines."Pmt. Discount Date" := SalesHeader."Prepmt. Pmt. Discount Date";
        PostedPaymentLines.Insert();
    end;

    local procedure InsertPaymentJnlLine(SalesHeader: Record "Sales Header")
    var
    ...
    begin
        ...
        PaymentLines."Due Date" := SalesHeader."Prepayment Due Date";
        PaymentLines."Discount %" := SalesHeader."Prepmt. Payment Discount %";
        PaymentLines."Pmt. Discount Date" := SalesHeader."Prepmt. Pmt. Discount Date";
        PaymentLines.Insert();
    end;
