...
codeunit 46 SelectionFilterManagement
{
    ...
    procedure GetSelectionFilterForBankAccount(var BankAccount: Record "Bank Account"): Text
    var
        RecRef: RecordRef;
    begin
        RecRef.GetTable(BankAccount);
        exit(GetSelectionFilter(RecRef, BankAccount.FieldNo("No.")));
    end;

    procedure GetSelectionFilterForEBPaymentJournal(var EBPaymentJournal: Record "Payment Journal Line"): Text
    var
        RecRef: RecordRef;
    begin
        RecRef.GetTable(EBPaymentJournal);
        exit(GetSelectionFilter(RecRef, EBPaymentJournal.FieldNo("Line No.")));
    end;
