...
codeunit 2004 "Machine Learning KeyVaultMgmt."
{
    ...
    begin
        // If MLStudioAppUrlKVNameLbl is set in KV, use new function to get access token.
        if AzureKeyVault.GetAzureKeyVaultSecret(MLStudioAppUrlKVNameLbl, Value) then begin
            GetMachineLearningCredentials(ApiUri, ApiKeySecret, LimitType, Limit);
            ApiKey := CopyStr(ApiKeySecret, 1, MaxStrLen(ApiKey));
            exit;
        end;
    ...
    [NonDebuggable]
    [Scope('OnPrem')]
    procedure GetMachineLearningCredentials(var ApiUrl: Text[250]; var ApiAccessToken: Text; var LimitType: Option; var Limit: Decimal): Boolean
    var
    ...
    [NonDebuggable]
    local procedure GetCertificate(CertificateKVName: Text; var Certificate: Text): Boolean
    var
        AzureKeyVault: Codeunit "Azure Key Vault";
