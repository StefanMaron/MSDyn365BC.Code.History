codeunit 57 "Document Totals"
{
    ...
    local procedure CalculateTotalPurchaseLineAndVATAmount(PurchaseHeader: Record "Purchase Header"; var VATAmount: Decimal; var TempTotalPurchaseLine: Record "Purchase Line" temporary; var TempPurchaseLine: Record "Purchase Line" temporary)
    var
    ...
    begin
        ...
        if PurchaseHeader.Get(TempCurrentPurchaseLine."Document Type", TempCurrentPurchaseLine."Document No.") then begin
            CalculateTotalPurchaseLineAndVATAmount(PurchaseHeader, VATAmount, TempTotalPurchaseLine, TempPurchaseLine);
    ...
    begin
        ...
        if PurchaseLine."VAT Calculation Type" = PurchaseLine."VAT Calculation Type"::"Reverse Charge VAT" then
            TotalPurchaseLine."Amount Including VAT" += PurchaseLine.Amount
        else
            TotalPurchaseLine."Amount Including VAT" += PurchaseLine."Amount Including VAT";
        if xPurchaseLine."VAT Calculation Type" = xPurchaseLine."VAT Calculation Type"::"Reverse Charge VAT" then
    ...
    begin
        ...
        PurchaseLineWithReverseChargeVAT.SetRange("Document Type", TotalPurchaseHeader."Document Type");
        PurchaseLineWithReverseChargeVAT.SetRange("Document No.", TotalPurchaseHeader."No.");
        PurchaseLineWithReverseChargeVAT.SetRange(
          "VAT Calculation Type", PurchaseLineWithReverseChargeVAT."VAT Calculation Type"::"Reverse Charge VAT");
        PurchaseLineWithReverseChargeVAT.CalcSums(Amount, "Amount Including VAT");
        VATAmountOfLinesWithRevChargeVAT :=
          PurchaseLineWithReverseChargeVAT."Amount Including VAT" - PurchaseLineWithReverseChargeVAT.Amount;
        VATAmount -= VATAmountOfLinesWithRevChargeVAT;
        TotalPurchaseLine2."Amount Including VAT" -= VATAmountOfLinesWithRevChargeVAT;

        PurchaseLine2.CopyFilters(TotalPurchaseLine);
        PurchaseLine2.SetRange("Document Type", TotalPurchaseHeader."Document Type");
    ...
    [IntegrationEvent(false, false)]
    local procedure OnPurchaseCalculateTotalsWithInvoiceRoundingOnAfterCalculateTotalPurchaseLineAndVATAmount(var TempPurchaseLine: Record "Purchase Line" temporary; var TempTotalPurchaseLine: Record "Purchase Line" temporary; var VATAmount: Decimal)
    begin
    end;
