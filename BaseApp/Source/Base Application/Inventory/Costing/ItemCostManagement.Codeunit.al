...
codeunit 5804 ItemCostManagement
{
    ...
    [Scope('OnPrem')]
    procedure CalcAvgUnitActualCost(ItemNo: Code[20]; LocationCode: Code[10]; PostingDate: Date): Decimal
    var
        Item: Record Item;
        ItemCostManagement: Codeunit ItemCostManagement;
        FullActualCost: Decimal;
        FullQty: Decimal;
    begin
        Item.Get(ItemNo);
        Item.SetFilter("Date Filter", '<%1', PostingDate);
        Item.SetRange("Location Filter", LocationCode);

        FullActualCost := CalculateCostAmt(Item, true);
        if FullActualCost = 0 then
            exit(0);

        FullQty := CalculateQuantity(Item);
        if FullQty <> 0 then
