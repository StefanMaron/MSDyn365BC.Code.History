...
codeunit 5037 "P.Inv. Rcpt. PDF Doc.Handler" implements IPdfDocumentHandler
{
    var
        PostedInventoryReceiptLbl: Label 'Posted Inventory Receipt';

    /// <summary>
    /// Generates a PDF blob for Posted Inventory Receipt
    /// </summary>
    ...
    begin
        if not InvtReceiptHeader.GetBySystemId(DocumentId) then
            exit(false);

        InvtReceiptHeader.SetRange("No.", InvtReceiptHeader."No.");
        ReportUsage := "Report Selection Usage"::"P.Inventory Receipt";
        ReportSelections.GetPdfReportForTable(TempBlob, ReportUsage, InvtReceiptHeader, Database::"Invt. Receipt Header");
        DocumentMailing.GetAttachmentFileName(Name, InvtReceiptHeader."No.", PostedInventoryReceiptLbl, ReportUsage.AsInteger());
