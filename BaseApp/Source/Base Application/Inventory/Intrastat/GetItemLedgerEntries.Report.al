...
report 594 "Get Item Ledger Entries"
{
    ...
    begin
        ...
        if TotalIndirectCost <> 0 then
            IntrastatJnlLine.Validate("Indirect Cost", Round(Abs(TotalIndirectCost), 1))
        else
    ...
    begin
                ...
                if not IntrastatJnlBatch."Amounts in Add. Currency" then begin
                    if ValueEntry."Item Charge No." = '' then begin
                        ...
                        if IncludeItemCharge then begin
                            ...
                            TotalIndirectCost += ValueEntry."Cost Amount (Actual)";
                            TotalIndirectCostExpected += ValueEntry."Sales Amount (Expected)";
                            TotalIndirectCostExpected += ValueEntry."Cost Amount (Expected)";
                        end;
                end else begin
                    if ValueEntry."Item Charge No." = '' then begin
                        ...
                        if IncludeItemCharge then begin
                            TotalIndirectCost += ValueEntry."Cost Amount (Actual) (ACY)";
                            TotalIndirectCostExpected += ValueEntry."Cost Amount (Expected) (ACY)";
                        end;
        ...
        if (TotalAmt = 0) and (ItemLedgerEntry."Entry Type" = ItemLedgerEntry."Entry Type"::Sale) then begin
    ...
    [Scope('OnPrem')]
    procedure IsJobService(JobLedgEntry: Record "Job Ledger Entry"): Boolean
    var
        Job: Record Job;
