...
report 594 "Get Item Ledger Entries"
{
    ...
    dataset
    {
        ...
        {
            DataItemTableView = sorting("Intrastat Code") where("Intrastat Code" = filter(<> ''), "EU Country/Region Code" = filter(<> ''));
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field("Cost Regulation %"; IndirectCostPctReq)
                    {
    ...
    begin
        ...
        IntrastatJnlLine.Amount := Abs(TotalAmt);
        if AmountInclItemCharges then
            IntrastatJnlLine.Amount := Abs(TotalAmt + TotalIndirectCost)
        else begin
            IntrastatJnlLine.Amount := Abs(TotalAmt);
            IntrastatJnlLine."Indirect Cost" := Abs(TotalAmt + TotalIndirectCost) - Abs(TotalAmt);
        end;
        ...
        if AmountInclItemCharges then
            ...
            IntrastatJnlLine.Validate("Indirect Cost");
