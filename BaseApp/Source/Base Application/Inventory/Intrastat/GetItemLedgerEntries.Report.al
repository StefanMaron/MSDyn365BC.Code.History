...
report 594 "Get Item Ledger Entries"
{
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field(GroupEntries; GroupEntries)
                    {
                        ...
                        ToolTip = 'Specifies if you want to group entries with the same Tariff No, Country/Region Code, Item No, Cust.VAT Registration No. This feature substract items returns from shipments and is required in the French Intrastat reporting.';
                    }
                }
                group(Additional)
                {
    ...
    begin
        ...
        if (IntrastatJnlLine."Partner VAT ID" = '') and
