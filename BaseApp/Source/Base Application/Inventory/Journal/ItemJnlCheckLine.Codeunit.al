...
#pragma warning restore AA0074
    ...
    begin
        ...
        if (InvtSetup."Unit of Measure Mandatory") and
          (ItemJournalLine."Value Entry Type" = ItemJournalLine."Value Entry Type"::"Direct Cost") and
          (ItemJournalLine."Item Charge No." = '') and (ItemJournalLine.Quantity <> 0)
        then
            ItemJournalLine.TestField("Unit of Measure Code", ErrorInfo.Create());

        if ItemJournalLine."FA No." <> '' then
            ItemJournalLine.TestField("Depreciation Book Code", ErrorInfo.Create())
        else
            ItemJournalLine.TestField("Depreciation Book Code", '', ErrorInfo.Create());
    ...
    begin
        ...
        if ItemJnlLine."Entry Type" = ItemJnlLine."Entry Type"::"Negative Adjmt." then begin
            ...
            ItemJnlLine.TestField("Applies-to Entry", 0);
            CheckGenBusPostingGroup(ItemJnlLine, ItemJnlLine."Applies-from Entry");
        end;

        if ItemJnlLine."Entry Type" = ItemJnlLine."Entry Type"::"Positive Adjmt." then begin
            ItemJnlLine.TestField("Applies-to Entry");
            ItemJnlLine.TestField("Applies-from Entry", 0);
            CheckGenBusPostingGroup(ItemJnlLine, ItemJnlLine."Applies-to Entry");
        end;

        if ItemJnlLine."Document Type" = ItemJnlLine."Document Type"::"Sales Return Receipt" then
            ItemJnlLine.TestField("Applies-from Entry", ErrorInfo.Create());

        if (ItemJnlLine."Document Type" = ItemJnlLine."Document Type"::"Purchase Return Shipment") or
          (ItemJnlLine."Entry Type" = ItemJnlLine."Entry Type"::Transfer) then
            ItemJnlLine.TestField("Applies-to Entry", ErrorInfo.Create());

        if (ItemJnlLine."Value Entry Type" = ItemJnlLine."Value Entry Type"::Revaluation) and
          (ItemJnlLine."Unit Cost" > 0) then
            ItemJnlLine.TestField("Red Storno", false, ErrorInfo.Create());
    end;
...
#pragma warning restore AA0074
    begin
        ...
        if (ItemJnlLine."Document Type" = ItemJnlLine."Document Type"::"Inventory Shipment") or
           (ItemJnlLine."Document Type" = ItemJnlLine."Document Type"::"Inventory Receipt")
        then begin
            //FA check
            if (ItemJnlLine."FA No." <> '') or (LocItemLedgerEntry."FA No." <> '') then begin
                if ItemJnlLine."FA No." <> LocItemLedgerEntry."FA No." then
                    Error(
                        ErrorInfo.Create(
                            StrSubstNo(
                                Text12401, ItemJnlLine.FieldCaption("FA No."), Format(LocItemLedgerEntry."FA No.")),
                            true));
                if ItemJnlLine."Depreciation Book Code" <> LocItemLedgerEntry."Depreciation Book Code" then
                    Error(
                        ErrorInfo.Create(
                            StrSubstNo(
                                Text12401, ItemJnlLine.FieldCaption("Depreciation Book Code"),
                ...
                LocLatestItemLedgerEntry.SetFilter("Posting Date", '>%1', ItemJnlLine."Posting Date");
                LocLatestItemLedgerEntry.SetRange("FA No.", ItemJnlLine."FA No.");
                if not LocLatestItemLedgerEntry.IsEmpty() then
                    Error(
                        ErrorInfo.Create(
                            StrSubstNo(
                                Text12402, ItemJnlLine.FieldCaption("FA No."), Format(ItemJnlLine."FA No."),
                                ItemJnlLine.FieldCaption("Depreciation Book Code"), Format(ItemJnlLine."Depreciation Book Code")),
                            true));

                if FADeprBook.Get(ItemJnlLine."FA No.", ItemJnlLine."Depreciation Book Code") then begin
                    FADeprBook.CalcFields("Book Value");
                    if FADeprBook."Book Value" < -ItemJnlLine.Amount then
                        Error(
                            ErrorInfo.Create(
                                StrSubstNo(
                                    Text12403, FADeprBook.FieldCaption("Book Value"), Format(FADeprBook."Book Value"),
                                    ItemJnlLine.FieldCaption("FA No."), Format(ItemJnlLine."FA No."),
                                    ItemJnlLine.FieldCaption("Depreciation Book Code"), Format(ItemJnlLine."Depreciation Book Code"),
                                    ItemJnlLine.FieldCaption(Amount), Format(ItemJnlLine.Amount),
                                    ItemJnlLine.FieldCaption("Line No."), Format(ItemJnlLine."Line No.")),
                                true));
                end;
            end;
            //end FA check
            LineNo := ItemJnlLine."Document Line No."
        end else
            LineNo := ItemJnlLine."Line No.";
        //Gen Bus/Prod check
        if ValueEntry.Get(FindValueEntry(ItemLedgerEntryNo)) then begin
            if ValueEntry."Gen. Bus. Posting Group" <> ItemJnlLine."Gen. Bus. Posting Group" then
                Error(
                    ErrorInfo.Create(
                        StrSubstNo(
                            Text12400, ItemJnlLine.FieldCaption("Gen. Bus. Posting Group"), Format(LineNo),
                            ItemJnlLine.FieldCaption("Gen. Bus. Posting Group"), LocItemLedgerEntry.TableCaption(), LocItemLedgerEntry."Entry No."),
                        true));

            if ValueEntry."Gen. Prod. Posting Group" <> ItemJnlLine."Gen. Prod. Posting Group" then
                Error(
                    ErrorInfo.Create(
                        StrSubstNo(
                            Text12400, ItemJnlLine.FieldCaption("Gen. Prod. Posting Group"), Format(LineNo),
                            ItemJnlLine.FieldCaption("Gen. Prod. Posting Group"), LocItemLedgerEntry.TableCaption(), LocItemLedgerEntry."Entry No."),
                        true));
        end;
    end;
