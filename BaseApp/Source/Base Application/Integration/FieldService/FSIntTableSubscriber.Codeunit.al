...
codeunit 6405 "FS Int. Table. Subscriber"
{
    ...
    begin
        if not FSConnectionSetup.ReadPermission() then begin
            Session.LogMessage('0000MMX', 'Insufficient permisions to check if Field Service integration is enabled.', Verbosity::Warning, DataClassification::SystemMetadata, TelemetryScope::ExtensionPublisher, 'Category', CategoryTok);
            exit;
        end;
        ...
        if not JobUsageLink.FindFirst() then begin
            Session.LogMessage('0000MN8', 'Unable to find Project Usage Link.', Verbosity::Warning, DataClassification::SystemMetadata, TelemetryScope::ExtensionPublisher, 'Category', CategoryTok);
            exit;
        end;

        if not JobPlanningLine.Get(JobUsageLink."Job No.", JobUsageLink."Job Task No.", JobUsageLink."Line No.") then begin
            Session.LogMessage('0000MN9', 'Unable to find Project Planning Line.', Verbosity::Warning, DataClassification::SystemMetadata, TelemetryScope::ExtensionPublisher, 'Category', CategoryTok);
            exit;
        end;
        ...
        if not CRMIntegrationRecord.ReadPermission() then begin
            Session.LogMessage('0000MMY', 'Insufficient permisions to update link to Field Service entity.', Verbosity::Warning, DataClassification::SystemMetadata, TelemetryScope::ExtensionPublisher, 'Category', CategoryTok);
            exit;
        end;
