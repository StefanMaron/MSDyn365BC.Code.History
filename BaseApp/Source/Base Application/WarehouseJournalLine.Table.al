table 7311 "Warehouse Journal Line"
{
    ...
    fields
    {
        ...
        field(10; Quantity; Decimal)
        {
            ...
            begin
                ...
                ItemTrackingMgt.CheckWhseItemTrkgSetup("Item No.", "Location Code", SNRequired, LNRequired, CDRequired, false);
    ...
    keys
    {
        ...
        key(Key3; "Item No.", "From Bin Code", "Location Code", "Entry Type", "Variant Code", "Unit of Measure Code", "Lot No.", "Serial No.", "CD No.")
        {
            MaintainSIFTIndex = false;
            SumIndexFields = "Qty. (Absolute, Base)", "Qty. (Absolute)", Cubage, Weight;
        }
        key(Key4; "Item No.", "To Bin Code", "Location Code", "Variant Code", "Unit of Measure Code", "Lot No.", "Serial No.", "CD No.")
        {
            MaintainSIFTIndex = false;
    ...
    begin
        if ("Line No." <> 0) and IsReclass("Journal Template Name") then begin
            ...
            SerialNo := '';
            CDNo := '';
            RetrieveItemTracking(LotNo, SerialNo, CDNo);
            BinCode := WMSMgt.BinContentLookUp("Location Code", "Item No.", "Variant Code", "Zone Code", LotNo, SerialNo, CDNo, "Bin Code");
        end else
