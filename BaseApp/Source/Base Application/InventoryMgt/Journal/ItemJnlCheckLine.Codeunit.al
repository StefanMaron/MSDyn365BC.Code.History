...
codeunit 21 "Item Jnl.-Check Line"
{
    ...
    begin
        case ItemJnlLine."Entry Type" of
            ItemJnlLine."Entry Type"::Output:
                begin
                    InvtPutAwayLocation := Location."Prod. Output Whse. Handling" = Enum::"Prod. Output Whse. Handling"::"Inventory Put-away";
                    OnAfterAssignInvtPutAwayRequired(ItemJnlLine, Location, InvtPutAwayLocation);
                    if InvtPutAwayLocation then
                        if ItemJnlLine.Quantity >= 0 then
                            exit(true);

                    InvtPickLocation := Location."Prod. Consump. Whse. Handling" = Enum::"Prod. Consump. Whse. Handling"::"Inventory Pick/Movement";
            ...
            ItemJnlLine."Entry Type"::"Assembly Consumption":
                begin
                    ...
                    InvtPickLocation := Location."Asm. Consump. Whse. Handling" = Enum::"Asm. Consump. Whse. Handling"::"Inventory Movement";
                    OnAfterAssignInvtPickRequired(ItemJnlLine, Location, InvtPickLocation);
                    if InvtPickLocation then
                        if ItemJnlLine.Quantity >= 0 then
                            exit(true);
