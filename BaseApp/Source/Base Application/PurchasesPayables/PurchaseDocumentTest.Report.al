        ...
        dataitem("Purchase Header"; "Purchase Header")
        {
                                ...
                                if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice, "Document Type"::"Credit Memo"] then
                                    if VATPostingSetup.IsEUService("VAT Bus. Posting Group", "VAT Prod. Posting Group") then begin
                                        if "Service Tariff No." = '' then
                                            AddError(
                                              StrSubstNo(
                                                Text006, FieldCaption("Service Tariff No.")));
                                        if "Transport Method" = '' then
                                            AddError(
                                              StrSubstNo(
                                                Text12100, FieldCaption("Transport Method")));
                                        if "Purchase Header"."Payment Method Code" = '' then
                                            AddError(
                                              StrSubstNo(
                                                Text12100, "Purchase Header".FieldCaption("Payment Method Code")));
                                    end;
            ...
            begin
                ...
                if Invoice then begin
                    ...
                    InclInVATReportValidation.ValidatePurchaseHeader("Purchase Header", InclInVATReportErrorLogTemp);
