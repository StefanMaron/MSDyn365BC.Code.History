codeunit 65 "Discount Notification Mgt."
{
    ...
    begin
        ...
        if DiscountPosting in [DiscountPosting::"No Discounts", ExceptDiscountPosting] then
            exit(false);
        if FindSetupMissingDiscountAccount(SetupRecordID.TableNo, DiscountPosting, GenBusPostingGroup, GeneralPostingSetup, FieldNumber) then begin
            SendNotification(SetupRecordID, DiscountPosting, GenBusPostingGroup);
            exit(true);
        end;
    end;

    local procedure FindSetupMissingDiscountAccount(SetupTableNo: Integer; DiscountPosting: Option; GenBusPostingGroup: Code[20]; var GeneralPostingSetup: Record "General Posting Setup"; var FieldNumber: Integer): Boolean
    begin
    ...
    local procedure SendNotification(SetupRecordID: RecordID; DiscountPosting: Integer; GenBusPostingGroup: Code[20])
    var
        NotificationLifecycleMgt: Codeunit "Notification Lifecycle Mgt.";
    ...
    begin
        ...
        GenBusPostingGroup := Notification.GetData('GenBusPostingGroup');
        FindSetupMissingDiscountAccount(SetupTableNo, DiscountPosting, GenBusPostingGroup, GeneralPostingSetup, FieldNumber);
