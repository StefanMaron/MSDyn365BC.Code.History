table 270 "Bank Account"
{
    ...
    fields
    {
        ...
        field(10006; "Last E-Pay Export File Name"; Text[30])
        {
            ...
            begin
                if ("Last E-Pay Export File Name" <> '') and not ValidateIncrementableText("Last E-Pay Export File Name") then
                    Error(NonIncrementingTextErr, FieldCaption("Last E-Pay Export File Name"));
            end;
        }
        ...
        field(10010; "Last Remittance Advice No."; Code[20])
        {
            ...
            begin
                if ("Last Remittance Advice No." <> '') and not ValidateIncrementableText("Last Remittance Advice No.") then
                    Error(NonIncrementingTextErr, FieldCaption("Last Remittance Advice No."));
            end;
        }
        field(10011; "Export Format"; Option)
        {
            ...
            OptionCaption = ',US,CA,MX,Other';
            OptionMembers = ,US,CA,MX,Other;
        }
    ...
    var
        ...
        Text004Msg: Label 'Before you can use Online Map, you must fill in the Online Map Setup window.\See Setting Up Online Map in Help.';
    ...
    begin
        ...
        if not ContBusRel.FindFirst() then begin
            if not Confirm(Text003Qst, false, TableCaption(), "No.") then
                exit;
            BankContUpdate.InsertNewContact(Rec, false);
    ...
    [TryFunction]
    local procedure ValidateIncrementableText(ValidateText: Text)
    begin
    ...
    [Scope('OnPrem')]
    procedure GetEFTCodeunitID(): Integer
    var
        BankExportImportSetup: Record "Bank Export/Import Setup";
    begin
        if "EFT Export Code" <> '' then
            BankExportImportSetup.Get("EFT Export Code")
        else
            BankExportImportSetup.Get("Payment Export Format");
        exit(BankExportImportSetup."Processing Codeunit ID");
    end;
