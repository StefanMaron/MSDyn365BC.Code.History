...
codeunit 148052 "OIOUBL-ERM Misc Elec. Invoice"
{
    ...
    begin
        ...
        // Verify: Verify Description and Name generated XML.
        VerifyElectronicDocumentMultipleNodeValues(DocumentNo);
    end;
    ...
    begin
        ...
        VerifyValuesOnElectronicDocument(DocumentNo, cbcAmountInclTaxCapTxt, SalesHeader."Amount Including VAT");
    end;
    ...
    begin
        ...
        VerifyValuesOnElectronicDocument(DocumentNo, cbcAmountInclTaxCapTxt, 0.0);
    end;
    ...
    local procedure ModifyContactPage(ContactNo: Code[20]);
    var
    ...
    local procedure VerifyElectronicDocumentMultipleNodeValues(PostedDocumentNo: Code[20]);
    var
    // Node : DotNet "'System.Xml, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b77a5c561934e089'.System.Xml.XmlNode" RUNONCLIENT;
    // NodeList : DotNet "'System.Xml, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b77a5c561934e089'.System.Xml.XmlNodeList" RUNONCLIENT;
    // ElementValue : Text;
    begin
        // TODO
        // LibraryXMLRead.InitializeOnServer(STRSUBSTNO(VerifyPathCap,TEMPORARYPATH,PostedDocumentNo)); // Initialize generated Electronic Invoice Document.
        // LibraryXMLRead.GetNodeListByElementName(cacItemCap,NodeList);
        // Node := NodeList.Item(0);
        // ElementValue := Node.Item(cbcNameCap).InnerText;
        // Assert.IsTrue(STRLEN(ElementValue) = 40,'Length should not be greater that 40 characters.');
        // ElementValue := Node.Item(cbcDescCap).InnerText;
        // Assert.AreEqual(STRSUBSTNO(DescriptionTxt),ElementValue,STRSUBSTNO(ValidationError,'Description',ElementValue));
    end;

    local procedure VerifyValuesOnElectronicDocument(DocumentNo: Code[20]; ElementName: Text[30]; ExpectedValue: Variant);
    begin
        LibraryXMLReadOnServer.Initialize(OIOUBLNewFileMock.PopFilePath());
        VerifyNodeDecimalValue(ElementName, ExpectedValue);
    end;

    local procedure VerifyBankAccInfo(FileName: Code[20]);
    var
        CompanyInfo: Record "Company Information";
    begin
        CompanyInfo.GET();
