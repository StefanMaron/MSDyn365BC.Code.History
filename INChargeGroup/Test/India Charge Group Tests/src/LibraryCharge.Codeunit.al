codeunit 18992 "Library - Charge"
{
    ...
    procedure CreateGLAccountNoGenPostingSetup(): Code[20]
    var
        GLAccount: Record "G/L Account";
        VATPostingSetup: Record "VAT Posting Setup";
        ChargeGroupGenProductPostingGroup: Code[20];
    begin
        CreateZeroVATPostingSetup(VATPostingSetup);
        LibraryERM.CreateGLAccount(GLAccount);
        CreateGeneralPostingSetupForChargeGroup(GenBusinessPostingGroup.Code, ChargeGroupGenProductPostingGroup);
        GLAccount.Validate("Gen. Bus. Posting Group", GenBusinessPostingGroup.Code);
        GLAccount.Validate("Gen. Prod. Posting Group", ChargeGroupGenProductPostingGroup);
        GLAccount.Validate("Gen. Posting Type", GLAccount."Gen. Posting Type"::Purchase);
        GLAccount.Validate("VAT Prod. Posting Group", VATPostingSetup."VAT Prod. Posting Group");
        GLAccount.Modify(true);
        exit(GLAccount."No.");
    ...
    procedure CreateGLAccWithGSTDetailsForChargeGroup(var VATPostingSetup: Record "VAT Posting Setup"; GSTGroupCode: Code[20]; HSNSACCode: Code[10]; Availment: Boolean; GLExempted: Boolean): Code[20]
    var
        GLAccount: Record "G/L Account";
        GLAccNo: Code[20];
    begin
        CreateZeroVATPostingSetup(VATPostingSetup);
        GLAccNo := CreateGLAccountNoGenPostingSetup();
        GLAccount.Get(GLAccNo);
        GLAccount.Validate("GST Group Code", GSTGroupCode);
        GLAccount.Validate("HSN/SAC Code", HSNSACCode);
        if Availment then
    ...
    procedure CreateChargeItemWithGSTDetailsForChargeGroup(
        var VATPostingSetup: Record "VAT Posting Setup";
        GSTGroupCode: Code[20];
        HSNSACCode: Code[10];
        Availment: Boolean;
    ...
    begin
        CreateZeroVATPostingSetup(VATPostingSetup);
        ItemChargeNo := CreateChargeItemWithGenPostingSetup();
    ...
    local procedure CreateChargeItemWithGenPostingSetup(): Code[20]
    var
        ItemCharge: Record "Item Charge";
        VATPostingSetup: Record "VAT Posting Setup";
        ChargeGroupGenProductPostingGroup: Code[20];
    begin
        ...
        LibraryInventory.CreateItemCharge(ItemCharge);
        CreateGeneralPostingSetupForChargeGroup(GenBusinessPostingGroup.Code, ChargeGroupGenProductPostingGroup);
        ItemCharge.Validate("Gen. Prod. Posting Group", ChargeGroupGenProductPostingGroup);
        ItemCharge.Validate("VAT Prod. Posting Group", VATPostingSetup."VAT Prod. Posting Group");
        ItemCharge.Modify(true);
        exit(ItemCharge."No.");
    end;

    procedure CreateGeneralPostingSetupForChargeGroup(GenBusinessPostingGroup: Code[20]; var GenProductPostingGroup: Code[20])
    var
        ChargeGroupGenProductPostingGroup: Record "Gen. Product Posting Group";
    begin
        LibraryERM.CreateGenProdPostingGroup(ChargeGroupGenProductPostingGroup);
        GenProductPostingGroup := ChargeGroupGenProductPostingGroup.Code;
        CreateGeneralPostingSetup(GenBusinessPostingGroup, GenProductPostingGroup);
    end;
